...
{
    public class CLGTTGenerationImpl : Interfaces.CLGTTGenerationInterface
    {
        ...
        public CLGTTGenerationImpl(TTContext ttcategory)
        {
        ...
        public CLGTTGenerationDTO getdetails(CLGTTGenerationDTO data)
        {
            try
            {
                data.versionlist = (from a in _ttcontext.TT_Final_GenerationDMO
                ...
                data.academiclist = _ttcontext.AcademicYear.Where(t => t.MI_Id.Equals(data.MI_Id) && t.Is_Active == true).OrderByDescending(y=>y.ASMAY_Order).ToList().ToArray();
                data.catelist = _ttcontext.TTMasterCategoryDMO.Where(t => t.MI_Id.Equals(data.MI_Id) && t.TTMC_ActiveFlag.Equals(true)).ToList().ToArray();
            }
            catch (Exception ee)
            {
        ...
        public CLGTTGenerationDTO get_count(CLGTTGenerationDTO data)
        {
            try
            {
                List<CLGTTGenerationDTO> result = new List<CLGTTGenerationDTO>();
                List<CLGTTGenerationDTO> result1 = new List<CLGTTGenerationDTO>();
                List<CLGTTGenerationDTO> result2 = new List<CLGTTGenerationDTO>();
                ...
                {
                    cmd.CommandText = "TT_CLG_GET_WORKLOAD_AND_COUNT";
                    ...
                    cmd.Parameters.Add(new SqlParameter("@TTMCIDs", SqlDbType.VarChar) { Value = ttmcs_ids });
                    ...
                    try
                    {
                        using (var dataReader = cmd.ExecuteReader())
                        {
                            ...
                            {
                                result.Add(new CLGTTGenerationDTO
                                {
                                    ...
                                    ACMS_SectionName = dataReader["ACMS_SectionName"].ToString(),
                            ...
                            if (dataReader.NextResult())
                            {
                                while (dataReader.Read())
                                {
                                    result1.Add(new CLGTTGenerationDTO
                                    {
                                        AMCO_CourseName = dataReader["AMCO_CourseName"].ToString(),
                                        AMB_BranchName = dataReader["AMB_BranchName"].ToString(),
                                        AMSE_SEMName = dataReader["AMSE_SEMName"].ToString(),
                            ...
                            if (dataReader.NextResult())
                            {
                                while (dataReader.Read())
                                {
                                    result2.Add(new CLGTTGenerationDTO
                                    {
        ...
        //generation
        public CLGTTGenerationDTO generate(CLGTTGenerationDTO _category)
        {
            try
            {
                var ttmcs_ids = "";
                ...
                using (var cmd = _ttcontext.Database.GetDbConnection().CreateCommand())
                {
                    ...
                    cmd.CommandText = "tt_final_gen_process_College";
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public CLGTTGenerationDTO getalldetailsviewrecords(CLGTTGenerationDTO _category)
        {
            CLGTTGenerationDTO objpge = Mapper.Map<CLGTTGenerationDTO>(_category);
            List<CLGTTGenerationDTO> list = new List<CLGTTGenerationDTO>();
            try
            {
                ...
                for (int i = 0; i < 1; i++)
                {
                    ...
                    foreach (CLGTTGenerationDTO dto in objpge.Time_Table)
                    {
                        list.Add(dto);
                    }
                }
        ...
        public CLGTTGenerationDTO saveTemptomain(CLGTTGenerationDTO _category)
        {
            try
            {
        ...
        public CLGTTGenerationDTO getreplacemntdetailsviewrecords(CLGTTGenerationDTO _category)
        {
            CLGTTGenerationDTO objpge = Mapper.Map<CLGTTGenerationDTO>(_category);
            List<CLGTTGenerationDTO> list = new List<CLGTTGenerationDTO>();
            try
        ...
        public CLGTTGenerationDTO resetTT(CLGTTGenerationDTO data)
        {
            try
            {
                ...
                {
                    cmd.CommandTimeout = 0;
                    cmd.CommandText = "TT_RESET_College";
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public CLGTTGenerationDTO deactivate(CLGTTGenerationDTO acd)
        {
            try
            {
