...
namespace TransportServiceHub.Services
{
    ...
    public class CLGBusPassImpl : Interfaces.CLGBusPassInterface
    {
        public TransportContext _TransportContext;
        public ILogger<CLGBusPassDTO> _log;
        public CLGBusPassImpl(TransportContext _context, ILogger<CLGBusPassDTO> log)
        {
        ...
        {
            CLGBusPassDTO data = new CLGBusPassDTO();
            try
            {
                ...
                data.getcourse = _TransportContext.MasterCourseDMO.Where(a => a.MI_Id == id && a.AMCO_ActiveFlag == true).ToArray();
        ...
        public CLGBusPassDTO searchdetails(CLGBusPassDTO data)
        {
            try
            {
                List<CLGBusPassDTO> details = new List<CLGBusPassDTO>();
                ...
                {
                    cmd.CommandText = "TransportBusPass_Details_Collage";
                ...
                List<CLGBusPassDTO> temp_group = new List<CLGBusPassDTO>();
                ...
                List<CLGBusPassDTO> lstopebal = new List<CLGBusPassDTO>();
                List<CLGBusPassDTO> lstopebal1 = new List<CLGBusPassDTO>();
                List<long> amstids = new List<long>();

                var amstlst = _TransportContext.CLGAdm_Std_Transport_ApplicationDMO.Where(e => e.MI_Id == data.MI_Id && e.ASTACO_ForAY == data.ASMAY_Id && e.ASTACO_ApplStatus == "Approved" && e.ASTACO_ActiveFlag == true 
                /*&& e.ASTACO_Amount > 0*/
                ).Distinct().ToList();

                foreach (var item in amstlst)
                { 
                    amstids.Add(item.AMCST_Id);
                    if(amstidss=="")
                    {
                        amstidss = item.AMCST_Id.ToString();
                    }
                    else
                    {
                        amstidss = amstidss+"," +item.AMCST_Id.ToString();
                    }
                }
                ...
                using (var cmd = _TransportContext.Database.GetDbConnection().CreateCommand())
                {
                    cmd.CommandText = "Fee_Buspass_Terms_Collage";
                    cmd.CommandType = CommandType.StoredProcedure;
              ...
              SqlDbType.BigInt)
                    ...
                    cmd.Parameters.Add(new SqlParameter("@amcstids",
              SqlDbType.VarChar)
                    {
                        Value = amstidss
                    });
                ...
                using (var cmd = _TransportContext.Database.GetDbConnection().CreateCommand())
                {
                    cmd.CommandText = "Fee_Buspass_Excess_OP_Bal_Collage";
                    cmd.CommandType = CommandType.StoredProcedure;
              ...
              SqlDbType.VarChar)
                ...
                data.transportcharges = (from a in _TransportContext.Fee_College_Student_StatusDMO
                                         ...
                                         where (a.FMG_Id == b.FMG_Id && a.FMH_Id == c.FMH_Id && a.FMH_Id == d.FMH_Id && a.FTI_Id == d.FTI_Id && d.FMT_Id == e.FMT_Id &&  amstids.Contains(a.AMCST_Id) && a.ASMAY_Id == data.ASMAY_Id && c.FMH_Flag == "T")
                                         select new CLGBusPassDTO
                                         {
                                             AMCST_Id=a.AMCST_Id,
                                             payableamount = a.FCSS_ToBePaid,
                                             termname = e.FMT_Name,
                ...
                data.transportchargespaid = (from a in _TransportContext.Fee_College_Student_StatusDMO
                                             from b in _TransportContext.FeeGroupDMO
                                             from c in _TransportContext.FeeHeadDMO
                                             from d in _TransportContext.FeeMasterTermHeadsDMO
                                             from e in _TransportContext.FeeTermDMO
                                             from f in _TransportContext.Fee_Y_Payment_College_Student
                                             from g in _TransportContext.Fee_T_College_PaymentDMO
                                             from h in _TransportContext.CLGFee_Y_PaymentDMO
                                             where (a.FMG_Id == b.FMG_Id && a.FMH_Id == c.FMH_Id && a.FMH_Id == d.FMH_Id && a.FTI_Id == d.FTI_Id && d.FMT_Id == e.FMT_Id && amstids.Contains(a.AMCST_Id) && a.ASMAY_Id == data.ASMAY_Id && c.FMH_Flag == "T" && f.AMCST_Id == a.AMCST_Id && f.FYP_Id == g.FYP_Id && g.FYP_Id == h.FYP_Id && g.FCMAS_Id == a.FCMAS_Id)
                                             select new CLGBusPassDTO
                                             {
                                                 AMCST_Id = a.AMCST_Id,
                                                 paidamount = a.FCSS_PaidAmount,
                                                 termname = e.FMT_Name,
                                                 headname = c.FMH_FeeName,
                                                 FMT_Id = e.FMT_Id,
                                                 FMT_Order = e.FMT_Order,
                                                 receiptno = h.FYP_ReceiptNo,
                                                 paiddate = h.FYP_ReceiptDate,
                                                 FMH_RefundFlag = c.FMH_RefundFlag
                                             }).OrderBy(t => t.FMT_Order).ToArray();



                //data.openingbalance = _TransportContext.FeeStudentTransactionDMO.Where(r => r.MI_Id == data.MI_Id).Select(r => r.FSS_ToBePaid).GroupBy(id => id).OrderByDescending(id => id.Sum()).Select(g => new CLGBusPassDTO { AMST_Id = g.Key, openingbalance = g.Sum() });
        ...
        public async Task<CLGBusPassDTO> showmodaldetails(CLGBusPassDTO data)
        {
            try
            {
