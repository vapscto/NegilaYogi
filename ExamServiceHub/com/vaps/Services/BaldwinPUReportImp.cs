...
{
    public class BaldwinPUReportImp : Interfaces.BaldwinPUReportInterface
    {
        ...
        ILogger<BaldwinPUReportImp> _acdimpl;
        public DomainModelMsSqlServerContext _db;
        public BaldwinPUReportImp(ExamContext cpContext, DomainModelMsSqlServerContext db, ILogger<BaldwinPUReportImp> _acdi)
        {
        ...
        public async Task<BaldwinAllReportDTO> savedetails(BaldwinAllReportDTO data)
        {
            ...
            // BaldwinAllReportDTO savedata = new BaldwinAllReportDTO();
            try
            {
                ...
                {
                    cmd.CommandText = "Exam_get_BB_Exam_Details_PU";
                        ...
                        using (var dataReader = await cmd.ExecuteReaderAsync())
                        {
                            while (await dataReader.ReadAsync())
                            {
                                result.Add(new BaldwinAllReportDTO
                                {
                                    ...
                                    AMST_AdmNo = (dataReader["AMST_RegistrationNo"].ToString() == null || dataReader["AMST_RegistrationNo"].ToString() == "" ? "" : dataReader["AMST_RegistrationNo"].ToString()),
                                    ...
                                    ESTMPSSS_ObtainedMarks = Convert.ToDecimal(dataReader["ESTMPSSS_ObtainedMarks"].ToString() == null || dataReader["ESTMPSSS_ObtainedMarks"].ToString() == "" ? "0" : dataReader["ESTMPSSS_ObtainedMarks"].ToString())
                ...
                data.savelisttot = _BaldwinAllReportContext.ExmStudentMarksProcessDMO.Where(t => t.ASMAY_Id == data.ASMAY_Id && t.ASMCL_Id == data.ASMCL_Id && t.MI_Id == data.MI_Id && t.ASMS_Id == data.ASMS_Id && t.EME_Id == data.EME_Id).Distinct().ToArray();

                data.subjlist = data.savelist.Distinct<BaldwinAllReportDTO>(new progressEqualityComparer1()).OrderBy(t => t.EYCES_SubjectOrder).ToArray();
    ...
    class progressEqualityComparer1 : IEqualityComparer<BaldwinAllReportDTO>
    {
