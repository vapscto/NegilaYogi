...
{
    public class CollegeDailyCollectionReportImpl: CollegeDailyCollectionReportInterface
    {

        public CollFeeGroupContext _ClgAdmissionContext;
        readonly ILogger<CollegemasterstudentconcessionImpl> _logger;
        public CollegeDailyCollectionReportImpl(CollFeeGroupContext _ClgAdmissionCon, ILogger<CollegemasterstudentconcessionImpl> log)
        {
            _logger = log;
        ...
        public CollegeConcessionDTO getdetails(CollegeConcessionDTO dt)
        {
            try
            {
                ...
                feemasnum = _ClgAdmissionContext.feemastersettings.Where(t => t.MI_Id == dt.MI_Id).ToList();//&& t.userid == data.UserId
                dt.feeconfig = feemasnum.ToArray();
                ...
                try
                {
                    using (var cmd1 = _ClgAdmissionContext.Database.GetDbConnection().CreateCommand())
                    {
                        ...
                        try
                        {
                            ...
                            dt.userdetails = retObject1.ToArray();
                        }
                ...
                dt.fetchmodeofpayment = (from a in _ClgAdmissionContext.IVRM_ModeOfPayment
                                           where (a.MI_Id == dt.MI_Id)
        ...
        public async Task<CollegeConcessionDTO> Getreportdetails(CollegeConcessionDTO data)
        {
            long fmgg_id = 0;
            var amco_ids = "";
            ...
            try
            {
                    using (var cmd1 = _ClgAdmissionContext.Database.GetDbConnection().CreateCommand())
                    {
                        cmd1.CommandText = "College_Daily_Collection_Report";
                        cmd1.CommandType = CommandType.StoredProcedure;
                    ...
                    cmd1.Parameters.Add(new SqlParameter("@amse_ids",
                        ...
                        cmd1.Parameters.Add(new SqlParameter("@type",
                          SqlDbType.VarChar)
                        {
                            Value = data.allorindivflag
                        });
                        cmd1.Parameters.Add(new SqlParameter("@done_by",
                          SqlDbType.VarChar)
                        {
                            Value = data.allorstdorothersflag
                        });
                        cmd1.Parameters.Add(new SqlParameter("@trans_by",
                          SqlDbType.VarChar)
                        {
                            Value = data.allorcorchoronlineflag
                        });
                        ...
                        cmd1.Parameters.Add(new SqlParameter("@Settlement",
                            SqlDbType.VarChar)
                        {
                            Value = data.rpttypmkckset
                        });
                    cmd1.Parameters.Add(new SqlParameter("@transdate",
                           SqlDbType.VarChar)
                    {
                        Value = data.type
                    });
