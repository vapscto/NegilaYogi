...
{
    public class SportHouseReportImpl : Interfaces.SportHouseReportInterface
    {
        private static ConcurrentDictionary<string, House_Report_DTO> _login =
   new ConcurrentDictionary<string, House_Report_DTO>();

        private readonly SportsContext _sportcontext;

        public SportHouseReportImpl(SportsContext sportcontext)
        {
        ...
        public House_Report_DTO Getdetails(House_Report_DTO data)//int IVRMM_Id
        {
            ...
            try
            {
                ...
                data.exam_list = _sportcontext.exammasterDMO.Where(R => R.EME_ActiveFlag == true && R.MI_Id == data.MI_Id).Distinct().ToArray();
        ...
        public House_Report_DTO get_class(House_Report_DTO dto)
        {
            try
            {
                ...
                dto.sectionList = _sportcontext.masterSection.Where(R => R.MI_Id == dto.MI_Id).Distinct().ToArray();
                //
                //dto.sectionList = (from a in _sportcontext.admissionyearstudent
                //                   from b in _sportcontext.masterSection

                //                   where (a.ASMAY_Id == dto.ASMAY_Id && a.AMAY_ActiveFlag == 1 && a.ASMS_Id == b.ASMS_Id && b.ASMC_ActiveFlag == 1 && b.MI_Id == dto.MI_Id)
        ...
        public House_Report_DTO get_section(House_Report_DTO dto)
        {
            try
            {
                dto.sectionList = (from a in _sportcontext.admissionyearstudent
        ...
        public async Task<House_Report_DTO> showdetails(House_Report_DTO data)
        {
            try
            {
                string hous_idss = "0";
                ...
                {
                    cmd.CommandText = "SPC_HouseORClassSectionWiseReport";
        ...
        public async Task<House_Report_DTO> showdetailsNew(House_Report_DTO data)
        {
            try
            {
                string hous_idss = "0";
                ...
                string Class_idss = "0";
                string Eme_Id = "0";
                if (data.selectedhouselist != null)
                {
                    foreach (var i in data.selectedhouselist)
                    {
                        hous_idss = hous_idss + ',' + i.SPCCMH_Id;
                    }
                }
                if (data.selectedSectionlist != null)
                {
                    foreach (var i in data.selectedSectionlist)
                    {
                        section_idss = section_idss + ',' + i.ASMS_Id;
                    }
                }
                if (data.Classelectedlist != null)
                {
                    foreach (var i in data.Classelectedlist)
                    {
                        Class_idss = Class_idss + ',' + i.ASMCL_Id;
                    }
                }
                if(data.ExamListHouses  !=null )
                {
                    foreach (var f in data.ExamListHouses)
                    {
                        Eme_Id = Eme_Id +',' + f.EME_Id;
                    }
                }
                using (var cmd = _sportcontext.Database.GetDbConnection().CreateCommand())
                {
                    cmd.CommandText = "SP_Housewise_Studentcountdetails";
                    cmd.CommandType = CommandType.StoredProcedure;
                ...
                using (var cmd = _sportcontext.Database.GetDbConnection().CreateCommand())
                {
                   ...
                   SqlDbType.VarChar)
                    ...
                    try
                    {
                        ...
                        data.HouseTotal = retObject.ToArray();
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine(ex.Message);
                ...
                using (var cmd = _sportcontext.Database.GetDbConnection().CreateCommand())
                {
                   ...
                   SqlDbType.VarChar)
                    ...
                    try
                    {
                        ...
                        data.OverallCount = retObject.ToArray();
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine(ex.Message);
