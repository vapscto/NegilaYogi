...
(function () {
    ...
    angular
.module('app')
        .controller('LeaveStatusReportController', LeaveStatusReportController)
    LeaveStatusReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$stateParams', '$filter', 'Excel', '$timeout', 'superCache']
    function LeaveStatusReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $stateParams, $filter, Excel, $timeout, superCache) {
        $scope.editEmployee = {};
        ...
        $scope.loadData = function () {
            ...
            var id = 2;
            // $scope.all_check();
            apiService.getURI("LeaveStatusReport/getleavereport", id).
       then(function (promise) {
        ...
        $scope.get_designation = function () {
            ...
            if ($scope.selectedemptypes.length != 0) {
                ...
                apiService.create("LeaveStatusReport/get_designation", data).
                       then(function (promise) {
                           $scope.Designation_types = promise.designation_types;
                           $scope.count1 = $scope.Designation_types.length;
        ...
        $scope.get_Employees = function () {
            ...
            if ($scope.selectedemptypes.length != 0) {
                ...
                apiService.create("LeaveStatusReport/get_Employees", data).
                       then(function (promise) {
        ...
        $scope.get_report = function () {
            ...
            if ($scope.myForm.$valid) {
               ...
               $scope.to_date = $scope.myDate_to;
                ...
                var data = {
                    //selectedreport: array1,
                    // "HRME_Id": $scope.emplname,
                    "HRML_Id": $scope.hrmL_Id,
                    //"Month": $scope.classmonth,
                    "HRMLY_Id": $scope.yearname,
                ...
                apiService.create("LeaveStatusReport/get_report", data).
                    then(function (promise) {
                        $scope.EmployeeDis = true;
                        $scope.activityIds = promise.activityIds;  
                        // $scope.gridReport.data = promise.activityIds;
