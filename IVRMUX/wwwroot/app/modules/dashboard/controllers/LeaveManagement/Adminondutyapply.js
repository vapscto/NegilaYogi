...
(function () {
    ...
    function AdminondutyapplyController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $q, $stateParams, $filter, $sce, $window) {
        ...
        $scope.loadData = function () {            var pageid = 2;            apiService.getURI("Adminondutyapply/Getdetails", pageid).then(function (promise) {                               $scope.employeename = promise.employeeList;                $scope.leave_name = promise.leave_name;            })        };
        ...
        $scope.editDetails = function (obj) {
            ...
            apiService.create("Adminondutyapply/editData", data).
                then(function (promise) {
                    ...
                    $scope.newcaste = {};
                    if ($scope.employeename != null && $scope.employeename.length > 0) {                        for (var i = 0; i < $scope.employeename.length; i++) {                            if (promise.editresults[0].hrmE_Id == $scope.employeename[i].hrmE_Id) {                                $scope.employeename[i].Selected = true;                                $scope.obj.hrmE_Id = $scope.employeename[i];                                $scope.newcaste = $scope.employeename[0].hrmE_Id;                                return;                            }                        }                    }
                })
        }
