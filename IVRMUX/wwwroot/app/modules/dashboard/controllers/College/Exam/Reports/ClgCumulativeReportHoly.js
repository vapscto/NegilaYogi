...
(function () {
    ...
    function ClgCumulativeReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout) {
        ...
        $scope.otherdaydatechange = function () {
            $scope.datagriv = false;
        }
        // TO Save The Data
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("ClgCumulativeReport/Getcmreport", data).
                    then(function (promise) {
                        ...
                        if (promise.savelist !== null && $scope.rdopunch == 'Format_I') {
                            $scope.datagriv = true;
                            ...
                            if (promise.getStudentWiseSubjectMakrs != null && promise.getStudentWiseSubjectMakrs.length > 0) {
                                $scope.TotalStudents = promise.getStudentWiseSubjectMakrs;

                            }


                        }
                        else if (promise.schmetypelist != null && promise.schmetypelist.length > 0 && $scope.rdopunch == 'Format_II') {
                            $scope.datagriv = true;
                            $scope.schmetypelist = promise.schmetypelist;
                            $scope.employeeid = [];
                            angular.forEach($scope.schmetypelist, function (dev) {
                            ...
                            angular.forEach($scope.employeeid, function (ddd) {
                                $scope.templist = [];
                                angular.forEach($scope.schmetypelist, function (dd) {
                                    if (dd.ISMS_Id === ddd.ISMS_Id) {
                                        $scope.templist.push(dd);
                                    }
                                });
                                ddd.schmetypelist = $scope.templist;

                            });
                            $scope.schmetypelistTemp = [];
                            $scope.schmetypelistTemp.push({
                                TotalStudents: "TotalStudents",

                            })
                            $scope.schmetypelistTemp.push({
                                TotalStudents: "AppearedStudents",
