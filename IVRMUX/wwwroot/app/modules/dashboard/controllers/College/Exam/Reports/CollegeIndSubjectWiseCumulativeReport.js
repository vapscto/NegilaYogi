...
(function () {
    'use strict';
    angular.module('app').controller('CollegeIndSubjectWiseCumulativeReportController', CollegeIndSubjectWiseCumulativeReportController)

    CollegeIndSubjectWiseCumulativeReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout']
    function CollegeIndSubjectWiseCumulativeReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout) {
        ...
        $scope.getindreport = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("CollegeCumulativeAvgBestReport/getindreport", data).
                    then(function (promise) {
                        
                        if (promise.reportinddata !== null && promise.reportinddata.length > 0) {
                            $scope.datagriv = true;
                            $scope.reportd = false;
                            ...
                            $scope.reportdata = promise.reportinddata;

                            $scope.maxmarks = $scope.reportdata[0].ecyseS_MaxMarks;
                            ...
                            angular.forEach($scope.exam_list, function (itm) {
                                if (itm.emE_Id === parseInt($scope.EME_Id)) {
                                    $scope.examname = itm.emE_ExamName;
                                }
                            });
