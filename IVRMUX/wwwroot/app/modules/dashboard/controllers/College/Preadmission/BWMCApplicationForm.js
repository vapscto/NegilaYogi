(function () {
    ...
    angular
        .module('app')
        .controller('BWMCApplicationFormController', BWMCApplicationFormController);


    BWMCApplicationFormController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$q', '$cookieStore', '$stateParams', '$filter', 'FormSubmitter', '$window', 'superCache', '$compile']
    function BWMCApplicationFormController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $q, $cookieStore, $stateParams, $filter, FormSubmitter, $window, superCache, $compile) {
        ...
        $scope.showprintdata = function (studentid) {
            ...
            apiService.getURI("ApplicationForm/getprintdata/", studentid).then(function (promise) {
                ...
                if (promise.studentList != null || promise.studentList.length > 0) {
                    ...
                    $scope.fnamearray = [];
                    if ($scope.PACA_FatherName != null && $scope.PACA_FatherName.length > 0) {
                        for (var i = 0; i < $scope.PACA_FatherName.length; i++) {
                            debugger;

                            $scope.fnamearray.push({ id: i, name: $scope.PACA_FatherName[i] });

                        }
                    }
                    else {
                        if ($scope.fnamearray.length < 10) {
                            for (var i = 0; i < $scope.namearray.length; i++) {
                                debugger;
                                if ($scope.fnamearray.length < 10) {
                                    $scope.fnamearray.push({ name: " " });
                                }
                    ...
                    $scope.mnamearray = [];
                    if ($scope.PACA_MotherName != null && $scope.PACA_MotherName.length > 0) {
                        for (var i = 0; i < $scope.PACA_MotherName.length; i++) {
                            debugger;

                            $scope.mnamearray.push({ id: i, name: $scope.PACA_MotherName[i] });

                        }
                    }
                    else {
                        if ($scope.mnamearray.length < 18) {
                            for (var i = 0; i < $scope.namearray.length; i++) {
                                debugger;
                                if ($scope.mnamearray.length < 18) {
                                    $scope.mnamearray.push({ name: " " });
                                }
                    ...
                    if (promise.studentGuardianDetails != null) {
                        if (promise.studentGuardianDetails.length > 0) {
                            ...
                            $scope.gnamearray = [];
                            if ($scope.PACSTG_GuardianName != null && $scope.PACSTG_GuardianName.length > 0) {
                                for (var i = 0; i < $scope.PACSTG_GuardianName.length; i++) {
                                    debugger;

                                    $scope.gnamearray.push({ id: i, name: $scope.PACSTG_GuardianName[i] });

                                }
                            }
                            else {
                                if ($scope.gnamearray.length < 18) {
                                    for (var i = 0; i < $scope.namearray.length; i++) {
                                        debugger;
                                        if ($scope.gnamearray.length < 18) {
                                            $scope.gnamearray.push({ name: " " });
                                        }
                    ...
                    $scope.namearray = [];
                    if ($scope.PACA_FirstName != null && $scope.PACA_FirstName.length > 0) {
                        for (var i = 0; i < $scope.PACA_FirstName.length; i++) {


                            $scope.namearray.push({ id: i, name: $scope.PACA_FirstName[i] });

                        }
                    }
                    //if ($scope.namearray.length<40) {
                    //    for (var i = 0; i < $scope.namearray.length; i++) {
