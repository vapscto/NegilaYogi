(function () {
    ...
    angular
        .module('app')
        .controller('BMCApplicationFormController', BMCApplicationFormController);
    BMCApplicationFormController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$q', '$cookieStore', '$stateParams', '$filter', 'FormSubmitter', '$window', 'superCache', '$compile']
    function BMCApplicationFormController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $q, $cookieStore, $stateParams, $filter, FormSubmitter, $window, superCache, $compile) {
            ...
            apiService.create("ApplicationForm/Getdetails/", data).
                then(function (promise) {
                    ...
                    for (var i = 0; i < $scope.allAcademicYear.length; i++) {
                        ...
                        $scope.onYearCahnge($scope.yearId);
                    }
            ...
            apiService.getURI("ApplicationForm/Edit/", MEditId).
                then(function (promise) {
                    ...
                    if (promise.studentList != null || promise.studentList.length > 0) {
                        ...
                        if (promise.prevSchoolDetails != null && promise.prevSchoolDetails.length > 0) {
                            if (promise.prevSchoolDetails.length > 0) {
                                ...
                                for (var i = 0; i < promise.prevSchoolDetails.length; i++) {
                                    ...
                                    $scope.prevSchoolDetails[i].pacstpS_PreSchoolPlace = promise.prevSchoolDetails[i].pacstpS_PreSchoolPlace;
                                    $scope.prevSchoolDetails[i].pacstpS_PreSchoolDistrict = promise.prevSchoolDetails[i].pacstpS_PreSchoolDistrict;
                        ...
                        if (promise.studentList[0].pacA_FatherMobleNo === 0) {
                        ...
                        if (promise.studentList[0].pacA_MotherMobleNo === 0) {
        ...
        $scope.showprintdata = function (studentid) {
            ...
            apiService.getURI("ApplicationForm/getprintdata/", studentid).then(function (promise) {
                ...
                if (promise.studentList != null || promise.studentList.length > 0) {
                    ...
                    if (promise.subjectlistlag != null && promise.subjectlistlag.length > 0) {

                        angular.forEach(promise.subjectlistlag, function (ff) {
                            if (ff.ismS_SubjectName.toLowerCase() != 'english') {
                                $scope.seconndlan = ff.ismS_SubjectName;
                            }
                    ...
                    $scope.pacA_PassportIssuedCounrtyname = promise.studentpasissuecountry.length > 0 ? promise.studentpasissuecountry[0].studconcountry : "";
                    ...
                    if (promise.achievementdata != null && promise.achievementdata.length > 0) {
                        ...
                        angular.forEach($scope.achievements, function (ee) {
                            if ($scope.achievementsdata == '') {
                                $scope.achievementsdata = ee.pacsaT_AchivementsName + '(' + ee.pacsaT_type + ')';
                            }
                            else {
                                $scope.achievementsdata = $scope.achievementsdata + ',' + +' ' + ee.pacsaT_AchivementsName + '(' + ee.pacsaT_type + ')';
                            }
                    ...
                    if (promise.prevSchoolDetails != null) {
                        if (promise.prevSchoolDetails.length > 0) {
                            ...
                            for (var i = 0; i < promise.prevSchoolDetails.length; i++) {
                                ...
                                $scope.pacstpS_PreSchoolPlace = promise.prevSchoolDetails[i].pacstpS_PreSchoolPlace;
                                $scope.pacstpS_PreSchoolDistrict = promise.prevSchoolDetails[i].pacstpS_PreSchoolDistrict;
                                ...
                                $scope.studentpreviouscountry = promise.studentpreviouscountry.length > 0 ? promise.studentpreviouscountry[0].countryName : "";
                    ...
                    if (promise.studentsubjectmarksarry != null) {
                        if (promise.studentsubjectmarksarry.length > 0) {
                            ...
                            angular.forEach($scope.prevexammarksdetailsprint, function (ee) {
                                if (ee.pacstsuM_MinMarks != null) {
                                    $scope.mintotal = $scope.mintotal + ee.pacstsuM_MinMarks;
                                }

                                if (ee.pacstsuM_MaxMarks != null) {
                                    $scope.maxtotal = $scope.maxtotal + ee.pacstsuM_MaxMarks;
                                }
                            })
                            ...
                            //        if (ee.pacstsuM_SubjectName.toLowerCase()=='hindi' ) {
                            //            $scope.hindi = 'Hindi';
                            //        }
                            //        else if (ee.pacstsuM_SubjectName.toLowerCase() == 'kannada' ) {
                            //            $scope.kannada = 'Kannada';
                            //        }

                            //        else if (ee.pacstsuM_SubjectName.toLowerCase() == 'french') {
                            //            $scope.french = 'French';
                            //        }
                            //        else {
                            //            $scope.other = 'OH';
                            //            if ($scope.otherdata == '' || $scope.otherdata == undefined || $scope.otherdata == null) {
                            //                $scope.otherdata = ee.pacstsuM_SubjectName
                            //            }
                            //            else {
                            //                $scope.otherdata = $scope.otherdata + ',' + ee.pacstsuM_SubjectName
                            //            }
                            ...
                            console.log($scope.prevexammarksdetailsprint);
        ...
        $scope.clear_first_tab = function (data) {
            ...
            $scope.obj.AMSE_Id = "";
            $scope.obj.PACA_DOB = "";
            $scope.obj.PACA_DOB_inwords = "";
            $scope.obj.IMC_Id = "";
            $scope.obj.PACA_StudentSubCaste = "";
            $scope.obj.PACA_BirthPlace = "";
            $scope.obj.PACA_Age = "";
            $scope.obj.PACA_Sex = "";
            ...
            $scope.obj.PACA_AadharNo = 0;
            $scope.obj.PACA_BloodGroup = "";

            $scope.obj.PACA_Taluk = "";
            $scope.obj.PACA_Village = "";
            $scope.obj.PACA_District = "";
            $scope.obj.PACA_PhysicallyChallenged = "";
            //   $scope.mobiles = [{ id: 'mobilesstd' }];
        ...
        $scope.savefinaltab = function () {
            if ($scope.myForm5.$valid) {
                ...
                var PassportIssueDate = $scope.obj.PACA_PassportIssueDate == null ? "" : $filter('date')($scope.obj.PACA_PassportIssueDate, "yyyy-MM-dd");
                var PassportExpiryDate = $scope.obj.PACA_PassportExpiryDate == null ? "" : $filter('date')($scope.obj.PACA_PassportExpiryDate, "yyyy-MM-dd");



                var VISAValidFrom = $scope.obj.PACA_VISAValidFrom == null ? "" : $filter('date')($scope.obj.PACA_VISAValidFrom, "yyyy-MM-dd");

                var VISAValidTo = $scope.obj.PACA_VISAValidTo == null ? "" : $filter('date')($scope.obj.PACA_VISAValidTo, "yyyy-MM-dd");
