...
(function () {
    ...
    TotalCountClgController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$q', '$cookieStore', '$stateParams', '$filter', 'FormSubmitter', '$window', 'superCache', 'Excel', '$timeout']
    function TotalCountClgController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $q, $cookieStore, $stateParams, $filter, FormSubmitter, $window, superCache, Excel, $timeout) {
        ...
        $scope.exporttoexcel = function (ff) {
        ...
        $scope.loaddata = function () {
            var pageid = 2;
            apiService.getURI("TotalCountClg/get_intial_data", pageid).
                then(function (promise) {

                    $scope.yearlst = promise.fillyear;
                    $scope.courselist = promise.fillcourse;
        ...
        $scope.ShowReport = function (enq, AMCO) {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("TotalCountClg/Getdetails", data).
                    then(function (promise) {
                        ...
                        if (promise.searchstudentDetails != null && promise.searchstudentDetails.length > 0) {
                                    ...
                                    if ($scope.reportdetails[i].FirstName != '') {
                                        if ($scope.reportdetails[i].MiddleName !== null) {
                                            if ($scope.reportdetails[i].LastName !== null) {
                                                $scope.albumNameArray1.push({ name: $scope.reportdetails[i].FirstName + " " + $scope.reportdetails[i].MiddleName + " " + $scope.reportdetails[i].LastName, pacA_Id: $scope.reportdetails[i].pacA_Id });
                                            }
                                            else {
                                                $scope.albumNameArray1.push({ name: $scope.reportdetails[i].FirstName + " " + $scope.reportdetails[i].MiddleName, pacA_Id: $scope.reportdetails[i].pacA_Id });
                                            }
                                        }
                                        else {
                                            $scope.albumNameArray1.push({ name: $scope.reportdetails[i].FirstName, pacA_Id: $scope.reportdetails[i].pacA_Id });
                                        }
                            ...
                            if (enq.ReportType != 1 && enq.ReportType != 2 && enq.ReportType != 8) {
                                ...
                                {
                                    name: 'Course', field: 'Course', title: 'COURSE', width: "120px"
                                },
                                {
