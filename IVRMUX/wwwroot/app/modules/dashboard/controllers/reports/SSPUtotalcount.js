...
(function () {
    ...
    angular
        .module('app')
        .controller('SSPUtotalcountController', SSPUtotalcountController)

    SSPUtotalcountController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$q', '$cookieStore', '$stateParams', '$filter', 'FormSubmitter', '$window', 'superCache', 'Excel', '$timeout']
    function SSPUtotalcountController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $q, $cookieStore, $stateParams, $filter, FormSubmitter, $window, superCache, Excel, $timeout) {
                ...
                apiService.create("TotalCountReport/Getdetails", data).
                    then(function (promise) {
                        ...
                        if (promise.searchstudentDetails != null && promise.searchstudentDetails.length > 0) {
                            ...
                            $scope.UserName = promise.searchstudentDetails[0].UserName;
                            ...
                            angular.forEach($scope.reportdetails, function (user) {
                                ...
                                if (user.UserName == 'SSPUAdmin' || user.UserName == 'PREUSER1' || user.UserName == 'PREUSER3' || user.UserName == 'PREUSER2') {
                                    user.UName = 'OffLine'

                                    user.UserNameDisplay = user.name + '(' + user.UName + ')'
                                }
                                else {
                                    user.UName = 'Online'
                                    user.UserNameDisplay = user.name + '(' + user.UName + ')'
                            ...
                            if (enq.ReportType != 1 && enq.ReportType != 2 && enq.ReportType != 8) {
                                    ...
                                    {
                                        name: 'name', field: 'UserNameDisplay', title: 'STUDENT NAME', width: "200Px", textAlign: "center"
                                    },
                                    //{
                                    //    name: 'UserName', field: 'UserNameDisplay',    title: 'UserName', width: "200Px", textAlign: "center"
                                    //},
                                    //{
                                    //    name: 'UName', field: 'UName', title: 'UName', width: "200Px", textAlign: "center"
                                    //},
                                    {
