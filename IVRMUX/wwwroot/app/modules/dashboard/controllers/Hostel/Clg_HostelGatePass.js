(function () {
    ...
    HostelAllotForCLGStudentController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter']
    function HostelAllotForCLGStudentController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter) {
        ...
        $scope.getAllDetail = function () {
            ...
            $scope.search = "";
            var pageid = 2;
            apiService.getURI("HostelAllotForCLGStudent/loaddata", pageid).then(function (promise) {
        ...
        $scope.get_course = function () {
            ...
            apiService.create("HostelAllotForCLGStudent/get_course", data).then(function (promise) {
                if (promise.courseLists.length > 0) {
                    $scope.courseLists = promise.courseLists;
                }
                else {
        ...
        $scope.get_branch = function () {
            ...
            apiService.create("HostelAllotForCLGStudent/get_branch", data).then(function (promise) {
                if (promise.branchLists.length > 0) {
                    $scope.branchLists = promise.branchLists;
                }
                else {
                    swal('Record Not Found!');
                }
        ...
        $scope.get_sem = function () {
            var data = {
                "ASMAY_Id": $scope.asmaY_Id,
                "AMCO_Id": $scope.amcO_Id,
                "AMB_Id": $scope.amB_Id
            }
            apiService.create("HostelAllotForCLGStudent/get_sem", data).then(function (promise) {
                if (promise.semLists.length > 0) {
                    $scope.semLists = promise.semLists;
                    $scope.secLists = promise.secLists;
                }
                else {
                    swal('Record Not Found!');
                }
        ...
        //    }
        //    apiService.create("HostelAllotForCLGStudent/get_sec", data).then(function (promise) {
        //        if (promise.secLists.length > 0) {
        //            $scope.secLists = promise.secLists;
        //        }
        //        else {
        //            swal('Record Not Found!');
        ...
        $scope.get_student = function () {
            ...
            apiService.create("HostelAllotForCLGStudent/get_student", data).then(function (promise) {
