...
(function () {
    ...
    angular
.module('app')
.controller('BCOEDIReportController', BCOEDIReportController)

    BCOEDIReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http']
    function BCOEDIReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http) {
                ...
                apiService.create("BaldwinAllReport/savedetails", data).
                         then(function (promise) {
                             ...
                             if (promise.savelist.length > 0) {
                                 ...
                                 angular.forEach($scope.report_list, function (dd) {
                                     angular.forEach($scope.stud_work_attendence, function (d) {
                                         if (dd.student_id === d.AMST_Id) {
                                             dd.TotalSchoolDays = d.TotalSchoolDays;
                                         }
                                     });

                                     angular.forEach($scope.stud_present_attendence, function (d) {
                                         if (dd.student_id === d.AMST_Id) {
                                             dd.TotalPresentDays = d.TotalPresentDays;
                                         }
                                     });    
