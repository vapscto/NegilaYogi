(function () {
    ...
    angular
        .module('app')
        .controller('Baldwin_Final_P_ReportController', Baldwin_Final_P_ReportController);

    Baldwin_Final_P_ReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$filter'];

    function Baldwin_Final_P_ReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $filter) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'Baldwin_Final_P_Report';
                                ...
                                if (promise.promotion_stumarks != null && promise.promotion_stumarks != "" && promise.promotion_stumarks.length > 0 && promise.promotion_stumarks_grpwise != null && promise.promotion_stumarks_grpwise != "" && promise.promotion_stumarks_grpwise.length > 0) {
                                    ...
                                    angular.forEach($scope.student_list, function (stud) {
                                        ...
                                        angular.forEach(stud.stu_temp_student_marks, function (stu_marks_g) {
                                            ...
                                            angular.forEach(stud.stu_subjects, function (dd) {

                                                if (dd.ismS_Id === stu_marks_g.ismS_Id && dd.flag) {
                                        ...
                                        console.log(stud.stu_Total_Array_Grp);
        ...
        $scope.get_col_span = function () {
            if ($scope.Grade_Show_Flag)
                $scope.flag_col_span = 2;
            else if (!$scope.Grade_Show_Flag)
                $scope.flag_col_span = 1;
        }
    }
})();
