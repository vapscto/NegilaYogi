(function () {
    ...
    angular
        .module('app')
        .controller('Baldwin_Final_P_Report_BBHSController', Baldwin_Final_P_Report_BBHSController);

    Baldwin_Final_P_Report_BBHSController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$filter'];

    function Baldwin_Final_P_Report_BBHSController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $filter) {
        /* jshint validthis:true */
        var vm = this;
        vm.title = 'Baldwin_Final_P_Report_BBHS';
                                ...
                                if (promise.promotion_stumarks != null && promise.promotion_stumarks != "" && promise.promotion_stumarks.length > 0 && promise.promotion_stumarks_grpwise != null && promise.promotion_stumarks_grpwise != "" && promise.promotion_stumarks_grpwise.length > 0) {
                                    ...
                                    angular.forEach($scope.student_list, function (stud) {
                                        ...
                                        angular.forEach(promise.promotion_stumarks, function (subj) {
                                            ...
                                            if (subj.amsT_Id == stud.amsT_Id) {
                                                ...
                                                var applicableflag = 0;
                                                angular.forEach(stud.stu_subjects, function (stu_subj) {
                                                    if (stu_subj.ismS_Id === subj.ismS_Id) {
                                                        applicableflag = stu_subj.flag;
                                                    }

                                                });

                                                $scope.temp_student_marks.push({
                                                    ismS_Id: subj.ismS_Id, grp_marks_o: subject_marks_other,
                                                    grp_marks_f: subject_marks_final, amsT_Id: stud.amsT_Id, applicableflag: applicableflag
                                                });
                                            }
                                        })
                                    ...
                                    angular.forEach($scope.student_list, function (stud) {
                                        ...
                                        angular.forEach(stud.stu_temp_student_marks, function (stu_marks_g) {
                                            ...
                                            if (stu_marks_g.applicableflag === 1) {
                                        ...
                                        angular.forEach(stud.stu_subjects, function (subj) {
                                            angular.forEach(promise.promotion_subectdetails, function (pro_sub_max_marks) {
                                                if (pro_sub_max_marks.ismS_Id == subj.ismS_Id && subj.flag==1) {
