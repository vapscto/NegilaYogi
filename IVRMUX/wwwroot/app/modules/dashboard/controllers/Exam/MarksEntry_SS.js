(function () {
    ...
    angular
        .module('app')
        .controller('MarksEntry_SSController', MarksEntry_SSController);

    MarksEntry_SSController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$window'];

    function MarksEntry_SSController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $window) {
        /* jshint validthis:true */
        //var vm = this;
        //vm.title = 'MarksEntry_SSController';
        // $scope.all = {};
        //activate();
        // $scope.all_s = {};
        //function activate() { }
        ...
        $scope.BindData = function () {
            apiService.getDATA("MarksEntry_SS/Getdetails").
       then(function (promise) {
        ...
        $scope.get_classes = function (ASMAY_Id) {
            ...
            apiService.create("MarksEntry_SS/get_classes", data).
       then(function (promise) {
        ...
        $scope.get_sections = function (ASMCL_Id, ASMAY_Id) {
            ...
            apiService.create("MarksEntry_SS/get_sections", data).
       then(function (promise) {
        ...
        $scope.get_exams = function (ASMS_Id, ASMCL_Id, ASMAY_Id) {
            ...
            apiService.create("MarksEntry_SS/get_exams", data).
       then(function (promise) {
        ...
        $scope.get_subjects = function (ASMS_Id, ASMCL_Id, ASMAY_Id, EME_Id) {
            ...
            apiService.create("MarksEntry_SS/get_subjects", data).
       then(function (promise) {
        ...
        $scope.onsearch = function (ASMS_Id, ASMCL_Id, ASMAY_Id, EME_Id, ISMS_Id) {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("MarksEntry_SS/onsearch", data).
               then(function (promise) {
                   if (promise.studentList != null && promise.studentList.length > 0) {
                       if (promise.eyceS_SubSubjectFlg && promise.subject_subsubjects != null && promise.subject_subsubjects.length > 0) {//&& promise.eyceS_SubExamFlg && promise.subject_subexams != null && promise.subject_subexams.length > 0
                           if (!promise.eyceS_SubExamFlg || (promise.eyceS_SubExamFlg && promise.subject_subexams != null && promise.subject_subexams.length == 0))
                           {
                               ...
                               if (promise.saved_studentList != null && promise.saved_studentList.length > 0 && promise.saved_ss_list != null && promise.saved_ss_list.length > 0) {
                                   $scope.map_marks(promise.saved_studentList, promise.saved_ss_list);
                               }
                           }
                           else {
        ...
        $scope.map_marks = function (saved_studentList, saved_ss_list) {
                            ...
                            angular.forEach(saved_ss_list, function (ss_s) {
                                if (ss_s.emsS_Id == ss.emsS_Id) {
                                    if (ss_s.estM_Id == stu_s.estM_Id) {
                                        ...
                                        $scope.valid_marks(ss, ss.emsS_Id, stu_m.ESTMSS_Marks, stu_m.amsT_Id, stu_m);
                                    }
                                }
        ...
        $scope.optionToggled = function () {
            ...
            $scope.all_s = $scope.temp_student_list_SS.every(function (itm)
        ...
        $scope.valid_marks = function (obj_ss, s_subj_id, values, stu_id, obj_student) {//, totalMarks, obtainmarks, row
        ...
        $scope.save = function (ASMS_Id, ASMCL_Id, ASMAY_Id, EME_Id, ISMS_Id) {
            ...
            if ($scope.myForm.$valid) {
                ...
                if (main_save_list.length > 0) {
                    ...
                    apiService.create("MarksEntry_SS/SaveMarks", data).
               then(function (promise) {
