(function () {
    'use strict';
    angular.module('app').controller('PromotionReportStdIX_20_21Controller', PromotionReportStdIX_20_21Controller)
    PromotionReportStdIX_20_21Controller.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$filter', '$window', 'Excel', '$timeout']
    function PromotionReportStdIX_20_21Controller($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $filter, $window, Excel, $timeout) {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("JSHSExamReports/BBHS_IX_20202021", data).then(function (promise) {
                    ...
                    if (promise !== null && promise.getstudentmarksdetails !== null && promise.getstudentmarksdetails.length>0) {
                        ...
                        angular.forEach(promise.getstudentdetails, function (ddd) {
                            angular.forEach($scope.selectedamstids, function (dd) {
                                if (dd.AMST_Id == ddd.AMST_Id) {
                                    $scope.studentdetails.push(ddd);
                                }
                            });
                        });
