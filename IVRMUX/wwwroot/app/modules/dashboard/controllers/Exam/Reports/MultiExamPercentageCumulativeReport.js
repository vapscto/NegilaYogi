(function () {
    'use strict';
    angular.module('app').controller('MultiExamPercentageCumulativeReportController', MultiExamPercentageCumulativeReportController)
    MultiExamPercentageCumulativeReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$filter', 'Excel','$timeout']
    function MultiExamPercentageCumulativeReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $filter, Excel, $timeout) {
                    ...
                    if ($scope.getmultipleexamcumulativereport_temp !== null && $scope.getmultipleexamcumulativereport_temp.length > 0) {
                        ...
                        angular.forEach($scope.getstudentmarksdetails, function (dd) {
                            if ($scope.studentdetails.length === 0) {
                                $scope.studentdetails.push({ AMST_Id: dd.AMST_Id, studentname: dd.studentname, admno: dd.admno, regno: dd.regno, rollno: dd.rollno });
                                ...
                                angular.forEach($scope.getstudentmarksdetails, function (marks) {
                                    if (stu.AMST_Id === marks.AMST_Id && exm.EME_Id === marks.examid) {
                                        if (marks.examid !== 50001) {
                                            $scope.temp_markslist.push({
                                                EME_Id: marks.examid, marksobtained: marks.marksobtained, maxmarks: marks.maxmiummarks,
                                                percentage: marks.percentage});
                                        } else {
                                            $scope.temp_markslist.push({
                                                EME_Id: marks.examid, marksobtained: marks.grade, maxmarks: marks.maxmiummarks,
                                                percentage: marks.percentage});
                                        }                                       
                                    }
                                });
        ...
        $scope.exportToExcel = function (tableId) {
            var exportHref = Excel.tableToExcel(tableId, 'EXAM PERCENTAGE WISE CUMULATIVE REPORT');
            var excelname = "EXAM PERCENTAGE WISE CUMULATIVE REPORT.xls";
            $timeout(function () {
