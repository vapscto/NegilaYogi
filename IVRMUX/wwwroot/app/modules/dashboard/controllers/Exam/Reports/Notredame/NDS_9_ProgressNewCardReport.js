(function () {
    'use strict';
    angular.module('app').controller('NDS_9_ProgressNewCardReportController', NDS_9_ProgressNewCardReportController)
    NDS_9_ProgressNewCardReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$filter', '$window']
    function NDS_9_ProgressNewCardReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $filter, $window) {
        ...
        //empG_GroupName,emE_Id,emE_ExamName,emE_ExamCode,emE_ExamOrder,empG_DistplayName,empsgE_ForMaxMarkrs
        // TO Save The Data
        ...
        $scope.saveddata = function (obj) {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("JSHSExamReports/nds_6_8_report", data).then(function (promise) {
                    $scope.getstudentmarksdetails_temp = promise.getstudentmarksdetails; $scope.getgroupdetailspp = [];
            ...
            //$scope.getgroupdetailstemp = promise.getgroupdetails;
            ...
            angular.forEach($scope.getgroupdetails, function (dd) {
                ...
                angular.forEach($scope.getgroupexamdetails, function (d) {
                    ...
                    if (dd.empG_DistplayName === d.empG_DistplayName) {
                        if ($scope.groupwiseexamlist.length === 0) {
                            ...
                            $scope.groupwiseexamlist_temp.push({
                                ...
                                examnamedisplay: d.emE_ExamName
                            });
                        ...
                        else if ($scope.groupwiseexamlist.length > 0) {
                            var intcount = 0;
                            angular.forEach($scope.groupwiseexamlist, function (ddd) {
                                if (ddd.EMPG_DistplayName === d.empG_DistplayName && ddd.EME_Id == d.emE_Id) {
                                    intcount += 1;
                                }
                            });
                            if (intcount === 0) {
