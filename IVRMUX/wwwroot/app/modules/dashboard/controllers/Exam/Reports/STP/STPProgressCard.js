(function () {
    'use strict';
    angular.module('app').controller('STPProgressCardController', STPProgressCardController)
    STPProgressCardController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$filter']
    function STPProgressCardController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $filter) {
        ...
        $scope.getbbkvreport = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("HHSAllReport/getbbkvreport", data).then(function (promise) {
                    ...
                    if ($scope.exm_sub_mrks_list !== null && $scope.exm_sub_mrks_list.length>0 ) {
                        ...
                        $scope.amsT_AdmNo = promise.stu_details[0].amsT_AdmNo;
                            ...
                            angular.forEach($scope.marksprocess_Pro_markscal, function (ww) {
                                if (xx.emE_Id == ww.EME_Id && xx.ismS_Id == ww.ISMS_Id) {
                                    ...
                                    $scope.subexwithtotal_mrks.push({
                                        emE_Id: xx.emE_Id, emsE_Id: ww.EMSE_Id, obtainedMarks: ww.ESTMPSSS_ObtainedMarks, ismS_Id: ww.ISMS_Id, emsE_Id: ww.EMSE_Id, maxMarks: ww.MaxMarks, estmpS_PassFailFlg: ww.ESTMPS_PassFailFlg
                                    });
                                }
                            });
                            if (totalmaxMarks > 0) {
                            ...
                            angular.forEach($scope.marksprocess_Pro_markscal, function (ww) {
                                ...
                                if (xx.emE_Id == ww.EME_Id && xx.ismS_Id == ww.ISMS_Id) {
                                    ...
                                    $scope.subexwithtotal_mrks1.push({
                                        ...
                                        emsE_Id: ww.EMSE_Id, maxMarks: ww.MaxMarks
