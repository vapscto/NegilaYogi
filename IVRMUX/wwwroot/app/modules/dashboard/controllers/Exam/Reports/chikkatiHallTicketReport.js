(function () {
    'use strict';
    angular.module('app').controller('chikkatiHallTicketReportController', chikkatiHallTicketReportController)

    chikkatiHallTicketReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$compile']
    function chikkatiHallTicketReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $compile) {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("VikasaHallTicketReport/report", data).then(function (promise) {
                    ...
                    if ($scope.main_list !== null && $scope.main_list.length > 0) {
                        //$scope.amsT_RegistrationNo = promise.datareport[0].amsT_RegistrationNo
