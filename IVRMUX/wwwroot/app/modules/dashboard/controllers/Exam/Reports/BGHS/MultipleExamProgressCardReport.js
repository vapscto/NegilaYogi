(function () {
    'use strict';
    angular.module('app').controller('MultipleExamProgressCardReportNewController', MultipleExamProgressCardReportNewController)
    MultipleExamProgressCardReportNewController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$filter', '$window', 'Excel', '$timeout']
    function MultipleExamProgressCardReportNewController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $filter, $window, Excel, $timeout) {
                ...
                apiService.create("JSHSExamReports/BGHS_GetMultiple_Exam_Progress_Report", data).then(function (promise) {
                    ...
                    if (promise !== null) {
                        ...
                        if (promise.getstudentmarksdetails !== null && promise.getstudentmarksdetails.length > 0) {
                           ...
                           // $scope.getexamdetailstemp = promise.getexamdetails;
                            var totalcount = 0;
                            ...
                            if ($scope.getstudentmarksdetails != null && $scope.getstudentmarksdetails.length > 0) {
                                angular.forEach($scope.getstudentmarksdetails, function (dd) {
                                    if (dd.ISMS_Id === 50001) {
                                        $scope.getstudentmarksTotals.push({
                                            AMST_Id: dd.AMST_Id,
                                            EME_Id: dd.EME_Id,
                                            EMGD_Remarks: dd.EMGD_Remarks
                                        })
                                    }
                                });
                            }
