(function () {
    'use strict';
    angular.module('app').controller('StjamesConsolidatedReportController', StjamesConsolidatedReportController)
    StjamesConsolidatedReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$filter', '$window', 'Excel', '$timeout']
    function StjamesConsolidatedReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $filter, $window, Excel, $timeout) {
                ...
                apiService.create("JSHSExamReports/stjamesexamconsolidatedreport", data).then(function (promise) {
                    if (promise !== null) {
                        if (promise.getstudentmarksdetails !== null && promise.getstudentmarksdetails.length > 0) {
                            ...
                            // STUDENT WISE GROUP LIST
                            //$scope.grouplist = [];
                            //angular.forEach($scope.getstudentdetails, function (dd) {
                            //    $scope.grouplist = [];
                            //    angular.forEach($scope.getexamdetails, function (d) {
                            //        $scope.grouplist.push(d);
                            //    });

                            //    dd.groupdetails = $scope.grouplist;
                            //});
                          
                            //// STUDENT GROUP WISE MARKS

                            //$scope.grouplistmarks = [];
                            //angular.forEach($scope.getstudentdetails, function (dd) {                               
                            //    angular.forEach(dd.groupdetails, function (d) {
                            //        $scope.grouplistmarks = [];
                            //        angular.forEach($scope.getstudentmarksdetails, function (ddd) {
                            //            if (dd.AMST_Id === ddd.AMST_Id && d.empG_GroupName === ddd.EMPSG_GroupName) {
                            //                $scope.grouplistmarks.push(ddd);
                            //            }
                            //        });
                            //        d.groupmarks = $scope.grouplistmarks;
                            //    });                               
                            //});
                            //console.log("dfsdf");
