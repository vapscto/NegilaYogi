(function () {
    'use strict';
    angular.module('app').controller('PromotionSmsAndEmailDetailsReport', PromotionSmsAndEmailDetailsReport)
    PromotionSmsAndEmailDetailsReport.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function PromotionSmsAndEmailDetailsReport($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.loaddata = function () {
            var pageid = 2;
            apiService.getURI("PromotionSmsAndEmailDetailsReport/loaddata", pageid).then(function (promise) {
        ...
        $scope.getclass = function () {
            ...
            apiService.create("PromotionSmsAndEmailDetailsReport/getclass", data).then(function (promise) {
        ...
        $scope.getsection = function () {
            ...
            apiService.create("PromotionSmsAndEmailDetailsReport/getsection", data).then(function (promise) {
        ...
        $scope.searchDetails = function () {
            if ($scope.myForm.$valid) {
                ...
                apiService.create("PromotionSmsAndEmailDetailsReport/searchDetails", data).then(function (promise) {
                    if (promise.studentdetails.length > 0) {
                        $scope.studentdetails = promise.studentdetails;
                        angular.forEach($scope.studentdetails, function (aa) {
                            aa.statusremarks = aa.promotiondetails;
                        });
                    }
                    else {
        ...
        $scope.SendSmsEmail = function () {
            ...
            if ($scope.clslst2.length > 0) {
                if ($scope.smsflag === true || $scope.emailflag === true) {
                    ...
                    apiService.create("PromotionSmsAndEmailDetailsReport/SendSmsEmail", data).then(function (promise) {
