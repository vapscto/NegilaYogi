...
(function () {
    ...
    angular
        .module('app')
        .controller('BCOEDIIIReportController', BCOEDIIIReportController)

    BCOEDIIIReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http']
    function BCOEDIIIReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http) {
        ...
        $scope.BindData = function () {
            apiService.getDATA("BaldwinAllReport/Getdetails").then(function (promise) {
        ...
        $scope.saveddata = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("BaldwinAllReport/savedetails", data).then(function (promise) {
                        ...
                        for (var x = 0; x < promise.savelist.length; x++) {
                            ...
                            if (temp_list.length === 0) {
                                temp_list.push({
                                    ...
                                    amsT_DOB: promise.savelist[x].amsT_DOB,
                                    amsT_AdmNo: promise.savelist[x].amsT_AdmNo,
                                    amaY_RollNo: promise.savelist[x].amaY_RollNo,
                                    asmcL_ClassName: promise.savelist[x].asmcL_ClassName,
                                    asmC_SectionName: promise.savelist[x].asmC_SectionName,
                                    estmP_TotalMaxMarks: promise.savelist[x].estmP_TotalMaxMarks,
                                    estmP_TotalObtMarks: promise.savelist[x].estmP_TotalObtMarks,
                                    estmP_Percentage: promise.savelist[x].estmP_Percentage,
                                    estmP_TotalGrade: promise.savelist[x].estmP_TotalGrade, sub_list: stu_subj_list,
                                ...
                                if (already_cnt === 0) {
                                    temp_list.push({
                                        ...
                                        amsT_DOB: promise.savelist[x].amsT_DOB, amsT_AdmNo: promise.savelist[x].amsT_AdmNo,
                                        amaY_RollNo: promise.savelist[x].amaY_RollNo, asmcL_ClassName: promise.savelist[x].asmcL_ClassName,
                                        asmC_SectionName: promise.savelist[x].asmC_SectionName, estmP_TotalMaxMarks: promise.savelist[x].estmP_TotalMaxMarks,
        ...
        $scope.printToCart = function () {
            ...
            popupWinindow.document.write('<html><head>' +
                ...
                '<link type="text/css" media="print" rel="stylesheet" href="css/print/baldwin/ProgressReport/BCOEDIIIReportPdf.css" />' +
