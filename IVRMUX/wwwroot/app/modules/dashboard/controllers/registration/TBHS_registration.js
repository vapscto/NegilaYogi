(function () {
    ...
    angular
        .module('app')
        .controller('TBHSRegistrationController', TBHSRegistrationController)

    TBHSRegistrationController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$q', '$cookieStore', '$stateParams', '$filter', 'FormSubmitter', '$window', 'superCache', '$compile', '$sce']
    function TBHSRegistrationController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $q, $cookieStore, $stateParams, $filter, FormSubmitter, $window, superCache, $compile, $sce) {
                ...
                if (promise.precutdate == "True") {
                    ...
                    if ($scope.arrlist != null && $scope.arrlist.length > 0) {
                        for (var i = 0; i < $scope.arrlist.length; i++) {
                            if ($scope.arrlist[i].ivrmmC_Default == 1) {
                                $scope.reg.PASR_Nationality = $scope.arrlist[i].ivrmmC_Id;
                                $scope.reg.PASR_ConCountry = $scope.arrlist[i].ivrmmC_Id;
                                $scope.reg.PASR_PerCountry = $scope.arrlist[i].ivrmmC_Id;
                                $scope.reg.PASR_FatherNationality = $scope.arrlist[i].ivrmmC_Id;
                                $scope.reg.PASR_MotherNationality = $scope.arrlist[i].ivrmmC_Id;
                    ...
                    if ($scope.arrlist7 != null && $scope.arrlist7.length > 0) {
                        for (var i = 0; i < $scope.arrlist7.length; i++) {
                            if ($scope.arrlist7[i].imC_CasteName == 'GENERAL') {
                                $scope.reg.Caste_Id = $scope.arrlist7[i];
                        ...
                        function (isConfirm) {
                            if (isConfirm) {
                                ...
                                var data = {
                                    ...
                                    "Caste_Id": $scope.reg.Caste_Id.imC_Id,
