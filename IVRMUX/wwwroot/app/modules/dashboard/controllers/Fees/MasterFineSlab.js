...
(function () {
    ...
    angular
.module('app')
.controller('FeeFineSlabController', fee12)

    fee12.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache','$stateParams']
    function fee12($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $stateParams) {
        ...
        $scope.loaddata = function () {
            ...
            apiService.getURI("FeeFineSlab/getalldetails", pageid).
                ...
                else
                {
                    if ($scope.FMFS_FromDay < $scope.FMFS_ToDay || $scope.FMFS_FineType == "GreaterThan" || $scope.FMFS_FineType == "LessThan") {
                        if ($scope.FMFS_FineType === "select") {
                            ...
                            apiService.create("FeeFineSlab/savedetail", data).
                            then(function (promise) {
                                $scope.students = promise.groupFineSlab;
        ...
        $scope.searchsource = function () {
            ...
            apiService.create("FeeFineSlab/1", data).
        then(function (promise) {
            $scope.students = promise.groupFineSlab;
        ...
        $scope.deletedata = function (employee, SweetAlert) {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    apiService.DeleteURI("FeeFineSlab/deletepages", pageid).
                    then(function (promise) {
                        $scope.students = promise.groupFineSlab;
        ...
        $scope.getorgvalue = function (employee) {
            $scope.editEmployee = employee.fmfS_Id;
            var pageid = $scope.editEmployee;
            apiService.getURI("FeeFineSlab/getdetails", pageid).
            then(function (promise) {
                ...
                else if (promise.groupFineSlab[0].fmfS_FineType == "Greater Than") {
        ...
        function (isConfirm) {
            if (isConfirm) {
                ...
                apiService.create("FeeFineSlab/deactivate", groupFineSlab).
                then(function (promise) {
                    $scope.MI_Id = "2";
                    $scope.FMFS_FineType = "";
                    $scope.FMFS_FromDay = "";
