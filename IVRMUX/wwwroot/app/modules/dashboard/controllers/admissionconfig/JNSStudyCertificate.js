(function () {
    ...
    angular
        .module('app')
        .controller('JNSStudyCertificateController', JNSStudyCertificateController)

    JNSStudyCertificateController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache']
    function JNSStudyCertificateController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache) {

        $scope.mi_idd = {};
        $scope.studlist = [];
        $scope.allorindiform = false;
        $scope.print_flag = true;
        $scope.print_flag1 = false;
        var data1;
        var frommonth;
        var tomonth;
        var datesufix;
                ...
                apiService.create("HHSBonafiedCertificate/Studdetails", data)
                    .then(function (promise) {
                        ...
                        if (promise.studentlist.length > 0 && promise.studentlist != null) {
                            ...
                            $scope.print_flag1 = true;
                            $scope.study_cer = true;
                            $scope.bonafide = false;

                            $scope.students = promise.studentlist;
                            
                            if (promise.studentlist[0].gender === "Male" || promise.studentlist[0].gender === "male" || promise.studentlist[0].gender === "MALE") {
                                $scope.gender = "Kumar";
                                $scope.gender1 = "He";
                                $scope.gender2 = "His";
                                $scope.fff = 'S/O';
                                ...
                                if (promise.studentlist[0].gender === "Female" || promise.studentlist[0].gender === "female" || promise.studentlist[0].gender === "FEMALE") {
        ...
        $scope.HHSStudyCert = function () {
            var innerContents = document.getElementById("BBSBonafideCert").innerHTML;
            var popupWinindow = window.open('');
            popupWinindow.document.open();
            popupWinindow.document.write('<html><head>' +
                '<link type="text/css" media="print" rel="stylesheet" href="css/print/PrintPdf_Bootstrap.css" />' +
                '<link type="text/css" media="print" rel="stylesheet" href="css/print/balabarathi/BBS_Study_Cert/BBSBonafideCertPDf.css" />' +
        ...
        $scope.getstudentname = function () {
            ...
            apiService.create("HHSBonafiedCertificate/getstudentname", data).
                then(function (promise) {

                    if (promise != null) {
                        if (promise.studentreportcount.length > 0) {
                            ...
                            swal({
                                title: "For This Student Already Study Certificate Is Issued",
                                text: "Do you want to issue again ?",
                                type: "warning",
                                showCancelButton: true,
                                confirmButtonColor: "#DD6B55", confirmButtonText: "Yes, it!",
