(function () {
    'use strict';
    angular.module('app').controller('SchoolTpinGenreationcontroller', SchoolTpinGenreationcontroller);
    SchoolTpinGenreationcontroller.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$filter', 'superCache', '$stateParams'];

    function SchoolTpinGenreationcontroller($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $filter, superCache, $stateParams) {
        ...
        $scope.loaddata = function () {
            var pageid = 1;
            apiService.getURI("SchoolTpinGenreation/loaddata", pageid).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.search = function (att) {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("SchoolTpinGenreation/search", data).then(function (promise) {
                    if (promise !== null) {
        ...
        $scope.generatetpin = function (att) {
            ...
            apiService.create("SchoolTpinGenreation/generatetpin", data).then(function (promise) {
