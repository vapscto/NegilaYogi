(function () {
    'use strict';
    angular.module('app').controller('stthomasstccustomreportController', stthomasstccustomreportController)
    stthomasstccustomreportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http']
    function stthomasstccustomreportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http) {
            ...
            apiService.create("studenttccustomreport/getTcdetails", data).then(function (promise) {
                if (promise.studentTCList.length > 0) {
                    ...
                    if ($scope.astC_FeePaid =="No") {
                        $scope.astC_FeePaid = "Not UptoDate";
                    }
                    else {
                        $scope.astC_FeePaid = "Uptodate";
                    }
