(function () {
    'use strict';
    angular.module('app').controller('tccustomController', tccustomController)
    tccustomController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http']
    function tccustomController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http) {
        var data1;
        var frommonth;
        var tomonth;
            ...
            if ($scope.myform.$valid) {
                ...
                apiService.create("BBKVCustomReport/get_JSHSTcdetails", data).then(function (promise) {
                    if (promise.studentTCList.length > 0) {
                        ...
                        $scope.Mothertounge = promise.studentTCList[0].admnoorname;
                        $scope.tctemporper = promise.studentTCList[0].tctemporper;
                        ...
                        if ($scope.IMCC_CategoryCode === "SC" || $scope.IMCC_CategoryCode === "ST") {
                            $scope.schedulecaste = "Yes";
                        } else {
                            $scope.schedulecaste = "No";
                        }
        ...
        $scope.printToCart = function () {
            var innerContents = document.getElementById('JSHSTC').innerHTML;
