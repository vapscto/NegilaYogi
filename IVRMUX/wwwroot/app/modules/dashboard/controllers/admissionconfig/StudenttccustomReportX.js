(function () {
    'use strict';
    angular.module('app').controller('StudentTcReportcustomXController', StudentTcReportcustomXController)

    StudentTcReportcustomXController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http']
    function StudentTcReportcustomXController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http) {
        ...
        $scope.StuAttRptDropdownList = function () {
            ...
            apiService.get("StudenttcReportcustom/getdetails/").then(function (promise) {
                ...
                if (parseInt(promise.masterCompany[0].miid) === 4) {
                    $scope.mididi = true;
                    $scope.mididi1 = true;
                    $scope.mididi1girls = true;
                }
                else if (parseInt(promise.masterCompany[0].miid) === 6) {
                    $scope.mididi = false;
                    $scope.mididi1 = true;
                    $scope.mididi1girls = false;
                    $scope.mididiCoedu = false;
                }
                else if (parseInt(promise.masterCompany[0].miid) === 5) {
                    $scope.mididi = false;
                    $scope.mididi1 = false;
                    $scope.mididi1girls = false;
                    $scope.mididiCoedu = true;
                }
                else {
                    $scope.mididi = false;
                    $scope.mididi1 = false;
        ...
        $scope.onclickregnoname = function () {
            ...
            apiService.create("StudenttcReportcustom/getnameregno", data).
        ...
        $scope.onacademicyearchange = function (yearlist) {
            ...
            apiService.getURI("StudenttcReportcustom/getACS", yearid).
            ...
            if ($scope.myform.$valid) {
                ...
                apiService.create("StudenttcReportcustom/getTcdetails", data).then(function (promise) {
                    ...
                    if (promise.studentTCList !== null && promise.studentTCList.length > 0) {
                        ...
                        $scope.castecate = promise.studentTCList[0].castecat;
                        $scope.studcastecategory = "";
                        if ($scope.castecate == 'SC' || $scope.castecate == 'ST' || $scope.castecate == 'SC AD' || $scope.castecate == 'S.T' || $scope.castecate == 'S.C') {
                            $scope.studcastecategory = "YES"
                        }

                        else {
                            $scope.studcastecategory = "NO"
                        }
                        //if ($scope.castecate == 'SC' || $scope.castecate == 'ST' || $scope.castecate == 'SC AD') {
                        //    $scope.studcastecategory = "Yes"
                        //}
                        ...
                        $scope.AMST_GovtAdmno = promise.studentTCList[0].AMST_GovtAdmno;
                        if (parseInt(promise.masterCompany[0].miid) === 4) {
                        ...
                        $scope.AMST_PENNo = promise.studentTCList[0].AMST_PENNo;
                        ...
                        $scope.subjectstudied = promise.studentTCList[0].subjectstudied;
                        ...
                        $scope.fee_concesion = promise.studentTCList[0].fee_concesion;
                        $scope.statename = promise.studentTCList[0].statename;
                        $scope.district = promise.studentTCList[0].district;
                        $scope.promotionflag = promise.studentTCList[0].promotionflag;
                        ...
                        $scope.AMST_SubCasteIMC_Id = promise.studentTCList[0].amsT_SubCasteIMC_Id;
                        
                        if ($scope.stuLcs != null) {
                            $scope.currentclass = "";

                            switch ($scope.stuLcs) {
                                case "I":
                                    $scope.currentclass = "First Standard"
                                    break;
                                case "II":
                                    $scope.currentclass = "Second Standard"
                                    break;
                                case "III":
                                    $scope.currentclass = "Third Standard"
                                    break;
                                case "IV":
                                    $scope.currentclass = "Fourth Standard"
                                    break;
                                case "V":
                                    $scope.currentclass = "Fifth Standard"
                                    break;
                                case "VI":
                                    $scope.currentclass = "Sixth Standard"
                                    break;
                                case "VII":
                                    $scope.currentclass = "Seventh Standard"
                                    break;
                                case "VIII":
                                    $scope.currentclass = "Eighth Standard"
                                    break;
                                case "IX":
                                    $scope.currentclass = "Ningth Standard"
                                    break;
                                case "X":
                                    $scope.currentclass = "Tenth Standard"
                                    break;
                                case "XI":
                                    $scope.currentclass = "Eleventh Standard"
                                    break;
                                case "XII":
                                    $scope.currentclass = "Tweleth Standard"
                                    break;
                                default:
                                    $scope.currentclass = ""
                                    break;
                        ...
                        $scope.gender = promise.studentTCList[0].amsT_Sex;
                        ...
                        if (promise.previousschool!=null && promise.previousschool.length > 0) {
                        ...
                        if (parseInt(promise.masterCompany[0].miid) === 4) {
                            angular.forEach($scope.classlist1, function (y) {
                                if (parseInt(y.asmcL_Id) === parseInt($scope.ASMCL_Id1)) {
        ...
        $scope.printToCart = function () {
            ...
            if (parseInt($scope.mi_id) === 4) {
            ...
            else if (parseInt($scope.mi_id) === 6) {
                //boys school
                ...
                //co-education
                data = 'printSectionIdcoedu';
                innerContents = document.getElementById(data).innerHTML;
                popupWinindow = window.open('_blank', 'padding-top=50%;');
                popupWinindow.document.open();
                popupWinindow.document.write('<html><head>' +
                    '<link type="text/css" media="print" rel="stylesheet" href="css/print/PrintPdf_Bootstrap.css" />' +
                    '<link type="text/css" media="print" rel="stylesheet" href="css/print/baldwin/BCOEStudentTcPdf.css" />' +
                    '<link type="text/css" media="print" rel="stylesheet" href="css/print/printpdf.css" />' +
                    '</head><body onload="window.print()" onfocus="window.setTimeout(function() { window.close(); }, 100);">' + innerContents + '</html>');
                popupWinindow.document.close();
            }
