(function () {
    'use strict';
    angular.module('app').controller('VaccineStudentDetailsController', VaccineStudentDetailsController)

    VaccineStudentDetailsController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$stateParams', '$filter', 'Excel', '$timeout']
    function VaccineStudentDetailsController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $stateParams, $filter, Excel, $timeout) {
        ...
        $scope.OnLoadVaccineStudentDetails = function () {
            var pageid = 2;
            apiService.getURI("VaccineAgeCriteria/OnLoadVaccineStudentDetails", pageid).then(function (promise) {
                if (promise !== null) {
