(function () {
    ...
    angular
        .module('app')
        .controller('TCcustomreportnagarjunaController', TCcustomreportnagarjunaController)

    TCcustomreportnagarjunaController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http']
    function TCcustomreportnagarjunaController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http) {
        var data1;
        var frommonth;
        var tomonth;
        $scope.tcpdf = false;
        ...
        if (admfigsettings != null && admfigsettings.length > 0) {
            $scope.Logo_Path = admfigsettings[0].asC_Logo_Path;
        }
        else {
            $scope.Logo_Path = "";
        }
                ...
                apiService.create("BBKVCustomReport/getTcdetails", data)
                    .then(function (promise) {
                        if (promise.studentTCList != null) {
                            ...
                            $scope.collagename = promise.studentTCList[0].mI_Name;
        ...
        $scope.printToCart = function () {
            ...
            var innerContents = document.getElementById('BBKVTC222222').innerHTML;
            var popupWinindow = window.open('');
            popupWinindow.document.open();
            popupWinindow.document.write('<html><head>' +
                '<link type="text/css" media="print" rel="stylesheet" href="css/print/PrintPdf_Bootstrap.css" />' +
