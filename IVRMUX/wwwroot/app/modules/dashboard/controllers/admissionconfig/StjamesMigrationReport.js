(function () {
    'use strict';
    angular.module('app').controller('StjamesMigrationCertificate', StjamesMigrationCertificate)

    StjamesMigrationCertificate.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache']
    function StjamesMigrationCertificate($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache) {
            ...
            angular.forEach($scope.yearlist, function (dd) {
                if (dd.asmaY_Id === Number($scope.asmaY_Id)) {
                    ...
                    angular.forEach(yearname_temp, function (d) {
                        ...
                        $scope.examarray.push({ certificatename: 'INDIAN CERTIFICATE OF SECONDARY EDUCATION, ' + d });
                    });
                }
            });
        };
