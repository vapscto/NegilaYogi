(function () {
    ...
    angular
        .module('app')
        .controller('ContinuousAbsentAttendanceReportController', ContinuousAbsentAttendanceReportController)
    ContinuousAbsentAttendanceReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$stateParams', 'superCache', 'Excel', '$timeout', '$filter']
    function ContinuousAbsentAttendanceReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $stateParams, superCache, Excel, $timeout, $filter) {
        ...
        $scope.Reportnew = function (ts) {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("DatewiseattendanceReport/Reportnew", data).then(function (promise) {
                    if (promise !== null) {
                        $scope.loginPData22 = promise.reportdata;
                        if ($scope.loginPData22 !== null) {
                            if ($scope.loginPData22.length === 0) {
                                swal("No Records Found");
                            } else {
                                $scope.presentCountgrid = $scope.loginPData22.length;
                                $scope.class = true;

                                angular.forEach($scope.newuser1, function (dd) {
                                    if (dd.asmaY_Id === parseInt(ts.asmaY_Id)) {
        ...
        $scope.filterValue = function (obj) {
            return ($filter('date')(obj.fromdate, 'dd/MM/yyyy').indexOf($scope.searchValue) >= 0)
                || ($filter('date')(obj.todate, 'dd/MM/yyyy').indexOf($scope.searchValue) >= 0)
                ...
                || (angular.lowercase(obj.sectionname).indexOf(angular.lowercase($scope.searchValue))) >= 0
                || (JSON.stringify(obj.absentcount)).indexOf($scope.searchValue) >= 0;
        };
    }
})
    ();