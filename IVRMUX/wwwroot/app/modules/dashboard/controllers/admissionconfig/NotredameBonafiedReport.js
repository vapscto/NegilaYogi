(function () {
    'use strict';
    angular.module('app').controller('NotredameBonafiedReport', NotredameBonafiedReport)

    NotredameBonafiedReport.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache']
    function NotredameBonafiedReport($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache) {
        ...
        $scope.ndsbonafied = false;
        $scope.cbsbonafied = false;
        ...
        $scope.onpageload = function () {
            //$scope.study = true;
            var pageid = 2;
            apiService.getURI("HHSBonafiedCertificate/getdata", pageid).then(function (promise) {
                ...
                apiService.create("HHSBonafiedCertificate/Studdetails", data).then(function (promise) {
                    if (promise.studentlist !== null && promise.studentlist.length > 0) {
                        ...
                        if (promise.studentlist[0].AMST_Sex === "FEMALE" || promise.studentlist[0].AMST_Sex === "Female") {
                            ...
                            $scope.gender4 = "He";
                            $scope.sonof = "s/o";
                            $scope.gender5 = "Master"
                        }

                        if ($scope.mi_idd == 7) {
                            $scope.ndsbonafied = true;
                            $scope.cbsbonafied = false;
                        }
                        else {
                            $scope.ndsbonafied = false;
                            $scope.cbsbonafied = true;
                        }
                        ...
                        $scope.rollNo = promise.studentlist[0].rollNo;
        ...
        $scope.HHSStudyCert = function () {
            ...
            if ($scope.mi_idd == 7) {
                ...
                var innerContents = document.getElementById("CBSbonafied").innerHTML;
                var popupWinindow = window.open('');
                popupWinindow.document.open();
                popupWinindow.document.write('<html><head>' +
                    '<link type="text/css" media="print" rel="stylesheet" href="css/print/PrintPdf_Bootstrap.css" />' +
