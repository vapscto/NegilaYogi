...
(function () {
    ...
    angular
.module('app')
        .controller('MobileRolePreviledgeController', MobileRolePreviledgeController)

    MobileRolePreviledgeController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$window', 'superCache']
    function MobileRolePreviledgeController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $window, superCache) {
        //modal start
        ...
        $scope.Previous = function () {
            $window.location.href = 'http://' + HostName + '/#/app/mobilemasterpage/';
        };

        //$scope.Next = function () {
        //    $window.location.href = 'http://' + HostName + '/#/app/StaffUserCreation/';
        //};
        ...
        $scope.modulefill = function () {
            ...
            var pageid = 2;
            apiService.getURI("MasterRolePreviledge/mobilegetalldetails", pageid).
        then(function (promise) {
            $scope.roles = promise.fillroletype;
            $scope.temproles = promise.fillroletype;
            $scope.grid1 = promise.allsaveddata;
            $scope.institutionDropDown = promise.fillinstitution;
        ...
        $scope.getpagesname = function (modpages) {
            ...
            apiService.create("MasterRolePreviledge/mobilegetmodulepages", data).
        then(function (promise) {
            $scope.previosgrid = true;
            if (promise.thirdgriddata.length > 0) {
                $scope.grid1 = promise.thirdgriddata;
                $scope.previousgrid = promise.previousgrid;
                $scope.thirdgrid = promise.firstgrigdata;
        ...
        $scope.Toggle_Addsave = function () {
            ...
            angular.forEach($scope.secondgrid, function (itm) {
                itm.IVRMMAP_AddFlg = toggleStatus2;
            });
        }
        ...
        $scope.Toggle_Deletesave = function () {
            ...
            angular.forEach($scope.secondgrid, function (itm) {
                itm.IVRMMAP_DeleteFlg = toggleStatus2;
            });
        }
        ...
        $scope.Toggle_Updatesave = function () {
            ...
            angular.forEach($scope.secondgrid, function (itm) {
                itm.IVRMMAP_UpdateFlg = toggleStatus2;
            });
        }
      ...
      function (isConfirm) {
        ...
        $scope.deletrec = function (employee, SweetAlert) {
            ...
            var data = {
                "IVRMRMAP_Id": employee.ivrmrP_Id
            }
            swal({
                title: "Are you sure?",
                text: "Do You Want To Delete Record !!!!!!!!",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        ...
                        var modulepageid = {
                            "IVRMRMAP_Id": modulepageid
                        };
                        apiService.create("MasterRolePreviledge/mobiledeletemodpages", modulepageid).
                            then(function (promise) {
        ...
        $scope.savadata = function (pagesrecord, previouspagesrecord) {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("MasterRolePreviledge/mobilesaveorgdet", data).
                then(function (promise) {
