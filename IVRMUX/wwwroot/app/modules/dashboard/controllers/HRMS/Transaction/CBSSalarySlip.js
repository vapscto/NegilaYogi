(function () {
    ...
    angular
        .module('app')
        .controller('CBSSalarySlipController', CBSSalarySlipController)

    CBSSalarySlipController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$stateParams', '$filter', 'superCache']
    function CBSSalarySlipController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $stateParams, $filter, superCache) {
        //object


        $scope.EmployeeDis = false;
        $scope.EmployeeDisword = false;
        $scope.Emp = {};
        $scope.currentPage = 1;
        ...
        //        "</style></head><body>" +
        //        "<img src='https://vapsclg.blob.core.windows.net/files/3/InstitutionLogo/8fbbda94-2ac9-4eda-904c-5fc72d5cfc87.jpg' alt='Header Image' style='width: 100%; max-width: 200px;' />"; // Change 'path_to_your_image.jpg' to the actual path of your image
        //    var footer = "</body></html>";
        //    var sourceHTML = header + $scope.sourceHTML + footer; // Assuming $scope.sourceHTML contains your HTML content

        //    var blob = new Blob([sourceHTML], { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
        //    var url = URL.createObjectURL(blob);
        //    var link = document.createElement('a');
        //    link.href = url;
        //    link.download = 'SalarySlip.docx';
        ...
        function getImageAsBase64(url, callback) {
            ...
            xhr.onload = function () {
                if (xhr.status === 200) {
                    ...
                    var binaryString = uInt8Array.reduce(function (data, byte) {
        ...
        function getImageAsBase64(url) {
            ...
            if (xhr.status === 200) {
                ...
                var byteArray = new Uint8Array(responseText.length);
                for (var i = 0; i < responseText.length; i++) {
                    byteArray[i] = responseText.charCodeAt(i) & 0xff;
                }
        ...
        $scope.exportHTML = function () {
            ...
            fileDownload.download = 'SalarySlip.doc';
            fileDownload.click();
