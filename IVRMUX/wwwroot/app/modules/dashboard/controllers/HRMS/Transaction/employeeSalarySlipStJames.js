(function () {
    ...
    angular
        .module('app')
        .controller('employeeSalarySlipStJamesController', employeeSalarySlipStJamesController)

    employeeSalarySlipStJamesController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$stateParams', '$filter', 'superCache']
    function employeeSalarySlipStJamesController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $stateParams, $filter, superCache) {
                        ...
                        if (promise.main_list.length > 0 && promise.main_list != null) {
                            ...
                            for (var i = 0; i < promise.main_list.length; i++) {
                                if (promise.main_list[i].employeeSalaryslipDetails != null && promise.main_list[i].employeeSalaryslipDetails.length != 0) {
                                    ...
                                    angular.forEach(promise.main_list[i].employeeSalaryslipDetails, function (sal) {
                                        if (sal.HRMED_Name == "Basic Pay" || sal.HRMED_Name == "DA" || sal.HRMED_Name == "PERSONAL PAY" || sal.HRMED_Name == "CL AMT") {
                                            $scope.sumcondition = $scope.sumcondition + sal.Amount;


                                            if (parseInt(promise.main_list[i].hrmE_Age) >= 58) {

                                               // promise.main_list[i].PensionFund = 0.00;

                                                if (promise.main_list[i].hreS_WorkingDays > 0) {
                                                    promise.main_list[i].PensionFund = Math.round((1250 * promise.main_list[i].hreS_WorkingDays) / promise.main_list[i].abc, 2);
                                                }
                                                else {
                                                    promise.main_list[i].PensionFund = 0.00;
                                                } 
                                            ...
                                            if (promise.main_list[0].employeeSalaryslipDetails[0].HRES_Month == 'June' && promise.main_list[0].employeeSalaryslipDetails[0].HRES_Year == 2023 && promise.main_list[i].hrmE_Id==132) {

                                                if (parseInt(promise.main_list[i].hrmE_Age) >= 58) {
                                                    //    temp.pensionFund = 0.00;
                                                    if (promise.main_list[i].hreS_WorkingDays > 0) {
                                                        //temp.pensionFund = Math.round((1250 * temp.hreS_WorkingDays) / (temp.abc), 2);
                                                        promise.main_list[i].PensionFund = Math.round(((12150) * (8.33 / 100)) / promise.main_list[i].abc * promise.main_list[i].hreS_WorkingDays);
                                                    }
                                                    else {
                                                        temp.pensionFund = 0.00;
                                                    }
                                                ...
                                                if ($scope.sumcondition < 15000 && promise.main_list[i].currentemployeeDetails.hrmE_FPFNotApplicableFlg == true) {
                                                    promise.main_list[i].PensionFund = Math.round((8.33 / 100) * $scope.sumcondition)
                                                }
                                                else if (promise.main_list[i].currentemployeeDetails.hrmE_FPFNotApplicableFlg == true) {
                                                    promise.main_list[i].PensionFund = 1250.00;
                                                }
                                                else {
                                                    promise.main_list[i].PensionFund = 0.00;
                                                }
