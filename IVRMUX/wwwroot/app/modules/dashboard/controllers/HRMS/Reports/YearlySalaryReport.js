    ...
    function YearlySalaryReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $stateParams, $filter, Excel, $timeout, superCache) {
        ...
        $scope.SearchEmployee = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("YearlySalaryReport/getEmployeedetailsBySelection", data).
        ...
        $scope.TotalCCA = function () {
            var total = 0;
            if ($scope.employeeSalaryslipDetails !== null) {
                for (var i = 0; i < $scope.employeeSalaryslipDetails.length; i++) {
                    var product = $scope.employeeSalaryslipDetails[i];
                    total += parseInt(product.CCA,10);
                }
            }
            return Math.round(total);
        };

        $scope.TotalConAllow = function () {
            var total = 0;
            if ($scope.employeeSalaryslipDetails !== null) {
                for (var i = 0; i < $scope.employeeSalaryslipDetails.length; i++) {
                    var product = $scope.employeeSalaryslipDetails[i];
                    total += parseInt(product.ConAllow, 10);
                }
            }
            return Math.round(total);
        };

        $scope.TotalDA = function () {
            var total = 0;
            if ($scope.employeeSalaryslipDetails !== null) {
                for (var i = 0; i < $scope.employeeSalaryslipDetails.length; i++) {
                    var product = $scope.employeeSalaryslipDetails[i];
                    total += parseInt(product.DA, 10);
                }
            }
            return Math.round(total);
        };

        $scope.TotalBasicPay = function () {
            var total = 0;
            if ($scope.employeeSalaryslipDetails !== null) {
                for (var i = 0; i < $scope.employeeSalaryslipDetails.length; i++) {
                    var product = $scope.employeeSalaryslipDetails[i];
        ...
        $scope.TotalProTax = function () {
            var total = 0;
            if ($scope.employeeSalaryslipDetails !== null) {
                for (var i = 0; i < $scope.employeeSalaryslipDetails.length; i++) {
                    var product = $scope.employeeSalaryslipDetails[i];
                    total += parseInt(product.ProTax, 10);
                }
            }
            return Math.round(total);
        };
