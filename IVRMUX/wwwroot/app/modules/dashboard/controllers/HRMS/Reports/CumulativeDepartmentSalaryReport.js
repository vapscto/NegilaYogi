(function () {
    ...
    angular
        .module('app')
        .controller('CumulativeDepartmentSalaryReportController', CumulativeDepartmentSalaryReportController)

    CumulativeDepartmentSalaryReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$stateParams', '$filter', 'Excel', '$timeout', 'superCache']
    function CumulativeDepartmentSalaryReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $stateParams, $filter, Excel, $timeout, superCache) {
        //object
        ...
        $scope.SearchEmployee = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("CumulativeSalaryReport/getEmployeedetailsByDepartment", data).
                    then(function (promise) {
                        ...
                        angular.forEach($scope.departmentdropdown, function (dept) {
                            if (dept.selected) {
                                ...
                                $scope.earningheadlistsum = [];
                                $scope.deductionheadlistsum = [];

                                angular.forEach($scope.earningheadlistsum, function (headresult) {
                                    headresult.hresD_Amount = 0;
                                    headresult.hresD_Amount = 0;
                                });

                                angular.forEach($scope.deductionheadlistsum, function (headresults) {
                                    headresults.hresD_Amount = 0;
                                });

                                temp_emp_list.push({ indexcount: "", flag: "1", hrmE_EmployeeCode: dept.hrmD_DepartmentName });

                                angular.forEach($scope.employeeSalaryslipDetails, function (emp) {
                                    if (emp.hrmD_Id == dept.hrmD_Id) {
                                        emp.indexcount = $scope.indexcounter + 1;
                                        emp.flag = "0";
                                        $scope.indexcounter = $scope.indexcounter + 1;
                                        //earning head totalAmount
                                        //angular.forEach(emp.earningresult, function (result) {
                                        //    result.hresD_Amount = Math.round(result.hresD_Amount);
                                        //    angular.forEach($scope.earningheadlistsum, function (headresult) {
                                        //        if (headresult.hrmeD_Name == result.hrmeD_Name) {
                                        //            headresult.hresD_Amount = Math.round(headresult.hresD_Amount + result.hresD_Amount);
                                        //        }
                                        //    });
                                        //});

                                        ////deduction head totalAmount
                                        //angular.forEach(emp.deductionresult, function (result) {
                                        //    result.hresD_Amount = Math.round(result.hresD_Amount);
                                        //    angular.forEach($scope.deductionheadlistsum, function (headresult) {
                                        //        if (headresult.hrmeD_Name == result.hrmeD_Name) {
                                        //            headresult.hresD_Amount = Math.round(headresult.hresD_Amount + result.hresD_Amount);
                                        //        }
                                        //    });
                                ...
                                angular.forEach($scope.earningheadlist, function (dd) {
                                    sumearning = 0;
                                    angular.forEach(temp_emp_list, function (ma) {
                                        if (dept.hrmD_Id == ma.hrmD_Id) {
                                            angular.forEach(ma.earningresult, function (maer) {
                                                if (dd.hrmeD_Name == maer.hrmeD_Name) {
                                                    sumearning += maer.hresD_Amount;
                                                }
                                            });
                                        }
                                    });
                                    $scope.earningdetails.push({ hrmeD_Name: dd.hrmeD_Name, hresD_Amount: sumearning });
                                });

                                var sumdeduction = 0;
                                $scope.deductiondetails = [];
                                angular.forEach($scope.deductionheadlist, function (dd) {
                                    sumdeduction = 0;
                                    angular.forEach(temp_emp_list, function (ma) {
                                        if (dept.hrmD_Id == ma.hrmD_Id) {
                                            angular.forEach(ma.deductionresult, function (maer) {
                                                if (dd.hrmeD_Name == maer.hrmeD_Name) {
                                                    sumdeduction += maer.hresD_Amount;
                                                }
                                            });
                                        }
                                    });
                                    $scope.deductiondetails.push({ hrmeD_Name: dd.hrmeD_Name, hresD_Amount: sumdeduction });
                                });

                                temp_emp_list.push({
                                    indexcount: "", flag: "1", hrmE_EmployeeCode: dept.hrmD_DepartmentName + " TOTAL", earningresult: $scope.earningdetails,
                                    deductionresult: $scope.deductiondetails
                                });
                            }
                        });
                        console.log(temp_emp_list);
