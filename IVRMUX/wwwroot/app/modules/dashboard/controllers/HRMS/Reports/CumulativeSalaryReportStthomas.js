(function () {
    ...
    angular
        .module('app')
        .controller('CumulativeSalaryReportStthomasController', CumulativeSalaryReportStthomasController)

    CumulativeSalaryReportStthomasController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$stateParams', '$filter', 'superCache', 'Excel', '$timeout']
    function CumulativeSalaryReportStthomasController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $stateParams, $filter, superCache, Excel, $timeout) {
                ...
                apiService.create("CumulativeSalaryReport/getEmployeedetailsBySelection", data).
                    then(function (promise) {
                        ...
                        if (promise.employeeSalaryslipDetails !== null && promise.employeeSalaryslipDetails.length > 0) {
                            ...
                            //$scope.deductiongrosslist = $scope.employeeSalaryslipDetails[0].empGrossSal
                            //angular.forEach($scope.deductiongrosslist, function (headresult) {
                            //    headresult.netamount = 0;
                            //});
                            ...
                            angular.forEach($scope.employeeSalaryslipDetails, function (itm) {
                                ...
                                itm.grossEarning = parseFloat(itm.grossEarning);
                                itm.grossDeduction = parseFloat(itm.grossDeduction);
                                itm.netSalary = Math.round(itm.netSalary);
                                $scope.hresD_Amount = 0;
                                //earning head totalAmount
                                angular.forEach(itm.earningresult, function (result) {

                                    if (result.hrmeD_Name == "Basic" || result.hrmeD_Name == "D.A") {
                                        $scope.hresD_Amount = $scope.hresD_Amount + result.hresD_Amount;
                                    }
                                    if (result.hrmeD_Name == "New Basic") {
                                        if (result.hresD_Amount > 0) {
                                            result.hresD_Amount;
                                        }
                                        else {
                                            result.hresD_Amount = $scope.hresD_Amount;
                                        }
                                    }
                                    ...
                                    result.hresD_Amount = parseFloat(result.hresD_Amount);
                                    angular.forEach($scope.earningheadlist, function (headresult) {
                                        if (headresult.hrmeD_Name == result.hrmeD_Name) {
                                            //headresult.netamount = Math.round(headresult.netamount + result.hresD_Amount);
                                            headresult.netamount = parseFloat(headresult.netamount + result.hresD_Amount);
                                        }
                                    });
                                });
