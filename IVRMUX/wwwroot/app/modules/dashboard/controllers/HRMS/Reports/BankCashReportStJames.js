(function () {
    ...
    angular
        .module('app')
        .controller('BankCashReportStjController', BankCashReportStjController)

    BankCashReportStjController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$stateParams', '$filter', 'superCache']
    function BankCashReportStjController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $stateParams, $filter, superCache) {
                    ...
                    then(function (promise) {
                        ...
                        if (promise.employeeDetails !== null && promise.employeeDetails.length > 0) {
                            ...
                            angular.forEach($scope.employeeDetails, function (ty) {
                                $scope.totalgrp += parseInt(ty.netSalary);
                            });

                            //$scope.totalgrp = totalgrp;

                            var x = $scope.totalgrp;
                            x = x.toString();
