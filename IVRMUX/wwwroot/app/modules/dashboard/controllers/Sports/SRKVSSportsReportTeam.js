    ...
    function SRKVSSportsReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        ...
        $scope.RepeatDta = function () {
            ...
            apiService.create("SRKVSReport/get_class", data).
                then(function (promise) {
                    $scope.categoryList = promise.categoryListnew;
                    $scope.CompetetionLevel = promise.competetionLevel;
                    $scope.MasterEvent = promise.masterEvent;
                    $scope.spcgrplist_new = promise.getMasterEvent;
                    ...
                    then(function (promise) {
                        ...
                        if (promise.getReport != null && promise.getReport.length > 0) {
                            if ($scope.Type == "Consolidated" || $scope.Type == "Indoor" || $scope.Type == 'Outdoor' || $scope.Type == "Team" || $scope.Type == "Externals") {
                                ...
                                $scope.SPCCMEV_EventVenue = $scope.GetReport[0].SPCCMEV_EventVenue;
                                $scope.SPCCE_StartDate = $scope.GetReport[0].SPCCE_StartDate;
                                $scope.SPCCMCL_CompitionLevel = $scope.GetReport[0].SPCCMCL_CompitionLevel;
                                ...
                                angular.forEach($scope.GetReport, function (dev) {
                                    if ($scope.employeeid.length === 0) {
                                        $scope.employeeid.push({
                                            ...
                                            , ASMCL_ClassName: dev.ASMCL_ClassName, ASMC_SectionName: dev.ASMC_SectionName, SPCCMSCCG_Id: dev.SPCCMSCCG_Id, SPCCMSCCG_SportsCCGroupName: dev.SPCCMSCCG_SportsCCGroupName, SPCCEST_Remarks: dev.SPCCEST_Remarks, SPCCMEV_EventVenue: dev.SPCCMEV_EventVenue, SPCCE_StartDate: dev.SPCCE_StartDate, SPCCMCL_CompitionLevel: dev.SPCCMCL_CompitionLevel
                                        });
                                ...
                                angular.forEach($scope.employeeid, function (dev) {
                                    ...
                                    if ($scope.employeeid1.length === 0) {
                                        $scope.employeeid1.push({
                                            ...
                                            SPCCMEV_EventVenue: dev.SPCCMEV_EventVenue,
                                            SPCCE_StartDate: dev.SPCCE_StartDate,
                                            SPCCMCL_CompitionLevel: dev.SPCCMCL_CompitionLevel
                                        });
