(function () {
    ...
    angular
        .module('app')
        .controller('SRKVSSportsReportRecordController', SRKVSSportsReportRecordController);

    SRKVSSportsReportRecordController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout'];

    function SRKVSSportsReportRecordController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        $scope.searchValue = "";
        $scope.DeleteRecord = {};
        $scope.EditRecord = {};
        $scope.obj = {};
        ...
        $scope.RepeatDta = function () {
            ...
            apiService.create("SRKVSReport/get_classs", data).
                then(function (promise) {
                    $scope.categoryList = promise.categoryListRecord;
                    $scope.CompetetionLevel = promise.competetionLevelRecord;
                    $scope.MasterEvent = promise.masterEvent;
                    $scope.spcgrplist_new = promise.getMasterEvent;
        ...
        $scope.showdetails = function () {
            ...
            if ($scope.categoryList != null && $scope.categoryList.length > 0) {
                if ($scope.Type == "Outdoor" || $scope.Type == "Team" || $scope.Type == "Externals" || $scope.Type == 'Finish') {
                ...
                apiService.create("SRKVSReport/showdetails", data).
                    then(function (promise) {
                        ...
                        if (promise.getReport != null && promise.getReport.length > 0) {
                            if ($scope.Type == "Consolidated" || $scope.Type == "Indoor" || $scope.Type == 'Outdoor' || $scope.Type == "Team" || $scope.Type == "Externals" || $scope.Type == "Selection") {
                                $scope.GetReport = promise.getReport;
                                $scope.SPCCME_EventName = promise.name;
                                ...
                                angular.forEach($scope.employeeid, function (ddd) {
                                    ...
                                    angular.forEach($scope.GetReport, function (dd) {
                                        if (dd.SPCCMCC_Id === ddd.SPCCMCC_Id && dd.SPCCMCL_Id === ddd.SPCCMCL_Id && dd.SPCCMSCCG_Id == ddd.SPCCMSCCG_Id) {
