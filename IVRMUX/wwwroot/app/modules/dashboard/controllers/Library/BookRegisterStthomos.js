...
(function () {
    ...
    function BookRegisterController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $q, $filter, $window, Excel, $timeout) {
        ...
        $scope.next1 = function () {
            ...
            if ($scope.myForm2.$valid) {
                ...
                var data = {
                    ...
                    "LMAL_Id": $scope.lmaL_Id,
                    "AuthFlag": $scope.master_publisher,
        ...
        var search_s = "";
        var list_s = [];
        $scope.onselectsearch = function (GGG) {
            if (GGG == 'NDDC') {
                $scope.search1234 = "";
        ...
        $scope.Loaddata = function () {
            ...
            $scope.sublisttemp = [];
            $scope.sublisttempold = [];
            $scope.search = "";
            var pageid = 2;
            $scope.sublist = [];
            $scope.lmS_Id = "";
            $scope.master_author = 0;
            $scope.master_publisher = 0;
            apiService.getURI("BookRegister/getdetails", pageid).then(function (promise) {               
                if (promise.subjectlist != null && promise.subjectlist.length > 0) {
                    angular.forEach($scope.sublisttempold, function (itm1) {
                        if (itm1.lmS_Level == '3') {
                            $scope.sublist.push({
                ...
                if (promise.subjectlist != null && promise.subjectlist.length > 0) {
                    angular.forEach(promise.subjectlist, function (itm1) {
                        if (itm1.lmS_Level == '1') {
        ...
        $scope.EditData = function (user) {
            ...
            apiService.create("BookRegister/Editdata", data)
                .then(function (promise) {
                    ...
                    $scope.newcaste = {};
                    if ($scope.publisherlst != null && $scope.publisherlst.length > 0) {                        for (var i = 0; i < $scope.publisherlst.length; i++) {                            if ($scope.editlis[0].LMP_Id == $scope.publisherlst[i].lmP_Id) {                                $scope.publisherlst[i].Selected = true;                                $scope.LMP_Id = $scope.publisherlst[i];                                $scope.newcaste = $scope.publisherlst[0].lmP_Id;                                                           }                        }                    }
