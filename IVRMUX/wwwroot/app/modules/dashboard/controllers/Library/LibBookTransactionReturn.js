(function () {
    ...
    angular
        .module('app')
        .controller('LibBookTransactionReturnController', LibBookTransactionReturnController)

    LibBookTransactionReturnController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter','$timeout']
    function LibBookTransactionReturnController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $timeout) {
        ...
        $scope.ChangeDate = function (Issue_Date, HolidayFlag) {            HolidayFlag = true;            if ($scope.Issue_Date != "") {                $scope.returndate = true;                $scope.newdate = new Date($scope.Issue_Date);                $scope.ret_date = $scope.newdate.setDate($scope.newdate.getDate() + $scope.maxdays);                $scope.Return_Date = new Date($scope.ret_date);                if (HolidayFlag == true) {                    var data = {                        "Return_Date": new Date($scope.ret_date).toDateString(),                        "Issue_Date": new Date($scope.Issue_Date).toDateString(),                        "difftotaldays": 0,                    }                    $scope.finelist = [];                    var config = {                        headers: {                            'Content-Type': 'application/json;'                        }                    }                    apiService.create("BookTransaction/ShowDiffrentDays", data).                        then(function (promise) {                            if (promise.configdata != null && promise.configdata.length > 0) {                                $scope.Due_Date = (new Date(promise.configdata[0].Holidays));                                $scope.Return_Date = (new Date(promise.configdata[0].Holidays));                            }                            else {                                $scope.Due_Date = new Date($scope.ret_date);                            }                        })                }                else {                    $scope.Due_Date = new Date($scope.ret_date);                }            }            else {                $scope.returndate = false;            }        }
        $scope.LMBANO_Id = '';
        ...
        $scope.showfine = function (dobj) {
            ...
            apiService.create("BookTransaction/showfine", data).
                then(function (promise) {
        ...
        $scope.Loaddata = function () {
            ...
            var pageid = 2;
            apiService.create("BookTransaction/getdetailsReturn", data).then(function (promise) {
                ...
                if (promise.msg != 'N') {
                    if ($scope.issuertype1 == 'std') {
                        ...
                        if (promise.clamsg != 'NS') {
                            ...
                            $scope.studlisttemp = [];
                            $scope.studlisttemp = promise.studentlist;                     
                         //   $scope.studlist = promise.studentlist;
                            ...
                            $scope.studlist = [];
                            if ($scope.studlisttemp != null && $scope.studlisttemp.length > 0) {
                                angular.forEach($scope.studlisttemp, function (dev) {
                                    if ($scope.ASMAY_Id == dev.ASMAY_Id) {
                                        $scope.studlist.push(dev);
                                    }
                                });
                            }
                    ...
                    if ($scope.onloadFlag == true) {
                        ...
                        $scope.get_StudentsmartCard();
                        $timeout($scope.Loaddata, 7999);
                    }
                }
                else {
        ...
        $scope.Savedata = function () {
            if ($scope.myForm.$valid) {
                if ($scope.LMBANO_Id !== "" && $scope.LMBANO_Id != null) {
                    ...
                    apiService.create("BookTransaction/Savedata", data)
                        .then(function (promise) {
        ...
        $scope.get_staff1 = function () {
            ...
            apiService.create("BookTransaction/get_staff1", data).then(function (promise) {
        ...
        $scope.getdepchange = function () {
            ...
            apiService.create("BookTransaction/getdepchange", data).then(function (promise) {
                $scope.filldesignation = promise.filldesignation;
                //  $scope.stafftlist = promise.stafftlist;
        ...
        $scope.loadbookavail = function () {
            if ($scope.book_avail == true) {
                ...
                apiService.create("BookTransaction/loadbookavail", data).then(function (promise) {
        ...
        $scope.get_Studentdetails = function () {
            ...
            apiService.create("BookTransaction/studentdetails", data).then(function (promise) {
                if ($scope.issuertype1 == 'std') {
        ...
        $scope.barcodechangetwo = function (dd) {
            if (dd.length >= '1') {
                $scope.finamt = 0;
                ...
                if ($scope.issuertype1 == 'std') {
                    ...
                    var asmcL_Id = 0;
                    var amsT_Id = 0;
                    if ($scope.booktitle != null && $scope.booktitle.length > 0) {
                        angular.forEach($scope.booktitle, function (stusubj) {
                            if (stusubj.lmbanO_AccessionNo === dd) {
                                book_Trans_Id = stusubj.book_Trans_Id;
                                asmcL_Id = stusubj.asmcL_Id;
                                amsT_Id = stusubj.amsT_Id;
                            }
                        });
                    }
                    else {
                        swal("Books Transctions Not Available  ! ")
                        return;
                    }
                    data = {
                ...
                apiService.create("BookTransaction/Editdata", data)
                    .then(function (promise) {
                        if ($scope.issuertype1 == 'std') {
                            $scope.booktitle = promise.booktitle;
        ...
        $scope.barcodechange = function (dd) {
            ...
            if (dd.length >= '1') {
                ...
                apiService.create("BookTransaction/searchfilterbarcode", data).
                    then(function (promise) {
                        $scope.booktitle = promise.booktitle;
        ...
        $scope.barcodechange1 = function (dd) {
            ...
            apiService.create("BookTransaction/searchfilterbarcode1", data).
                then(function (promise) {
        ...
        $scope.stdSearch_Grid = function () {
            ...
            apiService.create("BookTransaction/stdSearch_Grid", data).
                then(function (promise) {
        ...
        $scope.searchfilter = function (objj) {
            ...
            if (objj.search.length >= '1') {
                ...
                apiService.create("BookTransaction/searchfilter", data).
                    then(function (promise) {
        ...
        $scope.searchfilteravail = function (objj) {
            ...
            if (objj.search.length >= '1') {
                ...
                apiService.create("BookTransaction/searchfilteravail", data).
                    then(function (promise) {
                        $scope.avilbooktitle = promise.avilbooktitle;
                    })
            }
        ...
        $scope.get_bookdetails = function () {
            ...
            apiService.create("BookTransaction/get_bookdetails", data).then(function (promise) {
        ...
        $scope.availget_bookdetails = function () {
            ...
            apiService.create("BookTransaction/availget_bookdetails", data).then(function (promise) {
        ...
        $scope.get_bookdetailsedit = function (user) {
            ...
            if ($scope.issuertype1 == 'std') {
                ...
                if ($scope.booktitle != null && $scope.booktitle.length > 0) {
                    angular.forEach($scope.booktitle, function (stusubj) {
                        if (stusubj.lmbanO_Id === $scope.LMBANO_Id.lmbanO_Id) {
                            book_Trans_Id = stusubj.book_Trans_Id;
                            asmcL_Id = stusubj.asmcL_Id;
                            amsT_Id = stusubj.amsT_Id;
                        }
        ...
        $scope.renewaldata = function () {
            ...
            if (issuedate == '' && issuedate == null) {
                ...
                if ($scope.myForm.$valid) {
                    ...
                    apiService.create("BookTransaction/renewaldata", data)
                        .then(function (promise) {
                            if (promise.returnval != null && promise.duplicate != null) {
        ...
        $scope.returndata = function () {
            ...
            if (returndate != null && returndate != "" && returndate != undefined) {
                ...
                if ($scope.Book_Trans_Status != 'Return') {
                    apiService.create("BookTransaction/returndata", data)
                        .then(function (promise) {
        ...
        $scope.cancel = function () {
            $scope.studentdata = [];
            $scope.selctstaffdata = [];
            $scope.circularparamdetails = [];
            $scope.AMST_FirstName = ""; $scope.AMST_RegistrationNo = "";
            $scope.AMST_AdmNo = ""; $scope.AMAY_RollNo = ""; $scope.ASMCL_ClassName = "";
            $scope.ASMC_SectionName = ""; $scope.AMST_Photoname = ""; $scope.amsT_FatherMobleNo = "";
            $scope.amsT_FatherMobleNo = "";
            $scope.HRME_MobileNo = ""; $scope.HRMDES_DesignationName = ""; $scope.HRME_Photo = "";
            $scope.HRME_EmployeeCode = ""; $scope.HRME_EmployeeFirstName = ""; $scope.HRMD_DepartmentName = "";
            var data = {
                "LMBANO_Id": 0,
            }
            apiService.create("BookTransaction/availget_bookdetails", data).then(function (promise) {
        ...
        $scope.get_StudentsmartCard = function () {
            $scope.maxmsg = "";
            $scope.booktitle = [];
          
            var data = {               
                "searctype": $scope.issuertype1,
                "LMAL_Id": $scope.LMAL_Id,
                "ASMAY_Id": $scope.ASMAY_Id
            }
            apiService.create("BookTransaction/GettransctionDetails/", data).
                then(function (promise) {

                    if (promise.booktitle != null && promise.booktitle.length > 0) {
                        $scope.booktitle = promise.booktitle;
                        if (promise.userSsdo_Id > 0) {
                       
                            $scope.AMST_Id = $scope.studlist[0];
                            angular.forEach($scope.studlist, function (dev) {
                                if (promise.userSsdo_Id && dev.amsT_Id) {
                                    $scope.ASMCL_Id = dev.asmcL_Id;
                                    $scope.asmS_Id = dev.asmS_Id;
                                }
                            });
                            $scope.get_Studentdetails();
                            $scope.onloadFlag = true;
        ...
        $scope.get_Studentdetailstwo = function () {
            $scope.booktitle = [];
           
            var data = {
                "UserSsdo_Id": $scope.AMST_Id.amsT_Id,
                "searctype": $scope.issuertype1,
                "LMAL_Id": $scope.LMAL_Id,
                "ASMAY_Id": $scope.ASMAY_Id
            }
        ...
        $scope.GetStudentDetails1 = function () {
            ...
            if ($scope.ASMCL_Id > 0 && $scope.asmS_Id > 0 && $scope.ASMAY_Id > 0) {
                angular.forEach($scope.studlisttemp, function (dev) {
                    if ($scope.ASMCL_Id == dev.ASMCL_Id && $scope.asmS_Id == dev.ASMS_Id && $scope.ASMAY_Id == dev.ASMAY_Id) {
                        $scope.studlist.push(dev);
                    }
                });
                if ($scope.studlist != null && $scope.studlist.length > 0) {
                    $scope.onloadFlag = true;
                }
                else {
                    swal("Students Are Not Found These Sections !")
                }
                //  $scope.studlisttemp = promise.studentlist;
            }
            else {
            ...
            //}
            //apiService.create("BookTransaction/GetStudentDetails1/", data).
            //    then(function (promise) {
