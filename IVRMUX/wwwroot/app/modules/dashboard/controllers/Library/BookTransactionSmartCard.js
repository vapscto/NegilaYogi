(function () {
    ...
    angular
        .module('app')
        .controller('BookTransactionSmartCardController', BookTransactionController)

    BookTransactionController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$interval', '$timeout']
    function BookTransactionController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $interval, $timeout) {
        ...
        $scope.transtypechange = function () {
            ...
            apiService.create("BookTransaction/availget_bookdetails", data).then(function (promise) {
              ...
              $scope.Loaddata();
            // $timeout($scope.transtypechange, 6999);
           // 
        }
        ...
        $scope.Loaddata = function () {
            ...
            var pageid = 2;
            apiService.create("BookTransaction/getdetails", data).then(function (promise) {
        ...
        $scope.get_bookdetails = function () {
            ...
            apiService.create("BookTransaction/get_bookdetails", data).then(function (promise) {
                ...
                if ($scope.bookdetails.length != 0 && $scope.bookdetails != null) {
                    ...
                    $scope.lmrA_RackName = promise.bookdetails[0].lmrA_RackName;
            ...
            apiService.create("BookTransaction/Editdata", data)
                .then(function (promise) {
                    if ($scope.issuertype1 == 'std') {
                        ...
                        //if ($scope.AMST_Id = promise.editlist[0];)
                    }
                    else if ($scope.issuertype1 == 'stf') {
                        $scope.booktitle = promise.booktitle;
                        $scope.editlist = promise.editlist;
        ...
        $scope.get_StudentsmartCard = function () {
            ...
            $scope.HRME_MobileNo = ""; $scope.HRMDES_DesignationName = ""; $scope.HRME_Photo = "";
            $scope.HRME_EmployeeCode = ""; $scope.HRME_EmployeeFirstName = ""; $scope.HRMD_DepartmentName = "";
            $scope.HRMD_Id = ""; $scope.HRMDES_Id = ""; $scope.HRME_Id = "";
            //$scope.ASMCL_Id = "";// $scope.asmS_Id = "";
            var b_id = 0; var a_id = 0;
            if ($scope.LMBANO_Id.lmB_Id > 0) {
                b_id = $scope.LMBANO_Id.lmB_Id
            }
            if ($scope.LMBANO_Id.lmbanO_Id > 0) {
                a_id = $scope.LMBANO_Id.lmbanO_Id;
            }
            ...
            apiService.create("BookTransaction/studentdetails", data).then(function (promise) {
                if ($scope.issuertype1 == 'std') {
                    $scope.studentdata = promise.studentdata;
                    if ($scope.studentdata.length != 0 && $scope.studentdata != null) {
                        if (promise.amsT_Id > 0) {
                            $scope.AMST_Id = promise.studentdata[0];
                        }
                        $scope.onoadFlag = true;
                        $scope.AMST_FirstName = promise.studentdata[0].amsT_FirstName;
                        $scope.AMST_RegistrationNo = promise.studentdata[0].amsT_RegistrationNo;
                    ...
                    $scope.selctstaffdata = promise.selctstaffdata;
                    if ($scope.selctstaffdata != null && $scope.selctstaffdata.length > 0) {
                        $scope.onoadFlag = true;
                        $scope.HRME_EmployeeCode = promise.selctstaffdata[0].hrmE_EmployeeCode;
                        $scope.HRME_EmployeeFirstName = promise.selctstaffdata[0].hrmE_EmployeeFirstName;
                        $scope.HRMD_DepartmentName = promise.selctstaffdata[0].hrmD_DepartmentName;
                        ...
                        $scope.HRMD_Id = promise.selctstaffdata[0].hrmD_Id;
                        $scope.HRMDES_Id = promise.selctstaffdata[0].hrmdeS_Id;
                        $scope.HRME_Id = promise.selctstaffdata[0].hrmE_Id;; 
