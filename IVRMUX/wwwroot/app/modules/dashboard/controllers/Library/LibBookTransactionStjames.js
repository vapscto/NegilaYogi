(function () {
    ...
    angular
        .module('app')
        .controller('LibBookTransactionStjamesController', BookTransactionController)

    BookTransactionController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter']
    function BookTransactionController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter) {
        ...
        $scope.GeDateFiltred = function () {
            $scope.reportlist = [];
            if ($scope.BokStud == false) {
                return;
            }
            var issuertype1 = "";
            ...
            if ($scope.issuertype == "stf") {
                issuertype1 = "STAFF";
            }
            if ($scope.issuertype == "dep") {
                issuertype1 = "DEPARTMENT";
            }
            if ($scope.issuertype == "gst") {
                issuertype1 = "GUEST";
            }
            var TYPE = $scope.Return_Date == null ? "" : $filter('date')($scope.Return_Date, "yyyy-MM-dd");
            var C1 = $scope.Due_Date == null ? "" : $filter('date')($scope.Due_Date, "yyyy-MM-dd");
            var ClassNo = $scope.Issue_Date == null ? "" : $filter('date')($scope.Issue_Date, "yyyy-MM-dd");
            var data = {
                "Title": "Calculation",
                "TYPE": TYPE,
                "ClassNo": ClassNo,
            ...
            apiService.create("OpacSearch/report", data).then(function (promise) {
                if (promise.reportlist != null && promise.reportlist.length > 0) {
                    $scope.finamt = promise.reportlist[0].AMOUNT;
                    if ($scope.finamt > 0 && $scope.finecollected > 0) {
                        $scope.LBTR_FineCollected = (Number($scope.finamt) - (Number($scope.finecollected)));
                    }
                    else {
                        $scope.LBTR_FineCollected = promise.reportlist[0].AMOUNT;
                    }
        ...
        $scope.stdSearch_Grid = function () {
            ...
            var data = {
                ...
                "Issue_Date": issuedate,
                "bookcat_type": $scope.ReturnStatus,
            }
            var config = {
                headers: {
                    'Content-Type': 'application/json;'
        ...
        $scope.EditData = function (user) {
            ...
            apiService.create("BookTransaction/Editdata", data)
                .then(function (promise) {
                    ...
                    $scope.LBTR_FineCollected = $scope.finamt;
                    $scope.GeDateFiltred();
                })
