(function () {
    ...
    angular
        .module('app')
        .controller('DefaulterFeeCollectionGraphController', DefaulterFeeCollectionGraphController)
    DefaulterFeeCollectionGraphController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout']
    function DefaulterFeeCollectionGraphController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        $scope.route = false;
        $scope.print = false;
        ...
        $scope.loaddata = function () {
            ...
            apiService.getURI("DefaulterFeeCollectionGraph/getalldetails", pageid).
                then(function (promise) {

                    $scope.arrlist6 = promise.adcyear;
                    $scope.termlist = promise.terms;
                    $scope.asmaY_Id = academicyrlst[0].asmaY_Id;
                    $scope.FMT_Id = promise.terms[0].fmT_Id;
                    $scope.ShowReport();
                    $scope.ShowReport();
                    $scope.ShowReport();


                    //$scope.asmaY_Id = promise.adcyear[0].asmaY_Id;
                    //$scope.FMT_Id = promise.terms[0].fmT_Id;
                    //$scope.ShowReport();
                })
        }
        ...
        $scope.onselectyear = function () {
            ...
            apiService.create("DefaulterFeeCollectionGraph/getheadisbygrpid", data).
                then(function (promise) {
        ...
        $scope.ShowReport = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("DefaulterFeeCollectionGraph/radiobtndata", data).
                    then(function (promise) {
                        ...
                        for (var i = 0; i < promise.studentalldata.length; i++) {
                            $scope.ASMCL_ClassName.push(promise.studentalldata[i].ASMCL_ClassName);
                        }

                        $scope.TotalStrength = [];
                        for (var i = 0; i < promise.studentalldata.length; i++) {
                            $scope.TotalStrength.push(promise.studentalldata[i].TotalStrength);
                        }
                        $scope.Defaulters = [];
                        for (var i = 0; i < promise.studentalldata.length; i++) {
                            $scope.Defaulters.push(promise.studentalldata[i].Defaulters);
                        }

                        $scope.Defaulterscnt = [];
                        for (var i = 0; i < promise.studentalldata.length; i++) {
                            $scope.Defaulterscnt.push(promise.studentalldata[i].Defaulterscnt);
                        }
                        $scope.TillNowPaidCount = [];
                        for (var i = 0; i < promise.studentalldata.length; i++) {
                            $scope.TillNowPaidCount.push(promise.studentalldata[i].TillNowPaidCount);
                        }

                        $scope.DefaultersTotal = [];
                        for (var i = 0; i < promise.studentalldata.length; i++) {
                            $scope.DefaultersTotal.push(promise.studentalldata[i].DefaultersTotal);
                        }



                        function createChart1() {                            $("#chart1").kendoChart({                                title: {                                    text: "Defaulter Fee Collection Graph"                                },                                legend: {                                    position: "bottom"                                },                                chartArea: {                                    background: ""                                },                                seriesDefaults: {                                    type: "line",                                    style: "smooth"                                },                                series: [{                                    name: "Total Strength",                                    data: $scope.TotalStrength,                                },                                {                                    name: "Total Paid Count",                                    data: $scope.TillNowPaidCount,                                },                                {                                    name: "Total Defaulter",                                    data: $scope.DefaultersTotal,                                }, {                                    name: "Before Duedate Paid Count",                                    data: $scope.Defaulters,
                                },                                {                                    name: "After Duedate Paid Count",                                    data: $scope.Defaulterscnt,                                }                                ],                                valueAxis: {                                    labels: {                                        format: "{0}"                                    },                                    line: {                                        visible: false                                    },                                    axisCrossingValue: -10                                },                                categoryAxis: {                                    categories: $scope.ASMCL_ClassName,                                    majorGridLines: {                                        visible: false                                    },                                    labels: {                                        rotation: "auto"                                    }                                },                                tooltip: {                                    visible: true,                                    format: "{0}",                                    template: "#= series.name #: #= value #"                                }                            });                        }                        $(document).ready(createChart1);                        $(document).bind("kendo:skinChange", createChart1);                        $("#chart2").kendoChart({
                            title: {
                                text: "Category Wise Fee Collection"
                            },
                            legend: {
                                position: "top"
                            },
...

        {
            ...
            if ($scope.bulkdownload == true) {
                if ($scope.myForm.$valid) {
                    ...
                    apiService.create("CategoryWiseFeeCollection/radiobtndata", data).
                        then(function (promise) {
