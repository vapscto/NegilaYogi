(function () {
    ...
    angular
        .module('app')
        .controller('CategoryWiseFeeCollectionController', CategoryWiseFeeCollectionController)
    CategoryWiseFeeCollectionController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout']
    function CategoryWiseFeeCollectionController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        $scope.route = false;
        $scope.print = false;
        ...
        $scope.loaddata = function () {
            ...
            apiService.getURI("CategoryWiseFeeCollection/getalldetails", pageid).
                then(function (promise) {

                    $scope.arrlist6 = promise.adcyear;
                    $scope.cfg.asmaY_Id = 82;
                    $scope.ShowReport();
                ...
                apiService.create("CategoryWiseFeeCollection/radiobtndata", data).
                    then(function (promise) {
                        ...
                        for (var i = 0; i < promise.studentalldata.length; i++) {
                            $scope.category.push(promise.studentalldata[i].FMCC_ClassCategoryName);
                        }

                        $scope.stdcount = [];
                        for (var i = 0; i < promise.studentalldata.length; i++) {
                            $scope.stdcount.push(promise.studentalldata[i].studentcount);
                        }
                        $scope.estimated = [];
                        for (var i = 0; i < promise.studentalldata.length; i++) {
                            $scope.estimated.push(promise.studentalldata[i].Estimatedfeetobecollcted);
                        }

                        $scope.collection = [];
                        for (var i = 0; i < promise.studentalldata.length; i++) {
                            $scope.collection.push(promise.studentalldata[i].Feecollectiontillnow);
                        }
                        $scope.Pendingcount = [];
                        for (var i = 0; i < promise.studentalldata.length; i++) {
                        ...
                        $scope.excesscount = [];
                        for (var i = 0; i < promise.studentalldata.length; i++) {
                            $scope.excesscount.push(promise.studentalldata[i].excess);
                        }
                        ...
                        function createChart1() {                            $("#chart1").kendoChart({                                title: {                                    text: "Category Wise Fee Collection Graph"                                },                                legend: {                                    position: "bottom"                                },                                chartArea: {                                    background: ""                                },                                seriesDefaults: {                                    type: "line",                                    style: "smooth"                                },                                series: [                                    //    {                                    //    name: "Total Mapped Student Count",                                    //    data: $scope.stdcount,                                    //},                                    {                                        name: "Estimated Fee to be Collected",                                        data: $scope.estimated,                                    }, {                                        name: "Fee Collection Till Now",                                        data: $scope.collection,
