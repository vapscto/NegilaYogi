(function () {
    ...
    angular
        .module('app')
        .controller('NAAC_HSU_StudentComplaints252Page', NAAC_HSU_StudentComplaints252Page)

    NAAC_HSU_StudentComplaints252Page.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce', 'myFactorynaac'];
    function NAAC_HSU_StudentComplaints252Page($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        ...
        $scope.loaddata = function () {
            ...
            //var pageid = 2;
            apiService.getURI("NAAC_HSU_StudentComplaints252Page/loaddata", $scope.mI_Id).then(function (promise) {
        ...
        $scope.save = function () {
            if ($scope.myForm.$valid) {
                ...
                debugger;
                var data = {
                    "NCHSU252SC_Id": $scope.NCHSU252SC_Id,
                    "asmaY_Id": $scope.NCHSU252SC_Year,
                    "NCHSU252SC_NoOfStudentsComplaints": $scope.NCHSU252SC_NoOfStudentsComplaints,
                    "NCHSU252SC_TotalNoOfStudentsAppereadExam": $scope.NCHSU252SC_TotalNoOfStudentsAppereadExam,
                    
                    filelist: $scope.materaldocuupload,
                    "MI_Id": $scope.mI_Id
                }

                apiService.create("NAAC_HSU_StudentComplaints252Page/save", data).then(function (promise) {
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            debugger;
            var dystring = "";
            if (usersem.nchsU252SC_ActiveFlag == true) {
                dystring = "Deactivate";
            }
            else if (usersem.nchsU252SC_ActiveFlag == false) {
                dystring = "Activate"
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAAC_HSU_StudentComplaints252Page/deactive", usersem).
        ...
        $scope.edittab1 = function (user) {
            debugger;
            var data = {
                "NCHSU252SC_Id": user.nchsU252SC_Id,
                "MI_Id": user.mI_Id
            }
            apiService.create("NAAC_HSU_StudentComplaints252Page/EditData", data).then(function (promise) {
                if (promise.editlist.length > 0) {
                    $scope.instit = true;
                    $scope.NCHSU252SC_Id = promise.editlist[0].nchsU252SC_Id;
                    $scope.NCHSU252SC_Year = promise.editlist[0].nchsU252SC_Year;
                    $scope.NCHSU252SC_NoOfStudentsComplaints = promise.editlist[0].nchsU252SC_NoOfStudentsComplaints;
                    $scope.NCHSU252SC_TotalNoOfStudentsAppereadExam = promise.editlist[0].nchsU252SC_TotalNoOfStudentsAppereadExam;
        ...
        $scope.viewdocument = function (obj) {
            ...
            $scope.objdata = obj;
            apiService.create("NAAC_HSU_StudentComplaints252Page/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.deleteuploadfile = function (docfile) {
            var data = {
                "NCHSU252SCF_Id": docfile.nchsU252SCF_Id,
                "NCHSU252SC_Id": docfile.nchsU252SC_Id,
                "MI_Id": docfile.mI_Id
            };
            swal({
                title: "Are You Sure",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAAC_HSU_StudentComplaints252Page/deleteuploadfile", data).then(function (promise) {
        ...
        $scope.change_institution = function () {
            $scope.NCHSU252SC_Year = '';
            $scope.NCHSU252SC_NoOfStudentsComplaints = '';
            $scope.NCHSU252SC_TotalNoOfStudentsAppereadExam = '';
