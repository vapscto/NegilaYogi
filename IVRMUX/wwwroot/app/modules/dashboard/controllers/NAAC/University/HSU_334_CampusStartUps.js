...
(function () {
    ...
    angular
        .module('app')
        .controller('HSU_334_CampusStartUpsController', HSU_334_CampusStartUpsController);

    HSU_334_CampusStartUpsController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce', 'myFactorynaac'];

    function HSU_334_CampusStartUpsController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        ...
        $scope.loaddata = function () {
            ...
            //var pageid = 2;
            apiService.getURI("HSU_334_CampusStartUps/loaddata", $scope.mI_Id).
        ...
        $scope.deactive = function (usersem, SweetAlert) {
            var dystring = "";
            if (usersem.nchsU324CS_ActiveFlag == true) {
                dystring = "Deactivate";
            }
            else if (usersem.nchsU324CS_ActiveFlag == false) {
                dystring = "Activate"
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("HSU_334_CampusStartUps/deactive", usersem).
                            then(function (promise) {
        ...
        $scope.edittab1 = function (user) {
            var data = {
                "NCHSU324CS_Id": user.nchsU324CS_Id,
                "MI_Id": user.mI_Id
            }

            apiService.create("HSU_334_CampusStartUps/EditData", data).then(function (promise) {

                if (promise.editlist.length > 0) {
                    $scope.instit = true;
                    $scope.NCHSU324CS_Id = promise.editlist[0].nchsU324CS_Id;
                    $scope.NCHSU324CS_Year = promise.editlist[0].nchsU324CS_Year;
                    $scope.NCHSU324CS_StartUpName = promise.editlist[0].nchsU324CS_StartUpName;
                    $scope.NCHSU324CS_NatureOfStartUp = promise.editlist[0].nchsU324CS_NatureOfStartUp;
                    $scope.NCHSU324CS_Contactinfo = promise.editlist[0].nchsU324CS_Contactinfo;                    
        ...
        $scope.save = function () {
            ...
            if ($scope.myForm.$valid) {
                var data = {
                    "NCHSU324CS_Id": $scope.NCHSU324CS_Id,
                    "asmaY_Id": $scope.NCHSU324CS_Year,
                    "NCHSU324CS_StartUpName": $scope.NCHSU324CS_StartUpName,
                    "NCHSU324CS_NatureOfStartUp": $scope.NCHSU324CS_NatureOfStartUp,
                    "NCHSU324CS_Contactinfo": $scope.NCHSU324CS_Contactinfo,                    
                    "MI_Id": $scope.mI_Id,
                    filelist: $scope.materaldocuupload,
                }

                apiService.create("HSU_334_CampusStartUps/save", data).then(function (promise) {
                    debugger;
        ...
        $scope.viewdocument = function (obj) {
            ...
            $scope.objdata = obj;
            apiService.create("HSU_334_CampusStartUps/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.deleteuploadfile = function (docfile) {
            ...
            var data = {
                "NCHSU324CSF_Id": docfile.nchsU324CSF_Id,
                "NCHSU324CS_Id": docfile.nchsU324CS_Id,
                "MI_Id": docfile.mI_Id
            };
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("HSU_334_CampusStartUps/deleteuploadfile", data).then(function (promise) {
        ...
        $scope.change_institution = function () {
            $scope.NCHSU324CS_Id = 0;
            $scope.asmaY_Id = '';
            $scope.NCHSU324CS_StartUpName = '';
            $scope.NCHSU324CS_NatureOfStartUp = '';
            $scope.NCHSU324CS_Contactinfo = '';            
            $scope.usercheckC = '';
