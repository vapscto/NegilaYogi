...
(function () {
    ...
    angular
        .module('app')
        .controller('HSU_PhyAcaFacility451Report', HSU_PhyAcaFacility451Report)

    HSU_PhyAcaFacility451Report.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function HSU_PhyAcaFacility451Report($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.Report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("Naac_HSU_CR4Report/HSU_PhyAcaFacility451Report", data).then(function (promise) {
                    ...
                    if (promise.govtsclist.length > 0 && promise.govtsclist != null) {
                        ...
                        angular.forEach($scope.govtsclist, function (gg) {
                            ...
                            angular.forEach($scope.govtsclistfiles, function (ff) {
                                if (gg.NCAC441ExAcFc_Id == ff.ncaC441ExAcFc_Id) {
                                    govtfiles.push({ filename: ff.ncaC441ExAcFcF_FileName, filepath: ff.ncaC441ExAcFcF_FilePath, filedesc: ff.ncaC441ExAcFcF_Filedesc })
                                }
                            })
                            $scope.finalarray.push({ YEAR: gg.ASMAY_Year, NCAC441EXACFC_ExpAccFacility: gg.NCAC441EXACFC_ExpAccFacility, NCAC441EXACFC_ExpPhyFacility: gg.NCAC441EXACFC_ExpPhyFacility, FILES: govtfiles })
                        })
