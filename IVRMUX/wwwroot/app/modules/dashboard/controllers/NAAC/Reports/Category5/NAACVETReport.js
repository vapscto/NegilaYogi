(function () {
    ...
    angular
        .module('app')
        .controller('NAACVETReportController', NAACVETReportController)

    NAACVETReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout']
    function NAACVETReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        ...
        $scope.showdetails = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("NAACCriteriaFiveReport/get_report515", data).then(function (promise) {
                ...
                $scope.govtsclistfiles = promise.govtsclistfiles;
                    ...
                    angular.forEach($scope.govtsclist, function (gg) {
                        ...
                        angular.forEach($scope.govtsclistfiles, function (ff) {
                            if (gg.ncaC515VET_Id == ff.ncaC515VET_Id) {
                                govtfiles.push({ filename: ff.ncaC515VETF_FileName, filepath: ff.ncaC515VETF_FilePath, filedesc: ff.ncaC515VETF_Filedesc})
                            }
                        })
                        $scope.finalarray.push({ YEAR: gg.asmaY_Year, SCHEME: gg.program, GCNT: gg.noofstd, FILES: govtfiles})
                    })
