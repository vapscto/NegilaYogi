...
(function () {
    ...
    angular
        .module('app')
        .controller('NAAC_HSU_InterdisciplinaryProgrammes_123Report', NAAC_HSU_InterdisciplinaryProgrammes_123Report)

    NAAC_HSU_InterdisciplinaryProgrammes_123Report.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function NAAC_HSU_InterdisciplinaryProgrammes_123Report($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.Report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("HSU_CR1_Report/HSU_123_Report", data).then(function (promise) {
                    ...
                    if (promise.govtsclist.length > 0 && promise.govtsclist != null) {
                        ...
                        angular.forEach($scope.govtsclist, function (gg) {
                            ...
                            angular.forEach($scope.govtsclistfiles, function (ff) {
                                if (gg.filekid == ff.nchsuiP123_Id) {
                                    govtfiles.push({ filename: ff.nchsuiP123F_FileName, filepath: ff.nchsuiP123F_FilePath, filedesc: ff.nchsuiP123F_FileDesc })
                                }
                            })
                            $scope.finalarray.push({ YEAR: gg.ASMAY_Year, NCHSUIP123_TotalNoOfProg: gg.NCHSUIP123_TotalNoOfProg, NCHSUIP123_TotalNoOfCoursesAcrossProgs: gg.NCHSUIP123_TotalNoOfCoursesAcrossProgs, NCHSUIP123_NoOfInterdisciplinaryCoursesAcsProg: gg.NCHSUIP123_NoOfInterdisciplinaryCoursesAcsProg, FILES: govtfiles })
                        })
