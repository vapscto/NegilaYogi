(function () {
    'use strict';
    angular.module('app').controller('HSU_InterdisciplinaryCourses123', HSU_InterdisciplinaryCourses123)

    HSU_InterdisciplinaryCourses123.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout']
    function HSU_InterdisciplinaryCourses123($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout) {
        ...
        $scope.get_report = function () {
            ...
            apiService.create("HSU_CR1_Report/HSU_123_Report", data).then(function (promise) {
                ...
                if (promise.govtsclist.length > 0) {
                    $scope.showflag = true;
                    $scope.printflag = true;
                    $scope.reportlist = promise.govtsclist;
                    ...
                    angular.forEach($scope.yearlist, function (y1) {
                        ...
                        angular.forEach($scope.reportlist, function (y2) {
                            if (y1.asmaY_Year == y2.ASMAY_Year) {
                                ...
                                angular.forEach($scope.govtsclistfiles, function (ss) {
                                    if (y2.filekid == ss.nchsuiP123_Id) {
                    ...
                    console.log($scope.govtsclistfiles);
