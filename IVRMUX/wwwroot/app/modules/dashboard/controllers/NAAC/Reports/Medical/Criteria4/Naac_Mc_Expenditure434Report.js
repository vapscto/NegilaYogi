...
(function () {
    ...
    angular
        .module('app')
        .controller('Naac_MC_Expenditure434', Naac_MC_Expenditure434)

    Naac_MC_Expenditure434.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function Naac_MC_Expenditure434($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.Report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("Naac_MC_CR4/MedExpenditure434Report", data).then(function (promise) {
                    ...
                    if (promise.reportlist.length > 0) {
                        angular.forEach($scope.alldata4, function (tt) {
                            ...
                            angular.forEach($scope.alldata42, function (ss) {
                                if (tt.NCAC414BD_Id == ss.ncaC414BD_Id) {
