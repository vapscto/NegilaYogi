(function () {
    ...
    angular
        .module('app')
        .controller('NAAC364ReportController', NAAC364ReportController)

    NAAC364ReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout']
    function NAAC364ReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        ...
        $scope.exportToExcel = function (tableId) {
            var exportHref = Excel.tableToExcel(tableId, '3.6.4');
        ...
        $scope.showdetails = function () {
            ...
            //}
                ...
                apiService.create("NAACCriteria3Report/get_report364", data).then(function (promise) {
                    ...
                    angular.forEach($scope.govtsclist, function (gg) {
                        ...
                        angular.forEach($scope.govtsclistfiles, function (ff) {
                            if (gg.NCACET343_Id == ff.ncaceT343_Id) {
                                govtfiles.push({ filename: ff.ncaceT343F_FileName, filepath: ff.ncaceT343F_FilePath, filedesc: ff.ncaceT343F_Filedesc})
                            }
                        })
                        $scope.finalarray.push({ YEAR: gg.ASMAY_Year, SCHEME: gg.NCACET343_SchemeName, GCNT: gg.NCACET343_NoOfStudents, AG: gg.NCACET343_OrgAgency, ACT: gg.NCACET343_TypeOfActivity, FILES: govtfiles })
                    })
