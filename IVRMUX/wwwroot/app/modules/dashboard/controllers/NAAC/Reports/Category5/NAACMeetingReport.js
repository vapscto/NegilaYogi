(function () {
    ...
    angular
        .module('app')
        .controller('NAACMeetingReportController', NAACMeetingReportController)

    NAACMeetingReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout']
    function NAACMeetingReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        ...
        $scope.exportToExcel = function (tableId) {
            var exportHref = Excel.tableToExcel(tableId, '5.4.3');
        ...
        $scope.showdetails = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("NAACCriteriaFiveReport/get_report543", data).then(function (promise) {
                ...
                $scope.govtsclistfiles = promise.govtsclistfiles;
                    ...
                    angular.forEach($scope.govtsclist, function (gg) {
                        ...
                        angular.forEach($scope.govtsclistfiles, function (ff) {
                            if (gg.ncaC543ALMMET_Id == ff.ncaC543ALMMET_Id) {
                                govtfiles.push({ filename: ff.ncaC543ALMMETF_FileName, filepath: ff.ncaC543ALMMETF_FilePath, filedesc: ff.ncaC543ALMMETF_Filedesc})
                            }
                        })
                        $scope.finalarray.push({ YEAR: gg.asmaY_Year, MCNT: gg.ncaC543ALMMET_NoOfMeetings, DATE: gg.ncaC543ALMMET_MeetingDate, MACNT: gg.ncaC543ALMMET_NoOfMemAttnd, ALCNT: gg.ncaC543ALMMET_TotalAlumniCount, FILES: govtfiles})
                    })
