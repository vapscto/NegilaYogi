...
(function () {
    ...
    angular
        .module('app')
        .controller('HSU_ExpenditureBook434Report', HSU_ExpenditureBook434Report)

    HSU_ExpenditureBook434Report.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function HSU_ExpenditureBook434Report($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.loaddata = function () {
            var pageid = 2;
            apiService.getURI("Naac_HSU_CR4Report/loaddata", pageid).then(function (promise) {
        ...
        $scope.Report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("Naac_HSU_CR4Report/HSU_ExpenditureBook434Report", data).then(function (promise) {
                    ...
                    if (promise.govtsclist.length > 0 && promise.govtsclist != null) {
                        ...
                        angular.forEach($scope.govtsclist, function (gg) {
                            ...
                            angular.forEach($scope.govtsclistfiles, function (ff) {
                                if (gg.NCAC424EXP_Id == ff.ncaC424EXP_Id) {
                                    govtfiles.push({ filename: ff.ncaC424EXPF_FileName, filepath: ff.ncaC424EXPF_FilePath, filedesc: ff.ncaC424EXPF_Filedesc })
                                }
                            })
                            $scope.finalarray.push({ YEAR: gg.ASMAY_Year, NCAC424EXP_BooksExp: gg.NCAC424EXP_BooksExp, NCAC424EXP_JournalExp: gg.NCAC424EXP_JournalExp, NCAC424EXP_EJournalExp: gg.NCAC424EXP_EJournalExp, NCAC424EXP_Id: gg.NCAC424EXP_Id, FILES: govtfiles })
                        })
