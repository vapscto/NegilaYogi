(function () {
    ...
    angular
        .module('app')
        .controller('NAAC_AC_351_Linkage_Report', NAAC_AC_351_Linkage_Report)

    NAAC_AC_351_Linkage_Report.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function NAAC_AC_351_Linkage_Report($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        //    var pageid = 2;
        //    apiService.getURI("NAAC_AC_351_Linkage_Report/loaddata", pageid).then(function (promise) {
        ...
        $scope.Report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("NAAC_AC_351_Linkage_Report/report", data).then(function (promise) {
                    ...
                    if (promise.reportlist.length > 0) {
                        angular.forEach($scope.alldata4, function (tt) {
                            ...
                            angular.forEach($scope.alldata42, function (ss) {
                                if (tt.NCAC351LIN_Id == ss.ncaC351LIN_Id) {
