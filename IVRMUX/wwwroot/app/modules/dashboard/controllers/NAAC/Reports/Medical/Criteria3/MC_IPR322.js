(function () {
    ...
    angular
        .module('app')
        .controller('MC_IPR322', MC_IPR322)

    MC_IPR322.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout']
    function MC_IPR322($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        ...
        $scope.exportToExcel = function (table) {
            ...
            var excelname = 'Cat 3.2.2.xls';
            var exportHref = Excel.tableToExcel(table, '3.2.2');
            $timeout(function () {
        ...
        $scope.get_report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("Medical_Criteria3Reports/MC_322_Report", data).then(function (promise) {
                    ...
                    if (promise.reportlist.length > 0) {
                        angular.forEach($scope.reportlist, function (tt) {
                            ...
                            angular.forEach($scope.reportlist2, function (ss) {
                                if (tt.NCACIPR322_Id == ss.ncacipR322_Id) {
