...
(function () {
    ...
    angular
        .module('app')
        .controller('HSEGovernance623', HSEGovernance623)

    HSEGovernance623.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function HSEGovernance623($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.Report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("Naac_HSU_CR6Report/HSUEGovernance623Report", data).then(function (promise) {
                    ...
                    if (promise.reportlist.length > 0) {
                        angular.forEach($scope.alldata4, function (tt) {
                            ...
                            angular.forEach($scope.alldata42, function (ss) {
                                if (tt.filefkid == ss.ncaC623EGOV_Id) {
