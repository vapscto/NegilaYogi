(function () {
    ...
    angular
        .module('app')
        .controller('NAACMasterACReportController', NAACMasterACReportController)

    NAACMasterACReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout']
    function NAACMasterACReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        ...
        $scope.exportToExcel = function (tableId) {
            var exportHref = Excel.tableToExcel(tableId, '5.3.3');
        ...
        $scope.showdetails = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("NAACCriteriaFiveReport/get_report533", data).then(function (promise) {
                ...
                $scope.govtsclistfiles = promise.govtsclistfiles;
                    ...
                    angular.forEach($scope.govtsclist, function (gg) {
                        ...
                        angular.forEach($scope.govtsclistfiles, function (ff) {
                            if (gg.ncaC533SPCAA_Id == ff.ncaC533SPCAA_Id) {
                                govtfiles.push({ filename: ff.ncaC533SPCAAF_FileName, filepath: ff.ncaC533SPCAAF_FilePath, filedesc: ff.ncaC533SPCAAF_Filedesc})
                            }
                        })
                        $scope.finalarray.push({ YEAR: gg.asmaY_Year, STDNAME: gg.program,FILES: govtfiles})
                        })
