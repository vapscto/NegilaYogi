...
(function () {
    ...
    angular
        .module('app')
        .controller('HSU_DevPrograms634Report', HSU_DevPrograms634Report)

    HSU_DevPrograms634Report.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function HSU_DevPrograms634Report($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.Report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("Naac_HSU_CR6Report/HSUDevPrograms634Report", data).then(function (promise) {
                    ...
                    if (promise.govtsclist.length > 0 && promise.govtsclist != null) {
                        ...
                        angular.forEach($scope.govtsclist, function (gg) {
                            ...
                            angular.forEach($scope.govtsclistfiles, function (ff) {
                                if (gg.filefkid == ff.ncaC634DEVPRG_Id) {
                            ...
                            $scope.finalarray.push({ YEAR: gg.ASMAY_Year, NCAC634DEVPRG_PDProgTitle: gg.NCAC634DEVPRG_PDProgTitle, datefromtocon: gg.datefromtocon, NCAC634DEVPRG_NoOfTeachersAttnd: gg.NCAC634DEVPRG_NoOfTeachersAttnd, NCAC634DEVPRG_NameOfTeachers: gg.NCAC634DEVPRG_NameOfTeachers,FILES: govtfiles })
                        })
