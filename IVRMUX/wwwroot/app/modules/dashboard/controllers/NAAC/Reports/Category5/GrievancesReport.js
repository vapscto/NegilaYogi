(function () {
    ...
    angular
        .module('app')
        .controller('GrievancesReportController', GrievancesReportController)

    GrievancesReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout']
    function GrievancesReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        ...
        $scope.exportToExcel = function (tableId) {
            var exportHref = Excel.tableToExcel(tableId, '5.1.6');
        ...
        $scope.showdetails = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("NAACCriteriaFiveReport/get_report516", data).then(function (promise) {
                ...
                $scope.govtsclistfiles = promise.govtsclistfiles;
                    ...
                    angular.forEach($scope.govtsclist, function (gg) {
                        ...
                        angular.forEach($scope.govtsclistfiles, function (ff) {
                            if (gg.ncaC513INSCH_Id == ff.ncaC516GRI_Id) {
                                govtfiles.push({ filename: ff.ncaC516GRIF_FileName, filepath: ff.ncaC516GRIF_FilePath, filedesc: ff.ncaC516GRIF_Filedesc})
                            }
                        })
                        $scope.finalarray.push({ YEAR: gg.asmaY_Year, APCNT: gg.ncaC516GRI_GRIAPP, GCNT: gg.ncaC516GRI_GRIRED, DAYS: gg.ncaC516GRI_AvgTime, FILES: govtfiles})
                    })
