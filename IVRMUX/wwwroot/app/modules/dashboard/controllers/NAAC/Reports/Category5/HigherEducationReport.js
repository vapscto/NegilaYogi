(function () {
    ...
    angular
        .module('app')
        .controller('HigherEducationReportController', HigherEducationReportController)

    HigherEducationReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout']
    function HigherEducationReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        ...
        $scope.exportToExcel = function (tableId) {
            var exportHref = Excel.tableToExcel(tableId, '5.2.2');
        ...
        $scope.showdetails = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("NAACCriteriaFiveReport/get_report522", data).then(function (promise) {
                ...
                $scope.govtsclistfiles = promise.govtsclistfiles;
                    ...
                    angular.forEach($scope.govtsclist, function (gg) {
                        ...
                        angular.forEach($scope.govtsclistfiles, function (ff) {
                            if (gg.ncaC522HRED_Id == ff.ncaC522HRED_Id) {
                                govtfiles.push({ filename: ff.ncaC522HREDF_FileName, filepath: ff.ncaC522HREDF_FilePath, filedesc: ff.ncaC522HREDF_Filedesc})
                            }
                        })
                        $scope.finalarray.push({ YEAR: gg.asmaY_Year, STDNAME: gg.atudentname, COURSE: gg.amcO_CourseName, INSNAME: gg.institutionname, BRANCH: gg.amB_BranchName, ADMPRG: gg.program, ADMDEP: gg.department, FILES: govtfiles})
                    })
