(function () {
    ...
    angular
        .module('app')
        .controller('MC_ResearchProjects313', MC_ResearchProjects313)

    MC_ResearchProjects313.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function MC_ResearchProjects313($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.loaddata = function () {
            ...
            var pageid = 2;
            apiService.getURI("Medical_Criteria3Reports/getdata", pageid).then(function (promise) {
        ...
        $scope.get_report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("Medical_Criteria3Reports/MC_313_Report", data).then(function (promise) {
                    ...
                    if (promise.reportlist.length > 0) {
                        angular.forEach($scope.reportlist, function (tt) {
                            ...
                            angular.forEach($scope.reportlist2, function (ss) {
                                if (tt.NC323RPR_Id == ss.nC323RPR_Id) {
