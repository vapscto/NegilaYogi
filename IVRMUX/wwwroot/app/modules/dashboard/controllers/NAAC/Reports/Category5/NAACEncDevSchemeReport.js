(function () {
    ...
    angular
        .module('app')
        .controller('NAACEncDevSchemeReportController', NAACEncDevSchemeReportController)

    NAACEncDevSchemeReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout']
    function NAACEncDevSchemeReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        ...
        $scope.exportToExcel = function (tableId) {
            var exportHref = Excel.tableToExcel(tableId, '5.1.3');
        ...
        $scope.showdetails = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("NAACCriteriaFiveReport/get_report513", data).then(function (promise) {
                ...
                $scope.govtsclistfiles = promise.govtsclistfiles;
                    ...
                    angular.forEach($scope.govtsclist, function (gg) {
                        ...
                        angular.forEach($scope.govtsclistfiles, function (ff) {
                            if (gg.ncaC513INSCH_Id == ff.ncaC513INSCH_Id) {
                                govtfiles.push({ filename: ff.ncaC513INSCHF_FileName, filepath: ff.ncaC513INSCHF_FilePath, filedesc: ff.ncaC513INSCHF_Filedesc})
                            }
                        })
                        $scope.finalarray.push({ YEAR: gg.asmaY_Year, SCHEME: gg.ncaC513INSCH_DevSchemeName, GCNT: gg.noofstd, agency: gg.ncaC513INSCH_AgencyDetails, FILES: govtfiles})
                    })
