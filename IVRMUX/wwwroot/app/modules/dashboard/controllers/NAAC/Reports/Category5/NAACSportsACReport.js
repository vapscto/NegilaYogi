(function () {
    ...
    angular
        .module('app')
        .controller('NAACSportsACReportController', NAACSportsACReportController)

    NAACSportsACReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout']
    function NAACSportsACReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        ...
        $scope.exportToExcel = function (tableId) {
            var exportHref = Excel.tableToExcel(tableId, '5.3.1');
        ...
        $scope.showdetails = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("NAACCriteriaFiveReport/get_report531", data).then(function (promise) {
                ...
                $scope.govtsclistfiles = promise.govtsclistfiles;
                    ...
                    angular.forEach($scope.govtsclist, function (gg) {
                        ...
                        angular.forEach($scope.govtsclistfiles, function (ff) {
                            if (gg.ncaC531SPCA_Id == ff.ncaC531SPCA_Id) {
                                govtfiles.push({ filename: ff.ncaC531SPCAF_FileName, filepath: ff.ncaC531SPCAF_FilePath, filedesc: ff.ncaC531SPCAF_Filedesc})
                            }
                        })
                        $scope.finalarray.push({ YEAR: gg.asmaY_Year, STDNAME: gg.amcsT_FirstName, flag1: gg.ncaC531SPCAS_NatOrInterNatFlg, flag2: gg.ncaC531SPCAS_SportsCAIEEEFlg, award: gg.awardname, adhar: gg.amcsT_AadharNo, regno: gg.amcsT_RegistrationNo, FILES: govtfiles})
                    })
