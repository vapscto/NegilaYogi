...
(function () {
    ...
    angular
        .module('app')
        .controller('HSUDevPrograms633', HSUDevPrograms633)

    HSUDevPrograms633.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function HSUDevPrograms633($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.Report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("Naac_HSU_CR6Report/HSUDevPrograms633Report", data).then(function (promise) {
                    ...
                    if (promise.govtsclist.length > 0 && promise.govtsclist != null) {
                        ...
                        angular.forEach($scope.govtsclist, function (gg) {
                            ...
                            angular.forEach($scope.govtsclistfiles, function (ff) {
                                if (gg.filefkid1 == ff.ncaC634DEVPRG_Id) {
                                    govtfiles.push({ filename: ff.ncaC634DEVPRGF_FileName, filepath: ff.ncaC634DEVPRGF_FilePath, filedesc: ff.ncaC634DEVPRGF_Filedesc })
                                }
                            })
                            $scope.finalarray.push({ YEAR: gg.ASMAY_Year, NCAC634DEVPRG_PDProgTitle: gg.NCAC634DEVPRG_PDProgTitle, datefromtocon: gg.datefromtocon, NCAC634DEVPRG_NoOfTeachersAttnd: gg.NCAC634DEVPRG_NoOfTeachersAttnd, NCAC633ADMTRG_Title: gg.NCAC633ADMTRG_Title, datefromtoad: gg.datefromtoad, NCAC633ADMTRG_NoOfParticipants: gg.NCAC633ADMTRG_NoOfParticipants, FILES: govtfiles })
                        })
