(function () {
    ...
    angular
        .module('app')
        .controller('HSU511Controller', HSU511Controller)

    HSU511Controller.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout']
    function HSU511Controller($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout) {
        ...
        $scope.exportToExcel = function (tableId) {
            var exportHref = Excel.tableToExcel(tableId, '5.1.1');
        ...
        $scope.showdetails = function () {
            ...
            var data = {
                ...
                apiService.create("NAACCriteriaFiveReport/HSU511", data).then(function (promise) {

                $scope.govtsclist = promise.govtsclist;
                $scope.instsclist = promise.instsclist;
                $scope.nngovtsclist = promise.nngovtsclist;
                $scope.govtsclistfiles = promise.govtsclistfiles;
                $scope.instsclistfiles = promise.instsclistfiles;
                $scope.nngovtsclistfiles = promise.nngovtsclistfiles;
                if ($scope.govtsclist.length > 0 || $scope.instsclist.length>0) {
                    ...
                    angular.forEach($scope.nngovtsclist, function (gg) {
                        var govtfiles = [];

                        angular.forEach($scope.nngovtsclistfiles, function (ff) {
                            if (gg.ncaC512NGSCH_Id == ff.ncaC512NGSCH_Id) {
                                govtfiles.push({ filename: ff.ncaC512NGSCHF_FileName, filepath: ff.ncaC512NGSCHF_FilePath, filedesc: ff.ncaC512NGSCHF_Filedesc })
                            }
                        })
                        $scope.mainarray.push({ asmaY_Year: gg.asmaY_Year, aasmay_id: gg.ncaC511GSCH_Year, schname: gg.ncaC512NGSCH_SchemeName, ngcount: gg.ncaC512NGSCH_NoOfStudents, incount: 0, filelist: govtfiles })
                    })
                    ...
                    angular.forEach($scope.govtsclist, function (gg) {
                        ...
                        $scope.mainarray.push({ asmaY_Year: gg.asmaY_Year, aasmay_id: gg.ncaC512NGSCH_Year, schname: gg.ncaC511GSCH_SchemeName, gcount: gg.ncaC511GSCH_NoOfStudents, incount: 0, filelist: govtfiles})
                    })
                    //////INST/////
                    ...
                    angular.forEach($scope.selectedYear, function (yy) {
                        ...
                        angular.forEach($scope.mainarray, function (tt) {
                            if (yy.asmaY_Year == tt.asmaY_Year) {
                                $scope.tfinalarray.push({ YEAR: yy.asmaY_Year, SCHEME: tt.schname, GCNT: tt.gcount, NGCNT: tt.ngcount, ICNT: tt.incount, FILES: tt.filelist})
                            }
