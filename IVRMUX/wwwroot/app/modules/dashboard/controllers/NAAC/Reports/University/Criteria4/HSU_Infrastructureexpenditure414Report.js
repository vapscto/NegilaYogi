...
(function () {
    ...
    angular
        .module('app')
        .controller('HSU_Infrastructureexpenditure414Report', HSU_Infrastructureexpenditure414Report)

    HSU_Infrastructureexpenditure414Report.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function HSU_Infrastructureexpenditure414Report($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.Report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("Naac_HSU_CR4Report/HSU_Infrastructureexpenditure414Report", data).then(function (promise) {
                    ...
                    if (promise.govtsclist.length > 0 && promise.govtsclist != null) {
                        ...
                        angular.forEach($scope.govtsclist, function (gg) {
                            ...
                            angular.forEach($scope.govtsclistfiles, function (ff) {
                                if (gg.NCAC414BD_Id == ff.ncaC414BD_Id) {
                                    govtfiles.push({ filename: ff.ncaC414BDF_FileName, filepath: ff.ncaC414BDF_FilePath, filedesc: ff.ncaC414BDF_Filedesc })
                                }
                            })
                            $scope.finalarray.push({ YEAR: gg.ASMAY_Year, sum: gg.sum, NCAC414BD_TotalExpExcludeSal: gg.NCAC414BD_TotalExpExcludeSal, FILES: govtfiles })
                        })
