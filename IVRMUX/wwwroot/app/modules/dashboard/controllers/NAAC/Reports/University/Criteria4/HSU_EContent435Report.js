...
(function () {
    ...
    angular
        .module('app')
        .controller('HSU_EContent435Report', HSU_EContent435Report)

    HSU_EContent435Report.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function HSU_EContent435Report($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.Report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("Naac_HSU_CR4Report/HSUEcontent435Report", data).then(function (promise) {
                    ...
                    if (promise.reportlist.length > 0) {
                        angular.forEach($scope.alldata4, function (tt) {
                            ...
                            angular.forEach($scope.alldata42, function (ss) {
                                if (tt.NCMCMEC436_Id == ss.ncmcmeC436_Id) {
