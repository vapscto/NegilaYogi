(function () {
    'use strict';
    angular.module('app').controller('HSU_EvaluationRelated253', HSU_EvaluationRelated253)

    HSU_EvaluationRelated253.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout']
    function HSU_EvaluationRelated253($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout) {
        ...
        $scope.exportToExcel = function (table) {
            ...
            var excelname = 'Cat 2.5.3.xls';
            var exportHref = Excel.tableToExcel(table, '2.5.3');
            $timeout(function () {
        ...
        $scope.get_report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("HSU_CR2_Report/HSU_253_Report", data).then(function (promise) {
                    debugger;
                    ...
                    if (promise.reportlist.length > 0) {
                        angular.forEach($scope.reportlist, function (tt) {
                            ...
                            angular.forEach($scope.reportlist2, function (ss) {
                                if (tt.NCHSU253ER_Id === ss.nchsU253ER_Id) {
