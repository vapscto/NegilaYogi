...
(function () {
    ...
    angular
        .module('app')
        .controller('StudentExposed423', StudentExposed423)

    StudentExposed423.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function StudentExposed423($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.loaddata = function () {
            var pageid = 2;
            apiService.getURI("Naac_MC_CR4/loaddata", pageid).then(function (promise) {
        ...
        $scope.Report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("Naac_MC_CR4/MEDStudentExposed423Report", data).then(function (promise) {
                    if (promise.reportlist.length > 0) {
                        ...
                        angular.forEach($scope.yearlist, function (yy) {
                            var temparr = [];
                            angular.forEach($scope.alldata4, function (dd) {
                                if (yy.asmaY_Year == dd.ASMAY_Year) {
                                    temparr.push(dd);
                                }
                            })
                            if (temparr.length > 0) {
