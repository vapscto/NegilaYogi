...
(function () {
    ...
    angular
        .module('app')
        .controller('HSUFinancialSupport632', HSUFinancialSupport632)

    HSUFinancialSupport632.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function HSUFinancialSupport632($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.Report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("Naac_HSU_CR6Report/HSUFinancialSupport632Report", data).then(function (promise) {
                    ...
                    if (promise.govtsclist.length > 0 && promise.govtsclist != null) {
                        ...
                        angular.forEach($scope.govtsclist, function (gg) {
                            ...
                            angular.forEach($scope.govtsclistfiles, function (ff) {
                                if (gg.filefkid == ff.ncaC632FINSUP_Id) {
                                    govtfiles.push({ filename: ff.ncaC632FINSUPF_FileName, filepath: ff.ncaC632FINSUPF_FilePath, filedesc: ff.ncaC632FINSUPF_Filedesc })
                                }
                            })
                            $scope.finalarray.push({ YEAR: gg.ASMAY_Year, NCAC632FINSUP_TeacherName: gg.NCAC632FINSUP_TeacherName, NCAC632FINSUP_Name: gg.NCAC632FINSUP_Name, NCAC632FINSUP_AmountPaid: gg.NCAC632FINSUP_AmountPaid, NCAC632FINSUP_NameOfMembership: gg.NCAC632FINSUP_NameOfMembership, FILES: govtfiles })
                        })
