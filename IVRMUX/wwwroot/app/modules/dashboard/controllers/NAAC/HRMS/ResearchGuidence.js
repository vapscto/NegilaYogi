(function () {
    ...
    angular
        .module('app')
        .controller('naacResearchGuidenceController', naacResearchGuidenceController);

    naacResearchGuidenceController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$stateParams', '$filter', 'superCache', '$q', '$sce'];
    function naacResearchGuidenceController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $stateParams, $filter, superCache, $q, $sce) {
            ...
            apiService.create("naacHrmsDetails/get_EmployeALLDATA", data).
                then(function (promise) {
                    if (promise.researchGuidelist !== null && promise.researchGuidelist.length > 0) {
                        ...
                        $scope.gridOptions.data = $scope.documentListResGuidance;
                    }
                    else {
       ...
       //RESEARCH GUIDANCE
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                { name: 'hreregU_ThesisTitle', displayName: 'Thesis Title', enableHiding: false },
                { name: 'hreregU_Course', displayName: 'Course', enableHiding: false },
                { name: 'hreregU_ThesisStatus', displayName: 'Thesis Status', enableHiding: false },
                { name: 'hreregU_StudentName', displayName: 'Student Name', enableHiding: false },
                { name: 'hreregU_NameOftheGudie', displayName: 'Name Of the Gudie', enableHiding: false },
                { name: 'hreregU_Year', displayName: 'Year', enableHiding: false },
                {
                    field: 'id', name: '',
                    displayName: 'Actions', enableColumnMenu: false, enableFiltering: false, enableSorting: false, cellTemplate:
                        ...
                        '<a ng-if="row.entity.hreregU_ActiveFlg === false" href="javascript:void(0)" style="color:green;" ng-click="grid.appScope.DeletRecord(row.entity);" data-toggle="tooltip" title="Activate"> Activate</a>' +
                        '<span ng-if="row.entity.hreregU_ActiveFlg === true ""><a href="javascript:void(0)"  style="color:red;" ng-click="grid.appScope.DeletRecord(row.entity);" data-toggle="tooltip" title="Deactivate">  Deactivate</a><span>' +
                        '</div>'
                }
            ],
        ...
        $scope.EditData = function (record) {
            var data = {
                "HREREGU_Id": record.hreregU_Id,
                "TabName": "ResearchGuidanceTab"
            };
            apiService.create("naacHrmsDetails/editRecord", data).
                then(function (promise) {
                    if (promise.researchGuidelistedit != null && promise.researchGuidelistedit.length > 0) {
                        $scope.researchguid = promise.researchGuidelistedit[0];
                        if ($scope.researchguid.hreregU_Document !== null && $scope.researchguid.hreregU_Document !== undefined) {
                            var filename = $scope.researchguid.hreregU_Document.toString();
                            var nameArray = filename.split('.');
                            $scope.researchguid.extention = nameArray[nameArray.length - 1];
                        }
                    }
                });
        };
        ...
        $scope.validateResGuidanceDetails = function () {
            if ($scope.myForm6.$valid) {
                $scope.documentListResGuidance = [];
                $scope.documentListResGuidance.push($scope.researchguid);
                var duplicateda = false;
                if ($scope.researchguid.hreregU_Id == null || $scope.researchguid.hreregU_Id == undefined) {
