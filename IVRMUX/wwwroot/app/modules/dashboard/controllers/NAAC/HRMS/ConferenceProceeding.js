(function () {
    ...
    angular
        .module('app')
        .controller('naacConferenceProceedingController', naacConferenceProceedingController);

    naacConferenceProceedingController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$stateParams', '$filter', 'superCache', '$q', '$sce'];
    function naacConferenceProceedingController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $stateParams, $filter, superCache, $q, $sce) {
            ...
            apiService.create("naacHrmsDetails/get_EmployeALLDATA", data).
                then(function (promise) {
                    ...
                    if (promise.conferencelist !== null && promise.conferencelist.length > 0) {
                        ...
                        $scope.gridOptions.data = $scope.documentListConference;
                    }
                    else {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                { name: 'hreconF_PaperTitle', displayName: 'Paper Title', enableHiding: false },
                { name: 'hreconF_ConferenceName', displayName: 'Conference Name', enableHiding: false },
                { name: 'hreconF_ConferenceVenue', displayName: 'Venue', enableHiding: false },
                { name: 'hreconF_Duration', displayName: 'Duration', enableHiding: false },
                { name: 'hreconF_Authors', displayName: 'Authors', enableHiding: false },
                { name: 'hreconF_PublisherName', displayName: 'PublisherName', enableHiding: false },
                { name: 'hreconF_Year', displayName: 'Year', enableHiding: false },
                {
                    field: 'id', name: '',
                    displayName: 'Actions', enableColumnMenu: false, enableFiltering: false, enableSorting: false, cellTemplate:
                        ...
                        '<a ng-if="row.entity.hreconF_ActiveFlg === false" href="javascript:void(0)" style="color:green;" ng-click="grid.appScope.DeletRecord(row.entity);" data-toggle="tooltip" title="Activate"> Activate</a>' +
                        '<span ng-if="row.entity.hreconF_ActiveFlg === true ""><a href="javascript:void(0)"  style="color:red;" ng-click="grid.appScope.DeletRecord(row.entity);" data-toggle="tooltip" title="Deactivate">  Deactivate</a><span>' +
                        '</div>'
                }
            ],
        ...
        $scope.EditData = function (record) {
            var data = {
                "HRECONF_Id": record.hreconF_Id,
                "TabName": "ConferenceTab"
            };
            apiService.create("naacHrmsDetails/editRecord", data).
                then(function (promise) {
                    if (promise.conferencelistedit != null && promise.conferencelistedit.length > 0) {
                        $scope.conference = promise.conferencelistedit[0];
                        angular.forEach(promise.conferencelistedit, function (value, key) {
                            if (value.hreconF_Fromdate !== null) {
                                $scope.conference.hreconF_Fromdate = new Date(value.hreconF_Fromdate);
                            }
                            else {
                                $scope.conference.hreconF_Fromdate = null;
                            }

                            if (value.hreconF_Todate !== null) {
                                $scope.conference.hreconF_Todate = new Date(value.hreconF_Todate);
                            }
                            else {
                                $scope.conference.hreconF_Todate = null;
                            }

                            if ($scope.conference.hreconF_Document !== null && $scope.conference.hreconF_Document !== undefined) {
                                var filename = $scope.conference.hreconF_Document.toString();
                                var nameArray = filename.split('.');
                                $scope.conference.extention = nameArray[nameArray.length - 1];
                            }
                        });
                    }
                });
        ...
        $scope.validateConferenceDetails = function () {
            if ($scope.myForm9.$valid) {
                $scope.documentListConference = [];
                $scope.documentListConference.push($scope.conference);
                var duplicateda = false;
                if ($scope.conference.hreconF_Id == null || $scope.conference.hreconF_Id == undefined) {
