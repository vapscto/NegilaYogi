(function () {
    ...
    angular
        .module('app')
        .controller('naacBookPublishedController', naacBookPublishedController);

    naacBookPublishedController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$stateParams', '$filter', 'superCache', '$q', '$sce'];
    function naacBookPublishedController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $stateParams, $filter, superCache, $q, $sce) {
            ...
            apiService.create("naacHrmsDetails/get_EmployeALLDATA", data).
                then(function (promise) {
                    ...
                    if (promise.booklist !== null && promise.booklist.length > 0) {
                        ...
                        $scope.gridOptions.data = $scope.documentListBook;
                    }
                    else {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                { name: 'hrebK_BookName', displayName: 'Book Name', enableHiding: false },
                { name: 'hrebK_Author', displayName: 'Author', enableHiding: false },
                { name: 'hrebK_Title', displayName: 'Title', enableHiding: false },
                { name: 'hrebK_PublisherName', displayName: 'Publisher Name', enableHiding: false },
                { name: 'hrebK_DocumentPath', displayName: 'Document Path', enableHiding: false },
                { name: 'hrebK_Year', displayName: 'Year', enableHiding: false },
                {
                    field: 'id', name: '',
                    displayName: 'Actions', enableColumnMenu: false, enableFiltering: false, enableSorting: false, cellTemplate:
                        ...
                        '<a ng-if="row.entity.hrebK_ActiveFlg === false" href="javascript:void(0)" style="color:green;" ng-click="grid.appScope.DeletRecord(row.entity);" data-toggle="tooltip" title="Activate"> Activate</a>' +
                        '<span ng-if="row.entity.hrebK_ActiveFlg === true ""><a href="javascript:void(0)"  style="color:red;" ng-click="grid.appScope.DeletRecord(row.entity);" data-toggle="tooltip" title="Deactivate">  Deactivate</a><span>' +
                        '</div>'
                }
            ],
        ...
        $scope.EditData = function (record) {
            var data = {
                "HREBK_Id": record.hrebK_Id,
                "TabName": "BookTab"
            };
            apiService.create("naacHrmsDetails/editRecord", data).
                then(function (promise) {
                    if (promise.booklistedit != null && promise.booklistedit.length > 0) {
                        $scope.book = promise.booklistedit[0];
                        if ($scope.book.hrebK_Document !== null && $scope.book.hrebK_Document !== undefined) {
                            var filename = $scope.book.hrebK_Document.toString();
                            var nameArray = filename.split('.');
                            $scope.book.extention = nameArray[nameArray.length - 1];
                        }
                    }
                });
        };
        ...
        $scope.validateBookDetails = function () {
            if ($scope.myForm10.$valid) {
                $scope.documentListBook = [];
                $scope.documentListBook.push($scope.book);
                var duplicateda = false;
                if ($scope.book.hrebK_Id == null || $scope.book.hrebK_Id == undefined) {
