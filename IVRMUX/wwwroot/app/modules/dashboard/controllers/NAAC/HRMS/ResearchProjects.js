(function () {
    ...
    angular
        .module('app')
        .controller('naacResearchProjectController', naacResearchProjectController);

    naacResearchProjectController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$stateParams', '$filter', 'superCache', '$q', '$sce'];
    function naacResearchProjectController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $stateParams, $filter, superCache, $q, $sce) {
            ...
            apiService.create("naacHrmsDetails/get_EmployeALLDATA", data).
                then(function (promise) {
                    if (promise.researchProjectlist !== null && promise.researchProjectlist.length > 0) {
                        ...
                        $scope.gridOptions.data = $scope.documentListResProject;
                    }
                    else {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                { name: 'hrerepR_ProjectTitle', displayName: 'Project Title', enableHiding: false },
                { name: 'hrerepR_FundsReceived', displayName: 'Funds Received', enableHiding: false },
                { name: 'hrerepR_FundingAgency', displayName: 'Funding Agency', enableHiding: false },
                { name: 'hrerepR_Period', displayName: 'Time Period', enableHiding: false },
                { name: 'hrerepR_Role', displayName: 'Role', enableHiding: false },
                { name: 'hrerepR_ProjectStatus', displayName: 'Project Status', enableHiding: false },
                { name: 'hrerepR_Year', displayName: 'Year', enableHiding: false },
                {
                    field: 'id', name: '',
        ...
        $scope.EditData = function (record) {
            var data = {
                "HREREPR_Id": record.hrerepR_Id,
                "TabName": "ResearchProjectTab"
            };
            apiService.create("naacHrmsDetails/editRecord", data).
                then(function (promise) {
                    if (promise.researchProjectlistedit != null && promise.researchProjectlistedit.length > 0) {
                        $scope.researchproj = promise.researchProjectlistedit[0];
                        if ($scope.researchproj.hrerepR_Document !== null && $scope.researchproj.hrerepR_Document !== undefined) {
                            var filename = $scope.researchproj.hrerepR_Document.toString();
                            var nameArray = filename.split('.');
                            $scope.researchproj.extention = nameArray[nameArray.length - 1];
                        }
                    }
                });
        };
        ...
        $scope.validateResProjectDetails = function () {
            if ($scope.myForm5.$valid) {
                $scope.documentListResProject = [];
                $scope.documentListResProject.push($scope.researchproj);
                var duplicateda = false;
                if ($scope.researchproj.hrerepR_Id == null || $scope.researchproj.hrerepR_Id == undefined) {
