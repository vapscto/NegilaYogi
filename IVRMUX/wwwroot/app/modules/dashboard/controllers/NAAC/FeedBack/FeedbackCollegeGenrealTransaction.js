(function () {
    ...
    angular
        .module('app')
        .controller('FeedbackCollegeParentTransactionController', FeedbackCollegeParentTransactionController)

    FeedbackCollegeParentTransactionController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$filter']
    function FeedbackCollegeParentTransactionController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $filter) {
        ...
        $scope.BindData = function () {
            ...
            apiService.create("FeedbackTransaction/getdetails", data).then(function (promise) {

                if (promise.flag !== "Staff" && promise.flag !== "Alumni") {
                    if (promise.getstudentdetails === null || promise.getstudentdetails === undefined || promise.getstudentdetails.length === 0) {
        ...
        $scope.getfeedback = function () {
            ...
            apiService.create("FeedbackTransaction/getfeedback", data).then(function (promise) {
                if (promise !== null) {
                    $scope.count = promise.count;
