...
(function () {
    ...
    angular
        .module('app')
        .controller('NAACNonGovShcrshipHsuController', NAACNonGovShcrshipHsuController);

    NAACNonGovShcrshipHsuController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce','myFactorynaac'];

    function NAACNonGovShcrshipHsuController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        ...
        $scope.loaddata = function () {
            ...
            apiService.getURI("NAACNonGovShcrshipHsu/loaddata", $scope.mI_Id).then(function (promise) {
        ...
        $scope.getorganizationdata = function (obj) {
            apiService.create("NAACNonGovShcrshipHsu/getcomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.getorganizationdata1 = function (obj) {
            debugger;
            obj.ncaC512NGSCHF_Id = obj.cfileid;
            apiService.create("NAACNonGovShcrshipHsu/getfilecomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.addcomments = function (obje) {
            ...
            $scope.ccc = obje.ncaC512NGSCH_Id;
            $scope.generalcomments = '';
            $scope.obj.generalcomments = $scope.generalcomments;
            var bb;
        };
        ...
        $scope.savedatawisecomments = function (obj) {
            ...
            if ($scope.myFormc.$valid) {
                ...
                apiService.create("NAACNonGovShcrshipHsu/savemedicaldatawisecomments", data).then(function (promise) {
                    if (promise !== null) {
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            if ($scope.myFormc1.$valid) {
                ...
                apiService.create("NAACNonGovShcrshipHsu/savefilewisecomments", data).then(function (promise) {
                    if (promise !== null) {
                        if (promise.returnval === true) {
                            swal("Comments Saved Successfully");
                        } else {
        ...
        $scope.savedatatab1 = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                var data = {
                    "MI_Id": $scope.mI_Id,
                        "NCAC512NGSCH_Id": $scope.NCAC512NGSCH_Id,
                        "NCAC512NGSCH_Year": $scope.ASMAY_Id,
                        "NCAC512NGSCH_SchemeName": $scope.NCAC512NGSCH_SchemeName,
                        "NCAC512NGSCH_NoOfStudents": $scope.NCAC512NGSCH_NoOfStudents,
                        filelist: $scope.materaldocuupload,
                    }
               
            
                apiService.create("NAACNonGovShcrshipHsu/save", data).then(function (promise) {

                    if (promise.duplicate != null && promise.returnval != null) {
                        if (promise.duplicate == false) {
                            if (promise.returnval == true) {
                                if ($scope.NCAC512NGSCH_Id > 0) {
        ...
        $scope.edittab1 = function (data) {
            debugger;
            apiService.create("NAACNonGovShcrshipHsu/EditData", data).then(function (promise) {
                $scope.materaldocuupload = [{ id: 'Materal1' }];
                $scope.editlist = promise.editlist;
                $scope.NCAC512NGSCH_Id = promise.editlist[0].ncaC512NGSCH_Id;
                $scope.NCAC512NGSCH_SchemeName = promise.editlist[0].ncaC512NGSCH_SchemeName;
                $scope.ASMAY_Id = promise.editlist[0].ncaC512NGSCH_Year;
                $scope.NCAC512NGSCH_NoOfStudents = promise.editlist[0].ncaC512NGSCH_NoOfStudents;
        ...
        $scope.clrr = function () {
            $scope.NCAC512NGSCH_NoOfStudents = '';
            $scope.NCAC512NGSCH_SchemeName = '';
            $scope.NCAC512NGSCH_Id = 0;
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            $scope.NCAC512NGSCH_Id = usersem.ncaC512NGSCH_Id
            var dystring = "";
            if (usersem.ncaC512NGSCH_ActiveFlg == true) {
                dystring = "Deactivated";
            }
            else if (usersem.ncaC512NGSCH_ActiveFlg == false) {
                dystring = "Activated";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACNonGovShcrshipHsu/deactiveStudent", usersem).
        ...
        $scope.viewdocument = function (obj) {
            ...
            apiService.create("NAACNonGovShcrshipHsu/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
                    debugger;
        ...
        $scope.deleteuploadfile = function (obj) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACNonGovShcrshipHsu/deleteuploadfile", data).then(function (promise) {
