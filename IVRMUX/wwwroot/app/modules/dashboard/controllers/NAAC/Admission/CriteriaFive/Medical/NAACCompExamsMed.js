...
(function () {
    ...
    angular
        .module('app')
        .controller('NAACCompExamsMedController', NAACCompExamsMedController);

   NAACCompExamsMedController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce', 'myFactorynaac'];

    function NAACCompExamsMedController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        ...
        $scope.clrr = function () {
            $scope.NCAC514CPEX_ExamSchemeName = '';
            $scope.NCAC514CPEX_NoOfStudents = '';
            
            $scope.NCAC514CPEX_Id = 0;
        ...
        $scope.loaddata = function () {
            ...
            apiService.getURI("NAACCompExams/loaddata", $scope.mI_Id).then(function (promise) {
        ...
        $scope.savedatatab1 = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                var data = {
                    "MI_Id": $scope.mI_Id,
                        "NCAC514CPEX_Id": $scope.NCAC514CPEX_Id,
                        "NCAC514CPEX_ImpYear": $scope.ASMAY_Id,
                        "NCAC514CPEX_ExamSchemeName": $scope.NCAC514CPEX_ExamSchemeName,
                        "NCAC514CPEX_NoOfStudents": $scope.NCAC514CPEX_NoOfStudents,
                        filelist: $scope.materaldocuupload,
                       
                    }
               

                apiService.create("NAACCompExams/save", data).then(function (promise) {

                    if (promise.duplicate != null && promise.returnval != null) {
                        if (promise.duplicate == false) {
                            if (promise.returnval == true) {
                                if ($scope.NCAC514CPEX_Id > 0) {
        ...
        $scope.edittab1 = function (data) {
            debugger;
            apiService.create("NAACCompExams/EditData", data).then(function (promise) {

                $scope.editlist = promise.editlist;
                $scope.NCAC514CPEX_Id = promise.editlist[0].ncaC514CPEX_Id;
                $scope.NCAC514CPEX_ExamSchemeName = promise.editlist[0].ncaC514CPEX_ExamSchemeName;
                $scope.ASMAY_Id = promise.editlist[0].ncaC514CPEX_ImpYear;
                $scope.NCAC514CPEX_NoOfStudents = promise.editlist[0].ncaC514CPEX_NoOfStudents;
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            $scope.NCAC514CPEX_Id = usersem.ncaC514CPEX_Id
            var dystring = "";
            if (usersem.ncaC514CPEX_ActiveFlg == true) {
                dystring = "Deactivated";
            }
            else if (usersem.ncaC514CPEX_ActiveFlg == false) {
                dystring = "Activated";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACCompExams/deactiveStudent", usersem).
        ...
        $scope.getorganizationdata = function (obj) {
            apiService.create("NAACCompExams/getcomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.getorganizationdata1 = function (obj) {
            debugger;
            obj.ncaC514CPEXF_Id = obj.cfileid;
            apiService.create("NAACCompExams/getfilecomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.savedatawisecomments = function (obj) {
            ...
            if ($scope.myFormc.$valid) {
                ...
                apiService.create("NAACCompExams/savemedicaldatawisecomments", data).then(function (promise) {
                    if (promise !== null) {
                        if (promise.returnval === true) {
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            if ($scope.myFormc1.$valid) {
                ...
                apiService.create("NAACCompExams/savefilewisecomments", data).then(function (promise) {
                    if (promise !== null) {
                        if (promise.returnval === true) {
                            swal("Comments Saved Successfully");
                        } else {
        ...
        $scope.viewdocument = function (obj) {
            ...
            apiService.create("NAACCompExams/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
                    debugger;
        ...
        $scope.deleteuploadfile = function (obj) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACCompExams/deleteuploadfile", data).then(function (promise) {
