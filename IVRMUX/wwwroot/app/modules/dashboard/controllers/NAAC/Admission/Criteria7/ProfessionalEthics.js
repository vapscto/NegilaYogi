...
(function () {
    'use strict';
    angular.module('app').controller('ProfessionalEthicsController', ProfessionalEthicsController);

    ProfessionalEthicsController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce', 'myFactorynaac'];

    function ProfessionalEthicsController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        $scope.mI_Id = 0;
        $scope.equityid = 0;
        $scope.currentPage = 1;
        ...
        $scope.loaddata = function () {
            $scope.equityid = 0;
            var id = 2;
            apiService.getURI("ProfessionalEthics/loaddata", id).then(function (promise) {
        ...
        $scope.getData = function (mI_Id) {
            $scope.equityid = 0;
            var id = mI_Id;
            apiService.getURI("ProfessionalEthics/getData", id).then(function (promise) {
            ...
            if ($scope.myForm.$valid) {
                ...
                if ($scope.materaldocuupload.length > 0) {
                    angular.forEach($scope.materaldocuupload, function (val) {
                        if (val.ncaC7115PROETHF_FilePath !== null && val.ncaC7115PROETHF_FilePath !== undefined) {
                ...
                var data = {
                    ...
                    "NCAC7115PROETH_Id": $scope.equityid,
                    "NCAC7115PROETH_Year": $scope.page.ncaC7115PROETH_Year,
                    "NCAC7115PROETH_URL": $scope.page.ncaC7115PROETH_URL,
                    "NAACAC7115DTO": $scope.materaldocuupload22
                };
                apiService.create("ProfessionalEthics/save", data).then(function (promise) {
        ...
        $scope.edittab1 = function (data) {
            $scope.instit = true;
            apiService.create("ProfessionalEthics/EditData", data).then(function (promise) {
                $scope.equityid = data.ncaC7115PROETH_Id;
                ...
                if (promise.editfilelist != null && promise.editfilelist.length > 0) {
                    ...
                    angular.forEach($scope.materaldocuupload, function (ddd) {
                        var img = ddd.ncaC7115PROETHF_FilePath;
                        var imagarr = img.split('.');
                        var lastelement = imagarr[imagarr.length - 1];
                        ddd.filetype = lastelement;
                        if (lastelement === 'doc' || lastelement === 'docx' || lastelement === 'ppt' || lastelement === 'pptx' || lastelement === 'xlsx' || lastelement === 'xls') {
                            ddd.document_Pathnew = "https://view.officeapps.live.com/op/view.aspx?src=" + ddd.ncaC7115PROETHF_FilePath;
                        }
                    });
                }
            });
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            ...
            var dystring = "";
            if (usersem.ncaC7115PROETH_ActiveFlg == true) {
                dystring = "Deactivated";
            }
            else if (usersem.ncaC7115PROETH_ActiveFlg == false) {
                dystring = "Activated";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("ProfessionalEthics/deactivate", usersem).
        ...
        $scope.getorganizationdata = function (obj) {
            apiService.create("ProfessionalEthics/getfilecomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.getorganizationdata1 = function (obj) {
            apiService.create("ProfessionalEthics/getcomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.addcomments = function (obje) {
            ...
            $scope.cc = obje.ncaC7115PROETHF_Id;
        ...
        $scope.addcomments1 = function (obje) {
            ...
            $scope.ccc = obje.ncaC7115PROETH_Id;
            $scope.generalcomments = '';
            $scope.obj.generalcomments = $scope.generalcomments;
            var bb;
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            apiService.create("ProfessionalEthics/savemedicaldatawisecomments", data).then(function (promise) {
                if (promise !== null) {
                    if (promise.returnval === true) {
        ...
        $scope.savedatawisecomments = function (obj) {
            ...
            apiService.create("ProfessionalEthics/savefilewisecomments", data).then(function (promise) {
                if (promise !== null) {
                    if (promise.returnval === true) {
                        swal("Comments Saved Successfully");
                        $scope.page = $scope.page1;
        ...
        function UploaddianmateralPhoto(data) {
            ...
            $http.post("/api/ImageUpload/Uploadnaacdocuments", formData,
                ...
                .success(function (d) {
                    defer.resolve(d);
                    data.ncaC7115PROETHF_FilePath = d;
                    data.ncaC7115PROETHF_FileName = $scope.filename;
                    $('#').attr('src', data.ncaC7115PROETHF_FilePath);
                    var img = data.ncaC7115PROETHF_FilePath;
                    var imagarr = img.split('.');
                    var lastelement = imagarr[imagarr.length - 1];
                    data.filetype = lastelement;
                    if (lastelement === 'doc' || lastelement === 'docx' || lastelement === 'ppt' || lastelement === 'pptx' || lastelement === 'xlsx' || lastelement === 'xls') {
                        data.document_Pathnew = "https://view.officeapps.live.com/op/view.aspx?src=" + data.ncaC7115PROETHF_FilePath;
                    }
                })
                .error(function () {
        ...
        $scope.showGuardianPhotonew = function (data) {
            $scope.view_videos = [];
            $scope.videoSources = [];
            $scope.preview1 = data.ncaC7115PROETHF_FilePath;
            $scope.videdfd = data.ncaC7115PROETHF_FilePath;
            $scope.movie = { src: data.ncaC7115PROETHF_FilePath };
            $scope.movie1 = { src: data.ncaC7115PROETHF_FilePath };
            $scope.view_videos.push({ id: 1, coeeV_Videos: data.ncaC7115PROETHF_FilePath });
        ...
        $scope.viewdocument = function (obj) {
            ...
            apiService.create("ProfessionalEthics/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
                    ...
                    if (promise.editfiles !== null && promise.editfiles.length > 0) {
                        ...
                        angular.forEach($scope.uploaddocfiles, function (dd) {
                            var img = dd.ncaC7115PROETHF_FilePath;
                            var imagarr = img.split('.');
                            var lastelement = imagarr[imagarr.length - 1];
                            dd.filetype = lastelement;
                            if (lastelement === 'doc' || lastelement === 'docx' || lastelement === 'ppt' || lastelement === 'pptx' || lastelement === 'xlsx' || lastelement === 'xls') {
                                dd.document_Pathnew = "https://view.officeapps.live.com/op/view.aspx?src=" + dd.ncaC7115PROETHF_FilePath;
                            }
                        });
                    } else {
                        $('#popup11').modal('hide');
        ...
        $scope.deleteuploadfile = function (index, obj) {
            ...
            if (obj.ncaC7115PROETHF_Id == undefined) {
                $scope.materaldocuupload.splice(index, 1);
                swal("Record Deleted successfully");
            }
            else {
                var data = {
                    "NCAC7115PROETHF_Id": obj.ncaC7115PROETHF_Id
                };
                swal({
                    title: "Are You Sure",
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            apiService.create("ProfessionalEthics/deleteuploadfile", data).then(function (promise) {
                                if (promise.returnval === true) {
                                    swal("Record Deleted successfully");
                                    var data = { "ncaC7115PROETH_Id": $scope.equityid };
