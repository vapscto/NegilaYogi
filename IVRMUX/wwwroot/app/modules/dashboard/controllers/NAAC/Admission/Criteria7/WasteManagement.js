...
(function () {
    'use strict';
    angular.module('app').controller('WasteManagementController', WasteManagementController);

    WasteManagementController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce', 'myFactorynaac'];

    function WasteManagementController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        $scope.mI_Id = 0;
        $scope.equityid = 0;
        $scope.currentPage = 1;
        ...
        $scope.loaddata = function () {
            $scope.equityid = 0;
            var id = 2;
            apiService.getURI("WasteManagement/loaddata", id).then(function (promise) {
        ...
        $scope.getData = function (mI_Id) {
            $scope.equityid = 0;
            var id = mI_Id;
            apiService.getURI("WasteManagement/getData", id).then(function (promise) {
        ...
        $scope.savedatatab1 = function () {
            if ($scope.myForm.$valid) {
                ...
                var data = {
                    ...
                    "NCAC718WAMAN_Id": $scope.equityid,
                    "NCAC718WAMAN_Year": $scope.page.ncaC718WAMAN_Year,
                    "NCAC718WAMAN_Expenditure": $scope.page.ncaC718WAMAN_Expenditure,
                    "NAACAC7DTO": $scope.materaldocuupload
                };
                apiService.create("WasteManagement/save", data).then(function (promise) {
        ...
        $scope.edittab1 = function (data) {
            debugger;
            $scope.instit = true;
            apiService.create("WasteManagement/EditData", data).then(function (promise) {
                debugger;
                $scope.equityid = data.ncaC718WAMAN_Id;
                ...
                if (promise.editfilelist != null && promise.editfilelist.length > 0) {
                    ...
                    angular.forEach($scope.materaldocuupload, function (ddd) {
                        var img = ddd.ncaC718WAMANF_FilePath;
                        var imagarr = img.split('.');
                        var lastelement = imagarr[imagarr.length - 1];
                        ddd.filetype = lastelement;
                        if (lastelement === 'doc' || lastelement === 'docx' || lastelement === 'ppt' || lastelement === 'pptx' || lastelement === 'xlsx' || lastelement === 'xls') {
                            ddd.document_Pathnew = "https://view.officeapps.live.com/op/view.aspx?src=" + ddd.ncaC718WAMANF_FilePath;
                        }
                    });
                }
            });
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            ...
            var dystring = "";
            if (usersem.ncaC718WAMAN_ActiveFlg == true) {
                dystring = "Deactivated";
            }
            else if (usersem.ncaC718WAMAN_ActiveFlg == false) {
                dystring = "Activated";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("WasteManagement/deactivate", usersem).
        ...
        $scope.getorganizationdata = function (obj) {
                ...
                debugger;
                apiService.create("WasteManagement/getfilecomment", obj).then(function (promise) {
                    if (promise !== null) {
        ...
        $scope.getorganizationdata1 = function (obj) {
                ...
                apiService.create("WasteManagement/getcomment", obj).then(function (promise) {
                    if (promise !== null) {
        ...
        $scope.addcomments = function (obje) {
                ...
                $scope.cc = obje.ncaC718WAMANF_Id;
        ...
        $scope.addcomments1 = function (obje) {
                ...
                $scope.ccc = obje.ncaC718WAMAN_Id;
                $scope.generalcomments = '';
                $scope.obj.generalcomments = $scope.generalcomments;
                var bb;
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            apiService.create("WasteManagement/savemedicaldatawisecomments", data).then(function (promise) {
                if (promise !== null) {
                    if (promise.returnval === true) {
        ...
        $scope.savedatawisecomments = function (obj) {
            ...
            apiService.create("WasteManagement/savefilewisecomments", data).then(function (promise) {
                if (promise !== null) {
                    if (promise.returnval === true) {
                        swal("Comments Saved Successfully");
                        $scope.page = $scope.page1;
        ...
        function UploaddianmateralPhoto(data) {
            ...
            $http.post("/api/ImageUpload/Uploadnaacdocuments", formData,
                ...
                .success(function (d) {
                    defer.resolve(d);
                    data.ncaC718WAMANF_FilePath = d;
                    data.ncaC718WAMANF_FileName = $scope.filename;
                    $('#').attr('src', data.ncaC718WAMANF_FilePath);
                    var img = data.ncaC718WAMANF_FilePath;
                    var imagarr = img.split('.');
                    var lastelement = imagarr[imagarr.length - 1];
                    data.filetype = lastelement;
                    if (lastelement === 'doc' || lastelement === 'docx' || lastelement === 'ppt' || lastelement === 'pptx' || lastelement === 'xlsx' || lastelement === 'xls') {
                        data.document_Pathnew = "https://view.officeapps.live.com/op/view.aspx?src=" + data.ncaC718WAMANF_FilePath;
                    }
                })
                .error(function () {
        ...
        $scope.showGuardianPhotonew = function (data) {
            $scope.view_videos = [];
            $scope.videoSources = [];
            $scope.preview1 = data.ncaC718WAMANF_FilePath;
            $scope.videdfd = data.ncaC718WAMANF_FilePath;
            $scope.movie = { src: data.ncaC718WAMANF_FilePath };
            $scope.movie1 = { src: data.ncaC718WAMANF_FilePath };
            $scope.view_videos.push({ id: 1, coeeV_Videos: data.ncaC718WAMANF_FilePath });
        ...
        $scope.viewdocument = function (obj) {
            ...
            apiService.create("WasteManagement/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
                    ...
                    if (promise.editfiles !== null && promise.editfiles.length > 0) {
                        ...
                        angular.forEach($scope.uploaddocfiles, function (dd) {
                            var img = dd.ncaC718WAMANF_FilePath;
                            var imagarr = img.split('.');
                            var lastelement = imagarr[imagarr.length - 1];
                            dd.filetype = lastelement;
                            if (lastelement === 'doc' || lastelement === 'docx' || lastelement === 'ppt' || lastelement === 'pptx' || lastelement === 'xlsx' || lastelement === 'xls') {
                                dd.document_Pathnew = "https://view.officeapps.live.com/op/view.aspx?src=" + dd.ncaC718WAMANF_FilePath;
                            }
                        });
                    } else {
                        $('#popup11').modal('hide');
        ...
        $scope.deleteuploadfile = function (index,obj) {
            if (obj.ncaC718WAMANF_Id == undefined) {
                $scope.materaldocuupload.splice(index, 1);
                swal("Record Deleted successfully");
            }
            else {
                var data = {
                    "NCAC718WAMANF_Id": obj.ncaC718WAMANF_Id
                };
                ...
                swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            apiService.create("WasteManagement/deleteuploadfile", data).then(function (promise) {
                                if (promise.returnval === true) {
                                    debugger;
                                    swal("Record Deleted successfully");
                                    var data = { "ncaC718WAMAN_Id": $scope.equityid };
                                    $scope.edittab1(data);

                                    //$scope.page = $scope.page1;

                                    //$scope.page.mI_Id = $scope.page[0].mI_Id;
                                    //$scope.page.ncaC718WAMAN_Year = $scope.page[0].ncaC718WAMAN_Year;
                                    //$scope.page.ncaC718WAMAN_Expenditure = $scope.page[0].ncaC718WAMAN_Expenditure;
                                }
                                else {
