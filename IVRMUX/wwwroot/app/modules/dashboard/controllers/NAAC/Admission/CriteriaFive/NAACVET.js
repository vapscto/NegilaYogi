...
(function () {
    ...
    angular
        .module('app')
        .controller('NAACVETController', NAACVETController);

    NAACVETController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce','myFactorynaac'];

    function NAACVETController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        ...
        $scope.clrr = function () {
            ...
            $scope.NCAC515VET_VETProgramName = '';
            $scope.NCAC515VET_NoOfStudents = '';
        ...
        $scope.loaddata = function () {
            ...
            apiService.getURI("NAACVET/loaddata", $scope.mI_Id).then(function (promise) {
        ...
        $scope.getorganizationdata = function (obj) {
            apiService.create("NAACVET/getcomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.getorganizationdata1 = function (obj) {
            debugger;
            obj.ncaC515VETF_Id = obj.cfileid;
            apiService.create("NAACVET/getfilecomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.savedatawisecomments = function (obj) {
            ...
            if ($scope.myFormc.$valid) {
                ...
                apiService.create("NAACVET/savemedicaldatawisecomments", data).then(function (promise) {
                    if (promise !== null) {
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            if ($scope.myFormc1.$valid) {
                ...
                apiService.create("NAACVET/savefilewisecomments", data).then(function (promise) {
                    if (promise !== null) {
                        if (promise.returnval === true) {
                            swal("Comments Saved Successfully");
                        } else {
        ...
        $scope.savedatatab1 = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                var data = {
                    "MI_Id": $scope.mI_Id,
                        "NCAC515VET_Id": $scope.NCAC515VET_Id,
                        "NCAC515VET_Year": $scope.ASMAY_Id,
                        "NCAC515VET_VETProgramName": $scope.NCAC515VET_VETProgramName,
                        "NCAC515VET_NoOfStudents": $scope.NCAC515VET_NoOfStudents,
                        filelist: $scope.materaldocuupload,
                       
                    }
               

                apiService.create("NAACVET/save", data).then(function (promise) {

                    if (promise.duplicate != null && promise.returnval != null) {
                        if (promise.duplicate == false) {
                            if (promise.returnval == true) {
                                if ($scope.NCAC515VET_Id > 0) {
        ...
        $scope.edittab1 = function (data) {
            debugger;
            apiService.create("NAACVET/EditData", data).then(function (promise) {

                $scope.editlist = promise.editlist;
                $scope.NCAC515VET_Id = promise.editlist[0].ncaC515VET_Id;
                $scope.NCAC515VET_VETProgramName = promise.editlist[0].ncaC515VET_VETProgramName;
                $scope.ASMAY_Id = promise.editlist[0].ncaC515VET_Year;
                $scope.NCAC515VET_NoOfStudents = promise.editlist[0].ncaC515VET_NoOfStudents;
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            $scope.NCAC515VET_Id = usersem.ncaC515VET_Id
            var dystring = "";
            if (usersem.ncaC515VET_ActiveFlg == true) {
                dystring = "Deactivated";
            }
            else if (usersem.ncaC515VET_ActiveFlg == false) {
                dystring = "Activated";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACVET/deactiveStudent", usersem).
        ...
        $scope.viewdocument = function (obj) {
            ...
            apiService.create("NAACVET/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
                    debugger;
        ...
        $scope.deleteuploadfile = function (obj) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACVET/deleteuploadfile", data).then(function (promise) {
