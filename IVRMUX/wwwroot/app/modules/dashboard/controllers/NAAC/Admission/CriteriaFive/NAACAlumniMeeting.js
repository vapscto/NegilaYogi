...
(function () {
    ...
    angular
        .module('app')
        .controller('NAACAlumniMeetingController', NAACAlumniMeetingController);

    NAACAlumniMeetingController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce','myFactorynaac'];

    function NAACAlumniMeetingController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce,myFactorynaac) {
        ...
        $scope.clrr = function () {
            $scope.NCAC543ALMMET_MeetingDate = '';
            $scope.NCAC543ALMMET_NoOfMeetings = '';
            $scope.NCAC543ALMMET_NoOfMemAttnd = '';
            $scope.NCAC543ALMMET_TotalAlumniCount = '';
            $scope.NCAC543ALMMET_Id = 0;
        ...
        $scope.loaddata = function () {
            ...
            apiService.getURI("NAACAlumniMeeting/loaddata", $scope.mI_Id ).then(function (promise) {
        ...
        $scope.getorganizationdata = function (obj) {
            apiService.create("NAACAlumniMeeting/getcomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.getorganizationdata1 = function (obj) {
            debugger;
            obj.ncaC543ALMMETF_Id = obj.cfileid;
            apiService.create("NAACAlumniMeeting/getfilecomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.addcomments = function (obje) {
            ...
            $scope.ccc = obje.ncaC543ALMMET_Id;
            $scope.generalcomments = '';
            $scope.obj.generalcomments = $scope.generalcomments;
            var bb;
        };
        ...
        $scope.savedatawisecomments = function (obj) {
            ...
            if ($scope.myFormc.$valid) {
                ...
                apiService.create("NAACAlumniMeeting/savemedicaldatawisecomments", data).then(function (promise) {
                    if (promise !== null) {
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            if ($scope.myFormc1.$valid) {
                ...
                apiService.create("NAACAlumniMeeting/savefilewisecomments", data).then(function (promise) {
                    if (promise !== null) {
                        if (promise.returnval === true) {
                            swal("Comments Saved Successfully");
                        } else {
        ...
        $scope.savedatatab1 = function () {
            ...
            if ($scope.myForm.$valid) {
                var entrydate = $scope.NCAC543ALMMET_MeetingDate == null ? "" : $filter('date')($scope.NCAC543ALMMET_MeetingDate, "yyyy-MM-dd");
                var data = {
                    "MI_Id": $scope.mI_Id,
                        "NCAC543ALMMET_Id": $scope.NCAC543ALMMET_Id,
                        "NCAC543ALMMET_MeetingYear": $scope.ASMAY_Id,
                        "NCAC543ALMMET_MeetingDate": entrydate,
                        "NCAC543ALMMET_TotalAlumniCount": $scope.NCAC543ALMMET_TotalAlumniCount,
                        "NCAC543ALMMET_NoOfMeetings": $scope.NCAC543ALMMET_NoOfMeetings,
                        "NCAC543ALMMET_NoOfMemAttnd": $scope.NCAC543ALMMET_NoOfMemAttnd,
                        filelist: $scope.materaldocuupload,
                    }
               

                apiService.create("NAACAlumniMeeting/save", data).then(function (promise) {

                    if (promise.duplicate != null && promise.returnval != null) {
                        if (promise.duplicate == false) {
                            if (promise.returnval == true) {
                                if ($scope.NCAC543ALMMET_Id > 0) {
        ...
        $scope.edittab1 = function (data) {
            debugger;
            apiService.create("NAACAlumniMeeting/EditData", data).then(function (promise) {

                $scope.editlist = promise.editlist;
                $scope.NCAC543ALMMET_Id = promise.editlist[0].ncaC543ALMMET_Id;
                $scope.NCAC543ALMMET_MeetingDate = new Date(promise.editlist[0].ncaC543ALMMET_MeetingDate);
                $scope.ASMAY_Id = promise.editlist[0].ncaC543ALMMET_MeetingYear;
                $scope.NCAC543ALMMET_TotalAlumniCount = promise.editlist[0].ncaC543ALMMET_TotalAlumniCount;
                $scope.NCAC543ALMMET_NoOfMeetings = promise.editlist[0].ncaC543ALMMET_NoOfMeetings;
                $scope.NCAC543ALMMET_NoOfMemAttnd = promise.editlist[0].ncaC543ALMMET_NoOfMemAttnd;
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            ...
            var dystring = "";
            if (usersem.ncaC543ALMMET_ActiveFlg == true) {
                dystring = "Deactivated";
            }
            else if (usersem.ncaC543ALMMET_ActiveFlg == false) {
                dystring = "Activated";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACAlumniMeeting/deactiveStudent", usersem).
        ...
        $scope.viewdocument = function (obj) {
            ...
            apiService.create("NAACAlumniMeeting/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
                    debugger;
        ...
        $scope.deleteuploadfile = function (obj) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACAlumniMeeting/deleteuploadfile", data).then(function (promise) {
