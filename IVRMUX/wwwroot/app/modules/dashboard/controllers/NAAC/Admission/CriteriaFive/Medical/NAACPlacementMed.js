...
(function () {
    ...
    angular
        .module('app')
        .controller('NAACPlacementMedController', NAACPlacementMedController);

    NAACPlacementMedController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce','myFactorynaac'];

    function NAACPlacementMedController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        ...
        $scope.clrr = function () {
            ...
            $scope.NCAC521PLA_NoOfStudents = '';
            $scope.NCAC521PLA_NoOfstudentsselfemployed = '';
            $scope.NCAC521PLA_Package = '';


            $scope.NCAC521PLA_Id = 0;
        ...
        $scope.loaddata = function () {
            ...
            apiService.getURI("NAACPlacement/loaddata", $scope.mI_Id ).then(function (promise) {
        ...
        $scope.getorganizationdata = function (obj) {
            apiService.create("NAACPlacement/getcomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.getorganizationdata1 = function (obj) {
            debugger;
            obj.ncaC521PLAF_Id = obj.cfileid;
            apiService.create("NAACPlacement/getfilecomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.savedatawisecomments = function (obj) {
            ...
            if ($scope.myFormc.$valid) {
                ...
                apiService.create("NAACPlacement/savemedicaldatawisecomments", data).then(function (promise) {
                    if (promise !== null) {
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            if ($scope.myFormc1.$valid) {
                ...
                apiService.create("NAACPlacement/savefilewisecomments", data).then(function (promise) {
                    if (promise !== null) {
                        if (promise.returnval === true) {
                            swal("Comments Saved Successfully");
                        } else {
        ...
        $scope.savedatatab1 = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                var data = {
                    "MI_Id": $scope.mI_Id,
                        "NCAC521PLA_Id": $scope.NCAC521PLA_Id,
                        "ASMAY_Id": $scope.ASMAY_Id,
                        "AMCO_Id": $scope.AMCO_Id,
                        "AMB_Id": $scope.AMB_Id,
                    "NCAC521PLA_NoOfstudentsselfemployed": $scope.NCAC521PLA_NoOfstudentsselfemployed,
                        "NCAC521PLA_NoOfStudents": $scope.NCAC521PLA_NoOfStudents,
                        "NCAC521PLA_EmployerName": $scope.NCAC521PLA_EmployerName,
                        "NCAC521PLA_Package": $scope.NCAC521PLA_Package,
                        filelist: $scope.materaldocuupload,
                       
                    }
               

                apiService.create("NAACPlacement/save", data).then(function (promise) {

                    if (promise.duplicate != null && promise.returnval != null) {
                        if (promise.duplicate == false) {
                            if (promise.returnval == true) {
                                if ($scope.NCAC521PLA_Id > 0) {
        ...
        $scope.edittab1 = function (data) {
            debugger;
            apiService.create("NAACPlacement/EditData", data).then(function (promise) {

                $scope.editlist = promise.editlist;
                $scope.NCAC521PLA_Id = promise.editlist[0].ncaC521PLA_Id;
                $scope.NCAC521PLA_EmployerName = promise.editlist[0].ncaC521PLA_EmployerName;
                $scope.ASMAY_Id = promise.editlist[0].asmaY_Id;
                $scope.NCAC521PLA_NoOfStudents = promise.editlist[0].ncaC521PLA_NoOfStudents;
                $scope.NCAC521PLA_NoOfstudentsselfemployed = promise.editlist[0].ncaC521PLA_NoOfstudentsselfemployed;
                $scope.NCAC521PLA_Package = promise.editlist[0].ncaC521PLA_Package;
                debugger;
                $scope.AMCO_Id = promise.editlist[0].ncaC521PLA_GradCourse;
                if ($scope.AMCO_Id != "" && $scope.AMCO_Id != undefined && $scope.AMCO_Id != null) {
                    $scope.get_course();
                    $scope.AMCO_Id = promise.editlist[0].ncaC521PLA_GradCourse;
                }

                $scope.AMB_Id = promise.editlist[0].ncaC521PLA_GradBranch;
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            var dystring = "";
            if (usersem.ncaC521PLA_ActiveFlg == true) {
                dystring = "Deactivated";
            }
            else if (usersem.ncaC521PLA_ActiveFlg == false) {
                dystring = "Activated";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACPlacement/deactiveStudent", usersem).
        ...
        $scope.viewdocument = function (obj) {
            ...
            apiService.create("NAACPlacement/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
                    debugger;
        ...
        $scope.deleteuploadfile = function (obj) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACPlacement/deleteuploadfile", data).then(function (promise) {
