...
(function () {
    'use strict';
    angular.module('app').controller('LEDBulbsController', LEDBulbsController);

    LEDBulbsController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce', 'myFactorynaac'];

    function LEDBulbsController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        $scope.mI_Id = 0;
        $scope.equityid = 0;
        $scope.currentPage = 1;
        ...
        $scope.getData = function (mI_Id) {
            $scope.equityid = 0;
            var id = mI_Id;
            apiService.getURI("LEDBulbs/getData", id).then(function (promise) {
            ...
            if ($scope.myForm.$valid) {
                ...
                if ($scope.materaldocuupload.length > 0) {
                    angular.forEach($scope.materaldocuupload, function (val) {
                        if (val.ncaC714LEDBUF_FilePath !== null && val.ncaC714LEDBUF_FilePath !== undefined) {
                ...
                var data = {
                    ...
                    "NCAC714LEDBU_Id": $scope.equityid,
                    "NCAC714LEDBU_Year": $scope.page.ncaC714LEDBU_Year,
                    "NCAC714LEDBU_LightingsRequirements": $scope.page.ncaC714LEDBU_LightingsRequirements,
                    "NCAC714LEDBU_LughtingLED": $scope.page.ncaC714LEDBU_LughtingLED,
                    "NCAC714LEDBU_OtherSource": $scope.page.ncaC714LEDBU_OtherSource,
                    "NAACAC7DTO": $scope.materaldocuupload22
                };
                apiService.create("LEDBulbs/save", data).then(function (promise) {
        ...
        $scope.edittab1 = function (data) {
            $scope.instit = true;
            apiService.create("LEDBulbs/EditData", data).then(function (promise) {
                $scope.equityid = data.ncaC714LEDBU_Id;
                ...
                if (promise.editfilelist != null && promise.editfilelist.length > 0) {
                    ...
                    angular.forEach($scope.materaldocuupload, function (ddd) {
                        var img = ddd.ncaC714LEDBUF_FilePath;
                        var imagarr = img.split('.');
                        var lastelement = imagarr[imagarr.length - 1];
                        ddd.filetype = lastelement;
                        if (lastelement === 'doc' || lastelement === 'docx' || lastelement === 'ppt' || lastelement === 'pptx' || lastelement === 'xlsx' || lastelement === 'xls') {
                            ddd.document_Pathnew = "https://view.officeapps.live.com/op/view.aspx?src=" + ddd.ncaC714LEDBUF_FilePath;
                        }
                    });
                }
            });
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            ...
            var dystring = "";
            if (usersem.ncaC714LEDBU_ActiveFlg == true) {
                dystring = "Deactivated";
            }
            else if (usersem.ncaC714LEDBU_ActiveFlg == false) {
                dystring = "Activated";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("LEDBulbs/deactivate", usersem).
        ...
        $scope.getorganizationdata = function (obj) {
            ...
            apiService.create("LEDBulbs/getfilecommentLEDbulb", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.getorganizationdata1 = function (obj) {
            ...
            apiService.create("LEDBulbs/getcommentLEDbulb", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.addcomments = function (obje) {
            ...
            $scope.cc = obje.ncaC714LEDBUF_Id;
        ...
        $scope.addcomments1 = function (obje) {
            ...
            $scope.ccc = obje.ncaC714LEDBU_Id;
            $scope.generalcomments = '';
            $scope.obj.generalcomments = $scope.generalcomments;
            var bb;
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            apiService.create("LEDBulbs/savemedicaldatawisecommentsLEDbulb", data).then(function (promise) {
                if (promise !== null) {
                    if (promise.returnval === true) {
        ...
        $scope.savedatawisecomments = function (obj) {
            ...
            apiService.create("LEDBulbs/savefilewisecommentsLEDbulb", data).then(function (promise) {
                if (promise !== null) {
                    if (promise.returnval === true) {
                        swal("Comments Saved Successfully");
                        $scope.page = $scope.page1;
        ...
        function UploaddianmateralPhoto(data) {
            ...
            $http.post("/api/ImageUpload/Uploadnaacdocuments", formData,
                ...
                .success(function (d) {
                    defer.resolve(d);
                    data.ncaC714LEDBUF_FilePath = d;
                    data.ncaC714LEDBUF_FileName = $scope.filename;
                    $('#').attr('src', data.ncaC714LEDBUF_FilePath);
                    var img = data.ncaC714LEDBUF_FilePath;
                    var imagarr = img.split('.');
                    var lastelement = imagarr[imagarr.length - 1];
                    data.filetype = lastelement;
                    if (lastelement === 'doc' || lastelement === 'docx' || lastelement === 'ppt' || lastelement === 'pptx' || lastelement === 'xlsx' || lastelement === 'xls') {
                        data.document_Pathnew = "https://view.officeapps.live.com/op/view.aspx?src=" + data.ncaC714LEDBUF_FilePath;
                    }
                })
                .error(function () {
        ...
        $scope.showGuardianPhotonew = function (data) {
            $scope.view_videos = [];
            $scope.videoSources = [];
            $scope.preview1 = data.ncaC714LEDBUF_FilePath;
            $scope.videdfd = data.ncaC714LEDBUF_FilePath;
            $scope.movie = { src: data.ncaC714LEDBUF_FilePath };
            $scope.movie1 = { src: data.ncaC714LEDBUF_FilePath };
            $scope.view_videos.push({ id: 1, coeeV_Videos: data.ncaC714LEDBUF_FilePath });
                ...
                if (promise !== null) {
                    ...
                    if (promise.editfiles !== null && promise.editfiles.length > 0) {
                        ...
                        angular.forEach($scope.uploaddocfiles, function (dd) {
                            var img = dd.ncaC714LEDBUF_FilePath;
                            var imagarr = img.split('.');
                            var lastelement = imagarr[imagarr.length - 1];
                            dd.filetype = lastelement;
                            if (lastelement === 'doc' || lastelement === 'docx' || lastelement === 'ppt' || lastelement === 'pptx' || lastelement === 'xlsx' || lastelement === 'xls') {
                                dd.document_Pathnew = "https://view.officeapps.live.com/op/view.aspx?src=" + dd.ncaC714LEDBUF_FilePath;
                            }
                        });
                    } else {
                        $('#popup11').modal('hide');
        ...
        $scope.deleteuploadfile = function (index,obj) {
            if (obj.ncaC714LEDBUF_Id == undefined) {
                $scope.materaldocuupload.splice(index, 1);
                swal("Record Deleted successfully");
            }
            else {
                var data = {
                    "NCAC714LEDBUF_Id": obj.ncaC714LEDBUF_Id
                };
                        ...
                        if (isConfirm) {
                            apiService.create("LEDBulbs/deleteuploadfile", data).then(function (promise) {
                                if (promise.returnval === true) {
                                    ...
                                    var data = { "ncaC714LEDBU_Id": $scope.equityid };
