...
(function () {
    'use strict';
    angular.module('app').controller('WaterConservFacilitiesMCController', WaterConservFacilitiesMCController);

    WaterConservFacilitiesMCController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce','myFactorynaac'];

    function WaterConservFacilitiesMCController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        ...
        $scope.getDataMC = function (mI_Id) {
            $scope.equityid = 0;
            var id = mI_Id;
            apiService.getURI("LEDBulbs/getDataMCwater", id).then(function (promise) {
        ...
        $scope.savedatatab1 = function () {
            if ($scope.myForm.$valid) {
                ...
                var data = {
                    ...
                    "NCMC715WCF_Id": $scope.equityid,
                    "NCMC715WCF_Year": $scope.page.ncmC715WCF_Year,
                    "NCMC715WCF_RainWaterHarvestingFlag": $scope.page.ncmC715WCF_RainWaterHarvestingFlag,
                    "NCMC715WCF_BorewellOpenwellRecFlag": $scope.page.ncmC715WCF_BorewellOpenwellRecFlag,
                    "NCMC715WCF_ConstructionOftanksbundsFlag": $scope.page.ncmC715WCF_ConstructionOftanksbundsFlag,
                    "NCMC715WCF_MaintenanceOfWaterbodiesDSFlag": $scope.page.ncmC715WCF_MaintenanceOfWaterbodiesDSFlag,
                    "NCMC715WCF_WastewaterrecyclingFlag": $scope.page.ncmC715WCF_WastewaterrecyclingFlag
                };
                apiService.create("LEDBulbs/saveMCwater", data).then(function (promise) {
        ...
        $scope.edittab1 = function (data) {
            $scope.instit = true;
            apiService.create("LEDBulbs/EditDataMCwater", data).then(function (promise) {
                $scope.equityid = data.ncmC715WCF_Id;
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            var dystring = "";
            if (usersem.ncmC715WCF_ActiveFlg == true) {
                dystring = "Deactivated";
            }
            else if (usersem.ncmC715WCF_ActiveFlg == false) {
                dystring = "Activated";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("LEDBulbs/deactivateMCwater", usersem).
        ...
        $scope.getorganizationdata1 = function (obj) {
            apiService.create("LEDBulbs/getcomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            apiService.create("LEDBulbs/savemedicaldatawisecomments", data).then(function (promise) {
                if (promise !== null) {
                    if (promise.returnval === true) {
