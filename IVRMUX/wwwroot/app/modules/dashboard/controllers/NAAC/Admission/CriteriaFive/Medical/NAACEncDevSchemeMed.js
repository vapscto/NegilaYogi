...
(function () {
    ...
    angular
        .module('app')
        .controller('NAACEncDevSchemeMedController', NAACEncDevSchemeMedController);

    NAACEncDevSchemeMedController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce','myFactorynaac'];

    function NAACEncDevSchemeMedController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        ...
        $scope.clrr = function () {
            $scope.NCAC513INSCH_DevSchemeName = '';
            $scope.NCAC513INSCH_NoOfStudents = '';
            $scope.NCAC513INSCH_AgencyDetails = '';

            $scope.NCAC513INSCH_Id = 0;
        ...
        $scope.loaddata = function () {
            ...
            apiService.getURI("NAACEncDevScheme/loaddata", $scope.mI_Id).then(function (promise) {
        ...
        $scope.getorganizationdata = function (obj) {
            apiService.create("NAACEncDevScheme/getcomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.getorganizationdata1 = function (obj) {
            debugger;
            obj.ncaC513INSCHF_Id = obj.cfileid;
            apiService.create("NAACEncDevScheme/getfilecomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.addcomments = function (obje) {
            $scope.ccc = obje.ncaC513INSCH_Id;
        ...
        $scope.savedatawisecomments = function (obj) {
            ...
            if ($scope.myFormc.$valid) {
                ...
                apiService.create("NAACEncDevScheme/savemedicaldatawisecomments", data).then(function (promise) {
                    if (promise !== null) {
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            if ($scope.myFormc1.$valid) {
                ...
                apiService.create("NAACEncDevScheme/savefilewisecomments", data).then(function (promise) {
                    if (promise !== null) {
                        if (promise.returnval === true) {
                            swal("Comments Saved Successfully");
                        } else {
        ...
        $scope.savedatatab1 = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                var data = {
                    "MI_Id": $scope.mI_Id,
                        "NCAC513INSCH_Id": $scope.NCAC513INSCH_Id,
                        "NCAC513INSCH_ImpYear": $scope.ASMAY_Id,
                        "NCAC513INSCH_DevSchemeName": $scope.NCAC513INSCH_DevSchemeName,
                        "NCAC513INSCH_NoOfStudents": $scope.NCAC513INSCH_NoOfStudents,
                        "NCAC513INSCH_AgencyDetails": $scope.NCAC513INSCH_AgencyDetails,
                        filelist: $scope.materaldocuupload,
                    }
               

                apiService.create("NAACEncDevScheme/save", data).then(function (promise) {

                    if (promise.duplicate != null && promise.returnval != null) {
                        if (promise.duplicate == false) {
                            if (promise.returnval == true) {
                                if ($scope.NCAC513INSCH_Id > 0) {
        ...
        $scope.edittab1 = function (data) {
            debugger;
            apiService.create("NAACEncDevScheme/EditData", data).then(function (promise) {

                $scope.editlist = promise.editlist;
                $scope.NCAC513INSCH_Id = promise.editlist[0].ncaC513INSCH_Id;
                $scope.NCAC513INSCH_DevSchemeName = promise.editlist[0].ncaC513INSCH_DevSchemeName;
                $scope.ASMAY_Id = promise.editlist[0].ncaC513INSCH_ImpYear;
                $scope.NCAC513INSCH_NoOfStudents = promise.editlist[0].ncaC513INSCH_NoOfStudents;
                $scope.NCAC513INSCH_AgencyDetails = promise.editlist[0].ncaC513INSCH_AgencyDetails;
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            $scope.NCAC513INSCH_Id = usersem.ncaC513INSCH_Id
            var dystring = "";
            if (usersem.ncaC513INSCH_ActiveFlg == true) {
                dystring = "Deactivated";
            }
            else if (usersem.ncaC513INSCH_ActiveFlg == false) {
                dystring = "Activated";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACEncDevScheme/deactiveStudent", usersem).
        ...
        $scope.viewdocument = function (obj) {
            ...
            apiService.create("NAACEncDevScheme/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
                    debugger;
        ...
        $scope.deleteuploadfile = function (obj) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACEncDevScheme/deleteuploadfile", data).then(function (promise) {
