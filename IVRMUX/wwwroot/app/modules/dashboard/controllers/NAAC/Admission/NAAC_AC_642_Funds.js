(function () {
    ...
    angular
        .module('app')
        .controller('NAAC_AC_642_FundsController', NAAC_AC_642_FundsController)

    NAAC_AC_642_FundsController.$inject = ['$rootScope', '$scope', '$state', '$location', 'apiService', 'Flash', '$http', '$q', '$stateParams', '$filter', '$sce','myFactorynaac']
    function NAAC_AC_642_FundsController($rootScope, $scope, $state, $location, apiService, Flash, $http, $q, $stateParams, $filter, $sce, myFactorynaac) {
        ...
        $scope.loaddata = function () {
            ...
            apiService.getURI("NAAC_AC_642_Funds/loaddata", $scope.mI_Id ).then(function (promise) {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                ...
                { name: 'name', displayName: 'Agency Name', width: 150 },
                { name: 'amount', displayName: 'Amount', width: 100 },
                { name: 'org', displayName: 'Initiative', width: 100 },
        ...
        $scope.viewteacherguides = function (obj) {
            ...
            var data = {
                "NCAC642FUND_Id": obj.ncaC642FUND_Id,
                "MI_Id": obj.mI_Id ,
            };

            apiService.create("NAAC_AC_642_Funds/viewuploadflies", data).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.deleteuploadfile = function (obj) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAAC_AC_642_Funds/deleteuploadfile", data).then(function (promise) {
        ...
        $scope.save = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                var data = {
                    "NCAC642FUND_Id": $scope.NCAC642FUND_Id,
                    "ASMAY_Id": $scope.ASMAY_Id,
                    "flagradio": $scope.flagradio,
                    "amount": $scope.count,
                    "Name": $scope.projectname,
                    "org": $scope.Participant,
                    "pgTempDTO": $scope.ProgramDetails,
                   
                    "MI_Id": $scope.mI_Id,
                }

                apiService.create("NAAC_AC_642_Funds/save", data).then(function (promise) {
        ...
        $scope.deactiveStudent = function (usersem, SweetAlert) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAAC_AC_642_Funds/deactiveStudent", usersem).
        ...
        $scope.EditData = function (user) {
            ...
            var data = {
                "NCAC642FUND_Id": user.ncaC642FUND_Id,
                "MI_Id": user.mI_Id,
            }
            apiService.create("NAAC_AC_642_Funds/EditData", data).then(function (promise) {
                if (promise.editlist.length > 0) {
                   
                    $scope.institute_flag = true;
                    $scope.NCAC642FUND_Id = promise.editlist[0].ncaC642FUND_Id;
                    $scope.ASMAY_Id = promise.editlist[0].asmaY_Id;
                    $scope.projectname = promise.editlist[0].name;
                    $scope.count = promise.editlist[0].amount;
                    $scope.Participant = promise.editlist[0].org;
                    $scope.mI_Id = promise.editlist[0].mI_Id;
                    if (promise.editlist[0].ncaC642FUND_GovORNongovFlag == "G") {
                        $scope.flagradio ="S";
                    }
                    if (promise.editlist[0].ncaC642FUND_GovORNongovFlag == "N") {
                        $scope.flagradio ="L";
                    }
        ...
        $scope.getorganizationdata = function (obj) {
            debugger;
            apiService.create("NAAC_AC_642_Funds/getcomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.getorganizationdata1 = function (obj) {
            debugger;
            apiService.create("NAAC_AC_642_Funds/getfilecomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.savedatawisecomments = function (obj) {
            ...
            apiService.create("NAAC_AC_642_Funds/savemedicaldatawisecomments", data).then(function (promise) {
                if (promise !== null) {
                    if (promise.returnval === true) {
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            apiService.create("NAAC_AC_642_Funds/savefilewisecomments", data).then(function (promise) {
                if (promise !== null) {
                    if (promise.returnval === true) {
                        swal("Comments Saved Successfully");
                    } else {
