...
(function () {
    'use strict';
    angular.module('app').controller('AlternateEnergyController', AlternateEnergyController);

    AlternateEnergyController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce','myFactorynaac'];

    function AlternateEnergyController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        $scope.mI_Id = 0;
        $scope.equityid = 0;
        $scope.currentPage = 1;
        ...
        $scope.loaddata = function () {
            $scope.equityid = 0;
            var id = 2;
            apiService.getURI("AlternateEnergy/loaddata", id).then(function (promise) {
        ...
        $scope.getData = function (mI_Id) {
            $scope.equityid = 0;
            var id = mI_Id;
            apiService.getURI("AlternateEnergy/getData", id).then(function (promise) {
            ...
            if ($scope.myForm.$valid) {
                ...
                if ($scope.materaldocuupload.length > 0) {
                    angular.forEach($scope.materaldocuupload, function (val) {
                        if (val.ncaC713ALTENEF_FilePath !== null && val.ncaC713ALTENEF_FilePath !== undefined) {
                ...
                var data = {
                    ...
                    "NCAC713ALTENE_Id": $scope.equityid,
                    "NCAC713ALTENE_Year": $scope.page.ncaC713ALTENE_Year,
                    "NCAC713ALTENE_PowerRequirements": $scope.page.ncaC713ALTENE_PowerRequirements,
                    "NCAC713ALTENE_TotalPowerReq": $scope.page.ncaC713ALTENE_TotalPowerReq,
                    "NCAC713ALTENE_EnergySource": $scope.page.ncaC713ALTENE_EnergySource,
                    "NCAC713ALTENE_EnergyUsed": $scope.page.ncaC713ALTENE_EnergyUsed,
                    "NCAC713ALTENE_EnergySupplied": $scope.page.ncaC713ALTENE_EnergySupplied,
                    "NAACAC7DTO": $scope.materaldocuupload22
                };
                apiService.create("AlternateEnergy/save", data).then(function (promise) {
        ...
        $scope.edittab1 = function (data) {
            apiService.create("AlternateEnergy/EditData", data).then(function (promise) {
                $scope.equityid = data.ncaC713ALTENE_Id;
                ...
                if (promise.editfilelist != null && promise.editfilelist.length > 0) {
                    ...
                    angular.forEach($scope.materaldocuupload, function (ddd) {
                        var img = ddd.ncaC713ALTENEF_FilePath;
                        var imagarr = img.split('.');
                        var lastelement = imagarr[imagarr.length - 1];
                        ddd.filetype = lastelement;
                        if (lastelement === 'doc' || lastelement === 'docx' || lastelement === 'ppt' || lastelement === 'pptx' || lastelement === 'xlsx' || lastelement === 'xls') {
                            ddd.document_Pathnew = "https://view.officeapps.live.com/op/view.aspx?src=" + ddd.ncaC713ALTENEF_FilePath;
                        }
                    });
                }
            });
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            ...
            var dystring = "";
            if (usersem.ncaC713ALTENE_ActiveFlg == true) {
                dystring = "Deactivated";
            }
            else if (usersem.ncaC713ALTENE_ActiveFlg == false) {
                dystring = "Activated";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("AlternateEnergy/deactivate", usersem).
        ...
        $scope.viewdocument = function (obj) {
            ...
            apiService.create("AlternateEnergy/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.deleteuploadfile = function (index,obj) {
            if (obj.ncaC713ALTENEF_Id == undefined) {
                $scope.materaldocuupload.splice(index, 1);
                ...
                swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            apiService.create("AlternateEnergy/deleteuploadfile", data).then(function (promise) {
                                if (promise.returnval === true) {
                                    swal("Record Deleted successfully");
