...
(function () {
    'use strict';
    angular.module('app').controller('DifferentlyAbledController', DifferentlyAbledController);

    DifferentlyAbledController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce', 'myFactorynaac'];

    function DifferentlyAbledController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        $scope.mI_Id = 0;
        $scope.equityid = 0;
        $scope.currentPage = 1;
        ...
        $scope.getData = function (mI_Id) {
            $scope.equityid = 0;
            var id = mI_Id;
            apiService.getURI("DifferentlyAbled/getData", id).then(function (promise) {
            ...
            if ($scope.myForm.$valid) {
                ...
                if ($scope.materaldocuupload.length > 0) {
                    angular.forEach($scope.materaldocuupload, function (val) {
                        if (val.ncaC719DIFFABF_FilePath !== null && val.ncaC719DIFFABF_FilePath !== undefined) {
                ...
                var data = {
                    ...
                    "NCAC719DIFFAB_LIFTFacilityFlg": $scope.page.ncaC719DIFFAB_LIFTFacilityFlg,
                    "NCAC719DIFFAB_PhysicalFacilityFlg": $scope.page.ncaC719DIFFAB_PhysicalFacilityFlg,
                    "NCAC719DIFFAB_BrailleSaoftFlg": $scope.page.ncaC719DIFFAB_BrailleSaoftFlg,
                    "NCAC719DIFFAB_RestRoomFlg": $scope.page.ncaC719DIFFAB_RestRoomFlg,
                    "NCAC719DIFFAB_ExamScribeFlg": $scope.page.ncaC719DIFFAB_ExamScribeFlg,
                    "NCAC719DIFFAB_SPLSkilDevFlg": $scope.page.ncaC719DIFFAB_SPLSkilDevFlg,
                    "NCAC719DIFFAB_RampFacilityFlg": $scope.page.ncaC719DIFFAB_RampFacilityFlg,
                    "NCAC719DIFFAB_OtherFacility": $scope.page.ncaC719DIFFAB_OtherFacility,
                    "NAACAC7DTO": $scope.materaldocuupload22
                };
                apiService.create("DifferentlyAbled/save", data).then(function (promise) {
        ...
        $scope.edittab1 = function (data) {
            apiService.create("DifferentlyAbled/EditData", data).then(function (promise) {
                ...
                if (promise.editfilelist != null && promise.editfilelist.length > 0) {
                    ...
                    angular.forEach($scope.materaldocuupload, function (ddd) {
                        var img = ddd.ncaC719DIFFABF_FilePath;
                        var imagarr = img.split('.');
                        var lastelement = imagarr[imagarr.length - 1];
                        ddd.filetype = lastelement;
                        if (lastelement === 'doc' || lastelement === 'docx' || lastelement === 'ppt' || lastelement === 'pptx' || lastelement === 'xlsx' || lastelement === 'xls') {
                            ddd.document_Pathnew = "https://view.officeapps.live.com/op/view.aspx?src=" + ddd.ncaC719DIFFABF_FilePath;
                        }
                    });
                }
            });
        ...
        $scope.deleteuploadfile = function (index,obj) {
            if (obj.ncaC719DIFFABF_Id == undefined) {
                $scope.materaldocuupload.splice(index, 1);
