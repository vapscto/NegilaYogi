...
(function () {
    ...
    angular
        .module('app')
        .controller('NAACSportsController', NAACSportsController);

    NAACSportsController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce','myFactorynaac'];

    function NAACSportsController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        ...
        $scope.search = "";
        $scope.issuertype1 = 'Sports';
        $scope.norint = 'National';
        ...
        $scope.clrr = function () {
            $scope.AMCO_Id = "";
            $scope.AMB_Id = "";
            $scope.NCAC521PLA_EmployerName = '';
            $scope.NCAC531SPCAS_AwardName = '';
            $scope.AMCST_Id = '';
            $scope.NCAC531SPCA_NoOfStudents = '';

            $scope.AMSE_Id = '';
            $scope.ACMS_Id = '';
            $scope.NCAC531SPCA_Id = 0;
        ...
        $scope.get_course = function () {
            ...
            apiService.create("NAACSports/get_course", data).then(function (promise) {
        ...
        $scope.getorganizationdata = function (obj) {
            apiService.create("NAACSports/getcomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.getorganizationdata1 = function (obj) {
            debugger;
            obj.ncaC531SPCAF_Id = obj.cfileid;
            apiService.create("NAACSports/getfilecomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.addcomments = function (obje) {
            ...
            $scope.ccc = obje.ncaC531SPCAS_Id;
            $scope.generalcomments = '';
            $scope.obj.generalcomments = $scope.generalcomments;
            var bb;
        };
        ...
        $scope.savedatawisecomments = function (obj) {
            ...
            if ($scope.myFormc.$valid) {
                ...
                apiService.create("NAACSports/savemedicaldatawisecomments", data).then(function (promise) {
                    if (promise !== null) {
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            if ($scope.myFormc1.$valid) {
                ...
                apiService.create("NAACSports/savefilewisecomments", data).then(function (promise) {
                    if (promise !== null) {
                        if (promise.returnval === true) {
                            swal("Comments Saved Successfully");
                        } else {
        ...
        $scope.get_branch = function () {
            ...
            apiService.create("NAACSports/get_branch", data).then(function (promise) {
        ...
        $scope.get_sems = function () {
            ...
            apiService.create("NAACSports/get_sems", data).then(function (promise) {
        ...
        $scope.get_section = function () {
            ...
            apiService.create("NAACSports/get_section", data).then(function (promise) {
        ...
        $scope.GetStudentDetails = function () {
            ...
            apiService.create("NAACSports/GetStudentDetails", data).then(function (promise) {
        ...
        $scope.tabchange1 = function () {
            ...
            $scope.heading = "5.2.3 - Students qualifying in state/national/ international level examinations";
        ...
        $scope.loaddata = function () {
            ...
            apiService.getURI("NAACSports/loaddata", $scope.mI_Id).then(function (promise) {
        ...
        $scope.savedatatab1 = function () {
            ...
            if ($scope.myForm.$valid) {
                if ($scope.AMCST_Id == "" || $scope.AMCST_Id == null || $scope.AMCST_Id == undefined) {
                    swal('Select Student!')
                }
                else {
                    var data = {
                        "MI_Id": $scope.mI_Id,
                    "NCAC531SPCA_Id": $scope.NCAC531SPCA_Id,
                    "NCAC531SPCA_Year": $scope.ASMAY_Id,
                    "NCAC531SPCA_NoOfStudents": $scope.NCAC531SPCA_NoOfStudents,
                    "AMCST_Id": $scope.AMCST_Id.amcsT_Id,
                    "NCAC531SPCAS_AwardName": $scope.NCAC531SPCAS_AwardName,
                    "NCAC531SPCAS_NatOrInterNatFlg": $scope.norint,
                    "NCAC531SPCAS_SportsCAIEEEFlg": $scope.issuertype1,
                    filelist: $scope.materaldocuupload,
                }


                apiService.create("NAACSports/save", data).then(function (promise) {

                    if (promise.duplicate != null && promise.returnval != null) {
                        if (promise.duplicate == false) {
                            if (promise.returnval == true) {
                                if ($scope.NCAC531SPCA_Id > 0) {
        ...
        $scope.Savedatatab2 = function () {
            ...
            if ($scope.myForm2.$valid) {
                ...
                apiService.create("NAACSports/save1", data).then(function (promise) {
        ...
        $scope.edittab1 = function (data) {
            debugger;
            apiService.create("NAACSports/EditData", data).then(function (promise) {
                $scope.materaldocuupload = [{ id: 'Materal1' }];
                $scope.editlist = promise.editlist;
                $scope.NCAC531SPCA_Id = promise.editlist[0].ncaC531SPCA_Id;
                $scope.ASMAY_Id = promise.editlist[0].ncaC531SPCA_Year;
                $scope.NCAC531SPCA_NoOfStudents = promise.editlist[0].ncaC531SPCA_NoOfStudents;
                $scope.NCAC531SPCAS_AwardName = promise.editlist[0].ncaC531SPCAS_AwardName;
                $scope.norint = promise.editlist[0].ncaC531SPCAS_NatOrInterNatFlg;
                $scope.issuertype1 = promise.editlist[0].ncaC531SPCAS_SportsCAIEEEFlg;
                ...
                if ($scope.ASMAY_Id != null && $scope.ASMAY_Id != "" && $scope.ASMAY_Id != undefined && $scope.AMCO_Id != null && $scope.AMCO_Id != "" && $scope.AMCO_Id != undefined && $scope.AMB_Id != null && $scope.AMB_Id != "" && $scope.AMB_Id != undefined && $scope.AMSE_Id != null && $scope.AMSE_Id != "" && $scope.AMSE_Id != undefined) {
                    $scope.get_section();
                    $scope.ACMS_Id = promise.editlist[0].acmS_Id;
                }

                if ($scope.ASMAY_Id != null && $scope.ASMAY_Id != "" && $scope.ASMAY_Id != undefined && $scope.AMCO_Id != null && $scope.AMCO_Id != "" && $scope.AMCO_Id != undefined && $scope.AMB_Id != null && $scope.AMB_Id != "" && $scope.AMB_Id != undefined && $scope.AMSE_Id != null && $scope.AMSE_Id != "" && $scope.AMSE_Id != undefined && $scope.ACMS_Id != null && $scope.ACMS_Id != "" && $scope.ACMS_Id != undefined) {
                    $scope.GetStudentDetails();
                }
                $scope.AMCST_Id = promise.editlist[0];
               // $scope.AMCST_Id.amcsT_Id = promise.editlist[0].amcsT_Id;
        ...
        $scope.edittab3 = function (data) {
            debugger;
            apiService.create("NAACSports/EditData1", data).then(function (promise) {
        ...
        $scope.deactivYTab3 = function (usersem, SweetAlert) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACSports/deactiveStudent1", usersem).
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            ...
            var dystring = "";
            if (usersem.ncaC531SPCA_ActiveFlg == true) {
                dystring = "Deactivated";
            }
            else if (usersem.ncaC531SPCA_ActiveFlg == false) {
                dystring = "Activated";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACSports/deactiveStudent", usersem).
                            then(function (promise) {
                                if (promise.returnval == true) {
                                    swal("Record " + dystring +  " Successfully!!!");
                                }
        ...
        $scope.viewdocument = function (obj) {
            ...
            apiService.create("NAACSports/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
                    debugger;
        ...
        $scope.deleteuploadfile = function (obj) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACSports/deleteuploadfile", data).then(function (promise) {
