(function () {
    ...
    angular
        .module('app')
        .controller('NaacExpAcaFacility441', NaacExpAcaFacility441)

    NaacExpAcaFacility441.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', '$sce','myFactorynaac']
    function NaacExpAcaFacility441($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, $sce, myFactorynaac) {
        ...
        $scope.loaddata = function () {
            ...
            //var pageid = 2;
            apiService.getURI("NaacExpAcaFacility441/loaddata", $scope.mI_Id).
        ...
        $scope.save = function () {
            if ($scope.myForm.$valid) {
                ...
                var data = {
                    "NCAC441ExAcFc_Id": $scope.NCAC441ExAcFc_Id,
                    "asmaY_Id": $scope.NCAC441EXACFC_Year,
                    "NCAC441EXACFC_ExpAccFacility": $scope.NCAC441EXACFC_ExpAccFacility,
                    "NCAC441EXACFC_ExpPhyFacility": $scope.NCAC441EXACFC_ExpPhyFacility,
                    filelist: $scope.materaldocuupload,
                    "MI_Id": $scope.mI_Id,
                }
                apiService.create("NaacExpAcaFacility441/save", data).then(function (promise) {

                    if (promise.duplicate == false) {
                        if (promise.returnval === true) {
                            if ($scope.NCAC441ExAcFc_Id > 0) {
        ...
        $scope.deactiveStudent = function (usersem, SweetAlert) {
            var dystring = "";
            if (usersem.ncaC441EXACFC_ActiveFlg == true) {
                dystring = "Deactivate";
            }
            else if (usersem.ncaC441EXACFC_ActiveFlg == false) {
                dystring = "Activate"
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NaacExpAcaFacility441/deactiveStudent", usersem).
                            then(function (promise) {
                                if (promise.returnval == true) {
        ...
        $scope.EditData = function (user) {
            var data = {
                "NCAC441ExAcFc_Id": user.ncaC441ExAcFc_Id,
                "MI_Id": user.mI_Id
            }
            apiService.create("NaacExpAcaFacility441/EditData", data).then(function (promise) {
                if (promise.editlist.length > 0) {

                    $scope.instit = true;
                    $scope.NCAC441ExAcFc_Id = promise.editlist[0].ncaC441ExAcFc_Id;
                    $scope.NCAC441EXACFC_Year = promise.editlist[0].ncaC441EXACFC_Year;
                    $scope.NCAC441EXACFC_ExpAccFacility = promise.editlist[0].ncaC441EXACFC_ExpAccFacility;
                    $scope.NCAC441EXACFC_ExpPhyFacility = promise.editlist[0].ncaC441EXACFC_ExpPhyFacility;
        ...
        $scope.viewdocument = function (obj) {
            ...
            apiService.create("NaacExpAcaFacility441/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.deleteuploadfile = function (docfile) {
            ...
            var data = {
                "NCAC441ExAcFc_Id": docfile.ncaC441ExAcFc_Id,
                "NCAC441ExAcFcF_Id": docfile.ncaC441ExAcFcF_Id,
            }
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NaacExpAcaFacility441/deleteuploadfile", data).then(function (promise) {
        ...
        $scope.getorganizationdata = function (obj) {
            apiService.create("NaacExpAcaFacility441/getcomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.getorganizationdata1 = function (obj) {
            apiService.create("NaacExpAcaFacility441/getfilecomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.addcomments = function (obje) {
            $scope.ccc = obje.ncaC441ExAcFc_Id;
        ...
        $scope.addcomments1 = function (obje) {
            $scope.cc = obje.ncaC441ExAcFcF_Id;
            $scope.generalcomments = '';
            $scope.obj.generalcomments = $scope.generalcomments;
            var bb;
        };
        ...
        $scope.savedatawisecomments = function (obj) {
            ...
            apiService.create("NaacExpAcaFacility441/savemedicaldatawisecomments", data).then(function (promise) {
                if (promise !== null) {
                    if (promise.returnval === true) {
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            apiService.create("NaacExpAcaFacility441/savefilewisecomments", data).then(function (promise) {
                if (promise !== null) {
                    if (promise.returnval === true) {
                        swal("Comments Saved Successfully");
                    } else {
        ...
        $scope.change_institution = function () {
            $scope.NCAC441ExAcFc_Id = 0;
            $scope.NCAC441EXACFC_Year = '';
            $scope.NCAC441EXACFC_ExpAccFacility = '';
            $scope.NCAC441EXACFC_ExpPhyFacility = '';
