(function () {
    ...
    angular
        .module('app')
        .controller('NAACAC7110LocationalAdvtgReportController', NAACAC7110LocationalAdvtgReportController)

    NAACAC7110LocationalAdvtgReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', 'Excel', '$timeout']
    function NAACAC7110LocationalAdvtgReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, Excel, $timeout) {
        ...
        $scope.loaddata = function () {
            var pageid = 2;
            apiService.getURI("NAACAC7110LocationalAdvtgReport/loaddata", pageid).then(function (promise) {
        ...
        $scope.Report = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("NAACAC7110LocationalAdvtgReport/Report", data).then(function (promise) {
                    $scope.alldata6 = promise.alldata;
                    $scope.alldata62 = promise.alldatafile;

                    $scope.alldata116 = promise.alldata11;
                    $scope.alldata1162 = promise.alldata11file;

                    if (promise.alldata.length == 0 && promise.alldata11.length == 0) {
                        ...
                        if (promise.alldata.length > 0) {
                            angular.forEach($scope.alldata6, function (tt) {
                                ...
                                angular.forEach($scope.alldata62, function (ss) {
                                    if (tt.ncaC7110LOCADVTG_Id == ss.ncaC7110LOCADVTG_Id) {
                        ...
                        if (promise.alldata11.length > 0) {
                            angular.forEach($scope.alldata116, function (tt) {
                                $scope.mainArray = [];
                                var count = 0;
                                angular.forEach($scope.alldata1162, function (ss) {
                                    if (tt.ncaC7111LOCCOM_Id == ss.ncaC7111LOCCOM_Id) {
                                        $scope.mainArray.push(ss);
                                    }
                                });
                                tt.listdata = $scope.mainArray;
                            });
                            console.log($scope.alldata116);
                            $scope.printflag = true;
                        }
                    }
                });
