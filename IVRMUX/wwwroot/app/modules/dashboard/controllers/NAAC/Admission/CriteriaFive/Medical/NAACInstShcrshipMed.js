...
(function () {
    ...
    angular
        .module('app')
        .controller('NAACInstShcrshipMedController', NAACInstShcrshipMedController);

    NAACInstShcrshipMedController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce','myFactorynaac'];

    function NAACInstShcrshipMedController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        ...
        $scope.loaddata = function () {
            ...
            apiService.getURI("NAACInstShcrship/loaddata", $scope.mI_Id).then(function (promise) {
        ...
        $scope.getorganizationdata = function (obj) {
            apiService.create("NAACInstShcrship/getcomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.getorganizationdata1 = function (obj) {
            debugger;
            obj.ncaC512INSCHF_Id = obj.cfileid;
            apiService.create("NAACInstShcrship/getfilecomment", obj).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.addcomments = function (obje) {
            ...
            $scope.ccc = obje.ncaC512INSCH_Id;
            $scope.generalcomments = '';
            $scope.obj.generalcomments = $scope.generalcomments;
            var bb;
        };
        ...
        $scope.savedatawisecomments = function (obj) {
            ...
            if ($scope.myFormc.$valid) {
                ...
                apiService.create("NAACInstShcrship/savemedicaldatawisecomments", data).then(function (promise) {
                    if (promise !== null) {
        ...
        $scope.savedatawisecomments1 = function (obj) {
            ...
            if ($scope.myFormc1.$valid) {
                ...
                apiService.create("NAACInstShcrship/savefilewisecomments", data).then(function (promise) {
                    if (promise !== null) {
                        if (promise.returnval === true) {
                            swal("Comments Saved Successfully");
                        } else {
        ...
        $scope.savedatatab1 = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                var data = {
                    "MI_Id": $scope.mI_Id,
                        "NCAC512INSCH_Id": $scope.NCAC512INSCH_Id,
                        "NCAC512INSCH_Year": $scope.ASMAY_Id,
                        "NCAC512INSCH_SchemeName": $scope.NCAC512INSCH_SchemeName,
                        "NCAC512INSCH_NoOfStudents": $scope.NCAC512INSCH_NoOfStudents,
                        filelist: $scope.materaldocuupload,
                    }
               
            
                apiService.create("NAACInstShcrship/save", data).then(function (promise) {

                    if (promise.duplicate != null && promise.returnval != null) {
                        if (promise.duplicate == false) {
                            if (promise.returnval == true) {
                                if ($scope.NCAC512INSCH_Id > 0) {
        ...
        $scope.edittab1 = function (data) {
            debugger;
            apiService.create("NAACInstShcrship/EditData", data).then(function (promise) {
                $scope.materaldocuupload = [{ id: 'Materal1' }];
                $scope.editlist = promise.editlist;
                $scope.NCAC512INSCH_Id = promise.editlist[0].ncaC512INSCH_Id;
                $scope.NCAC512INSCH_SchemeName = promise.editlist[0].ncaC512INSCH_SchemeName;
                $scope.ASMAY_Id = promise.editlist[0].ncaC512INSCH_Year;
                $scope.NCAC512INSCH_NoOfStudents = promise.editlist[0].ncaC512INSCH_NoOfStudents;
        ...
        $scope.clrr = function () {
            $scope.NCAC512INSCH_NoOfStudents = '';
            $scope.NCAC512INSCH_SchemeName = '';
            $scope.NCAC512INSCH_Id = 0;
        ...
        $scope.deactivYTab1 = function (usersem, SweetAlert) {
            $scope.NCAC512INSCH_Id = usersem.ncaC512INSCH_Id
            var dystring = "";
            if (usersem.ncaC512INSCH_ActiveFlg == true) {
                dystring = "Deactivated";
            }
            else if (usersem.ncaC512INSCH_ActiveFlg == false) {
                dystring = "Activated";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACInstShcrship/deactiveStudent", usersem).
        ...
        $scope.viewdocument = function (obj) {
            ...
            apiService.create("NAACInstShcrship/viewuploadflies", obj).then(function (promise) {
                if (promise !== null) {
                    debugger;
        ...
        $scope.deleteuploadfile = function (obj) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("NAACInstShcrship/deleteuploadfile", data).then(function (promise) {
