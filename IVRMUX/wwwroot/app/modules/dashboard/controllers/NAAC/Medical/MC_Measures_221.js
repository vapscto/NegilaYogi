(function () {
    ...
    angular
        .module('app')
        .controller('MC_Measures_221Controller', MC_Measures_221Controller);

    MC_Measures_221Controller.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', '$timeout', '$q', '$filter', '$sce', 'myFactorynaac'];

    function MC_Measures_221Controller($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, $timeout, $q, $filter, $sce, myFactorynaac) {
        ...
        $scope.savedata = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                if ($scope.check1 == '1') {
                    $scope.NCMCM221_MesCrFolldRegSlowPerFlag = true;
                }
                if ($scope.check2 == '1') {
                    $scope.NCMCM221_MesCrFolldAdLersFlag = true;
                }
                if ($scope.check3 == '1') {
                    $scope.NCMCM221_SpecialprogCrLowORAdlersFlag = true;
                }
                if ($scope.check4 == '1') {
                    $scope.NCMCM221_ProclsMeasureAchsFlag = true;
                }


                var data = {
                    "MI_Id": $scope.mI_Id,
                    "NCMCM221_Id": $scope.ncmcM221_Id,
                    "ASMAY_Id": $scope.ASMAY_Id,
                    "NCMCM221_MesCrFolldRegSlowPerFlag": $scope.NCMCM221_MesCrFolldRegSlowPerFlag,
                    "NCMCM221_MesCrFolldAdLersFlag": $scope.NCMCM221_MesCrFolldAdLersFlag,
                    "NCMCM221_SpecialprogCrLowORAdlersFlag": $scope.NCMCM221_SpecialprogCrLowORAdlersFlag,
                    "NCMCM221_ProclsMeasureAchsFlag": $scope.NCMCM221_ProclsMeasureAchsFlag,

                }
                apiService.create("NAAC_MC_VACcommon/M_savedata221", data).then(function (promise) {
                    if (promise.duplicate == false) {
                        if (promise.returnval == true) {
                            if ($scope.ncmcM221_Id > 0) {
