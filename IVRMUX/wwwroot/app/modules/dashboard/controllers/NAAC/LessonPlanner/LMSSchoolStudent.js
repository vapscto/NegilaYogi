(function () {
    'use strict';
    angular.module('app').controller('LMSSchoolStudentcontroller', LMSSchoolStudentcontroller)

    LMSSchoolStudentcontroller.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$sce']
    function LMSSchoolStudentcontroller($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $sce) {
        $scope.DeleteRecord = {};
        $scope.EditRecord = {};
        $scope.showbtn = false;
        $scope.details = false;
        ...
        $scope.BindData = function () {
            var id = 2;
            apiService.getURI("LMSStudent/Getdetailsschool", id).then(function (promise) {
                $scope.masteryear = promise.getallyear;
                $scope.currentyear = promise.getcurrentyear;

                angular.forEach($scope.masteryear, function (ys) {
                    angular.forEach($scope.currentyear, function (cs) {
                        if (ys.asmaY_Id === cs.asmaY_Id) {
                            ys.Selected = true;
                ...
                $scope.currentclass = promise.getcurrentclass;

                angular.forEach($scope.masterclass, function (ys) {
                    angular.forEach($scope.currentclass, function (cs) {
                        if (ys.asmcL_Id === cs.asmcL_Id) {
                            ys.Selected = true;
                            $scope.ASMCL_Id = $scope.currentclass[0].asmcL_Id;
                            $scope.classname = $scope.currentclass[0].asmcL_ClassName;
                        }
                    });
                });
        ...
        $scope.onchangeyear = function () {
            ...
            apiService.create("LMSStudent/onchangeyear", data).then(function (promise) {
                if (promise !== null) {
                    $scope.getsubjectdetailsnew = promise.getsubjectdetails;
                    if ($scope.getsubjectdetailsnew !== null && $scope.getsubjectdetailsnew.length > 0) {
                        $scope.getsubjectdetails = $scope.getsubjectdetailsnew;
        ...
        $scope.onchangeclass = function () {
            ...
            apiService.create("LMSStudent/onchangeclass", data).then(function (promise) {
                if (promise !== null) {
                    $scope.getsubjectdetailsnew = promise.getsubjectdetails;
                    if ($scope.getsubjectdetailsnew !== null && $scope.getsubjectdetailsnew.length > 0) {
                        $scope.getsubjectdetails = $scope.getsubjectdetailsnew;
                        ...
                        angular.forEach($scope.masterclass, function (dd) {
        ...
        $scope.gettopics = function (obj) {
            ...
            apiService.create("LMSStudent/getschooltopics", data).then(function (promise) {
                if (promise !== null) {
        ...
        $scope.getdocuments = function (obj) {
            ...
            data.ASMCL_Id = $scope.ASMCL_Id,
                apiService.create("LMSStudent/getschooldocuments", data).then(function (promise) {
                    if (promise !== null) {
