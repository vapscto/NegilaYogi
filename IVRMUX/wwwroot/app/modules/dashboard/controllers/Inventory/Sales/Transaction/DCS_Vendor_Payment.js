...
(function () {
    ...
    angular
        .module('app')
        .controller('DCS_Vendor_PaymentController', DCS_Vendor_PaymentController);
    DCS_Vendor_PaymentController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache'];
    function DCS_Vendor_PaymentController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache) {
        ...
        $scope.loaddata = function () {
            ...
            $scope.search = "";
            var pageid = 2;
            apiService.getURI("DCS_Vendor_Payment/getloaddata", pageid).
        ...
        $scope.classSectionflag = function () {
            ...
            apiService.create("DCS_Vendor_Payment/getStudentlist", data).
                then(function (promise) {
        ...
        $scope.storeChange = function () {
            ...
            apiService.create("DCS_Vendor_Payment/getitem", data).
                then(function (promise) {
        ...
        $scope.onitemchange = function (itemid, objid) {
            ...
            apiService.create("DCS_Vendor_Payment/getitemDetail", data).
                then(function (promise) {
                    ...
                    angular.forEach($scope.transrows, function (obj) {
                        if ($scope.editS === true) {
                            ...
                            if (obj.itrS_Id === objid.itrS_Id) {
                                ...
                                //obj.invmuoM_Id = $scope.get_itemDetail[0].invmuoM_Id;
                                //obj.invtsL_BatchNo = $scope.get_itemDetail[0].invstO_BatchNo;
                                //obj.invtsL_SalesPrice = $scope.get_itemDetail[0].invmP_ProductPrice;
        ...
        $scope.viewtxfn = function (qwe) {
            if ($scope.editS === true) {
                ...
                $scope.purrate = parseFloat(qwe.invmI_Id.invmP_ProductPrice);
            }
            //$scope.purrate = parseFloat(qwe.invmI_Id.invmP_ProductPrice);
        ...
        $scope.countAmt = function (objstk, items) {
            ...
            if ($scope.editS === true) {
                ...
                $scope.salerate = parseFloat(objstk.invmI_Id.invmP_ProductPrice);
            }
            ...
            $scope.totalfee = objstk.invtsL_Amount;
            if ($scope.editS === true) {
                angular.forEach($scope.transrows, function (obj) {
        ...
        $scope.printDatanew = function (printmodal) {
            ...
            popupWinindow.document.write('<html><head>' +
                ...
                '<link href="css/print/printpdf.css" rel="stylesheet"/>' +
                '<link href="css/print/printPdf_Bootstrap.css" rel="stylesheet"/>' +
                '<link href="css/print/preadmission/RegReport.css" rel="stylesheet"/>' +
                '<link href="css/style.css" rel="stylesheet"/>' + '</head><body onload="window.print()"  onfocus = "window.setTimeout(function() { window.close(); }, 100);">' + innerContents + '</html>');
            popupWinindow.document.close();
        }
        $scope.typemodelclick = function (typ) {

            var data = {
                "DCSSPT_Id": typ.dcsspT_Id,
                //  "saletype": typ.invmsL_StuOtherFlg
            };
            apiService.create("DCS_Vendor_Payment/getbilldetails", data).
                then(function (promise) {
                    $scope.get_Saletypes = promise.get_SaleItemDetails;
                });
        };
        ...
        $scope.countDiscount = function (objtp, items) {
            ...
            if ($scope.editS === true) {
                ...
                $scope.salerate = parseFloat(objtp.invmI_Id.invmP_ProductPrice);
            }
            $scope.qty1 = parseFloat(objtp.invtsL_SalesQty);
            ...
            $scope.totalfee = objtp.invtsL_Amount;
            angular.forEach($scope.transrows, function (obj) {
            ...
            if ($scope.myForm.$valid) {
                ...
                if ($scope.arraySaletax !== undefined) {
                    angular.forEach($scope.transrows, function (sale) {
                        $scope.arraySale.push({ invmI_Id: sale.invmI_Id.invmI_Id, invtsL_SalesPrice: sale.invmI_Id.invmP_ProductPrice, invtsL_SalesQty: sale.invtsL_SalesQty, invsptgrN_Amount: sale.invtsL_Amount, invsptgrN_Remarks: sale.invtsL_Narations});
                    }); 
                }
                else {
                    $scope.arraySaletax = [];
                    angular.forEach($scope.transrows, function (sale) {
                        $scope.arraySale.push({ invmI_Id: sale.invmI_Id.invmI_Id, invtsL_SalesPrice: sale.invmI_Id.invmP_ProductPrice, invtsL_SalesQty: sale.invtsL_SalesQty, invsptgrN_Amount: sale.invtsL_Amount, invsptgrN_Remarks: sale.invtsL_Naration });
                    });
                }

                
          
                $scope.INVMS_Id = $scope.obj.invmS_Id.invmS_Id;
                    data = {
                        "INVSPT_PaymentDate": $scope.invmsL_SalesDate,
                        "INVMSL_Remarks": $scope.FYP_Remarks,
                        "INVSPT_ModeOfPayment": $scope.FYP_Bank_Or_Cash,
                        "INVSPT_ChequeDDDate": new Date($scope.FYP_DD_Cheque_Date).toDateString(),
                        "INVSPT_ChequeDDNo": $scope.FYP_DD_Cheque_No,
                        "INVSPT_BankName": $scope.FYP_Bank_Name,
                        "INVSPT_Amount": $scope.totalfee,
                        "paymentdto": $scope.arraySale, 
                        "INVMST_Id": $scope.INVMS_Id
                ...
                apiService.create("DCS_Vendor_Payment/savedetails", data).then(function (promise) {
        ...
        $scope.printDatanew = function (printmodal) {
            ...
            popupWinindow.document.write('<html><head>' +
                ...
                '<link type="text/css" media="print" rel="stylesheet" href="css/print/Inventory/transaction/DCS_Vendor_paymentPdf.css" />' +
        ...
        $scope.edit = function (editsl) {
            ...
            apiService.create("DCS_Vendor_Payment/getSaleItemDetails", data).
                then(function (promise) {
        ...
        //    var data = {
        //        "DCSMSL_Id": typ.dcsmsL_Id,
        //      //  "saletype": typ.invmsL_StuOtherFlg
        //    };
        //    apiService.create("DCS_Vendor_Payment/getSaletypes", data).
        //        then(function (promise) {
        ...
        $scope.transmodelclick = function (id) {
            ...
            apiService.create("DCS_Vendor_Payment/getSaleItemTax", data).
                then(function (promise) {
        ...
        $scope.deactive = function (item, SweetAlert) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("DCS_Vendor_Payment/deactive", item).
                            then(function (promise) {
                                if (promise.returnval === true) {
                                    if (promise.returnduplicatestatus === 'Updated') {
        ...
        $scope.deactiveS = function (item, SweetAlert) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("DCS_Vendor_Payment/deactiveS", item).
                            then(function (promise) {
                                if (promise.returnval === true) {
                                    swal("Record " + dystring + "d Successfully!!!");
                                }
        ...
        $scope.deactivetax = function (item, SweetAlert) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("DCS_Vendor_Payment/deactivetax", item).
                            then(function (promise) {
                                if (promise.returnval === true) {
                                    swal("Record " + dystring + "d Successfully!!!");
                                }
