...
(function () {
    ...
    angular
        .module('app')
        .controller('INV_ItemConsumptionCLGController', INV_ItemConsumptionCLGController);
    INV_ItemConsumptionCLGController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache'];
    function INV_ItemConsumptionCLGController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache) {
        ...
        $scope.loaddata = function () {
            ...
            apiService.getURI("INV_ItemConsumption/getloaddata", pageid).
                then(function (promise) {
                    ...
                    $scope.sectionClglist = promise.sectionClglist;
        ...
        $scope.classSectionCS = function (get_ICuser) {
            ...
            apiService.create("INV_ItemConsumption/getstudent", data).then(function (promise) {
                ...
                $scope.transgrid = false;
                if (get_ICuser != null && get_ICuser.length > 0) {
                    $scope.transgrid = true;
                    angular.forEach($scope.get_Studentlist, function (itm) {
                        angular.forEach(get_ICuser, function (itmtwo) {
                            if (itm.AMST_Id == itmtwo.amsT_Id) {
                                itm.secck = true;
                            }
                        });
                    });
                ...
                then(function (promise) {
                    ...
                    if ($scope.userfg === "Staff") {
                        ...
                        angular.forEach($scope.staffuser, function (obju) {
                            $scope.get_employee.push({
                                hrmE_Id: obju.HRME_Id, employeename: obju.username,
                                hrmE_EmployeeCode: obju.HRME_EmployeeCode,
                                hrmeid: true,
                                staffCk: true
                            });
                        });
                        ...
                        $scope.amsE_Id = $scope.get_ICuser[0].AMSE_Id;
                        $scope.acmS_Id = $scope.get_ICuser[0].ACMS_Id;                       
                        $scope.invmiC_StuOtherFlg = "Student";
                        $scope.classSectionCS(promise.get_ICuser);
                    }
                    $scope.cnt = 0;
                    angular.forEach($scope.get_ICdetails, function (objic) {
                        ...
                        $scope.transrows.push({
                            invtiC_Id: objic.invtiC_Id, invmI_Id: objic.invmI_Id,
                            invmI_ItemName: objic.invmI_ItemName, invmuoM_Id: objic.invmuoM_Id,
                            invstO_SalesRate: objic.invtiC_ICPrice, invmuoM_UOMName: objic.invmuoM_UOMName,
                            invmP_Id: objic.invmP_Id, invmP_ProductName: objic.invmP_ProductName,
                            invtiC_BatchNo: objic.invtiC_BatchNo, invtiC_ICQty: objic.invtiC_ICQty, invtiC_Naration: objic.invtiC_Naration, 'itrS_Id': 'trans' + newItemNo
                        });
                    });
