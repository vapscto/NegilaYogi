...
(function () {
    ...
    angular
        .module('app')
        .controller('INV_StockRegister_ReportController', INV_StockRegister_ReportController);
    INV_StockRegister_ReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'Excel', 'superCache', '$timeout', '$filter'];
    function INV_StockRegister_ReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, Excel, superCache, $timeout, $filter) {
        ...
        $scope.onreport = function () {
            ...
            if ($scope.myForm.$valid) {                            
                ...
                apiService.create("INV_StockSummary/onreporttwo", data).
                    then(function (promise) {
                        if (promise.stock_summaryreport != null && promise.stock_summaryreport.length > 0) {
                            ...
                            angular.forEach($scope.get_report, function (dev) {
                                ...
                                INVSTO_PurchaseDate:dev.INVSTO_PurchaseDate,
                                INVSTO_PurOBQty:dev.INVSTO_PurOBQty,
                                 INVSTO_PurchaseRate:dev.INVSTO_PurchaseRate,
                                  Value:dev.Value,
                              INVSTO_SalesRate:dev.INVSTO_SalesRate,
                           INVSTO_SalesQty:dev.INVSTO_SalesQty,
                        INVSTO_AvaiableStock :dev.INVSTO_AvaiableStock,
