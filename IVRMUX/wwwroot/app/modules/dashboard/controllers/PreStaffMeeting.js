(function () {
    ...
    angular
        .module('app')
        .controller('PreMeetingController', PreMeetingController);
    PreMeetingController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', '$q', '$cookieStore', '$stateParams', '$filter', 'FormSubmitter', '$window', 'superCache', '$compile', '$timeout'];
    function PreMeetingController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, $q, $cookieStore, $stateParams, $filter, FormSubmitter, $window, superCache, $compile, $timeout) {
        ...
        $scope.onLoadGetData = function () {
            var pageid = 2;
            apiService.getDATA("PreLiveMeetingSchedule/getempdetails").then(function (promise) {
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        ...
                        $scope.ttt = false;
                        apiService.create("PreLiveMeetingSchedule/endmainmeeting", data).then(function (promise) {
                            //$scope.meetinglist = promise.meetinglist;
        ...
        $scope.SubmitForm = function (user) {
            ...
            if ($scope.myFormmodal.$valid) {
                ...
                apiService.create("PreLiveMeetingSchedule/saveremarks", data).
            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        ...
                        $scope.currentuser = "";
                        $scope.viewstudent = user.pasr_id;
                        $scope.type = 'M';
                        ...
                        apiService.create("PreLiveMeetingSchedule/onstartmeeting", user).then(function (promise) {
                            if ($scope.vcflag === "BBB") {
        ...
        $scope.ondatechange = function () {
            ...
            apiService.create("PreLiveMeetingSchedule/ondatechange", data).then(function (promise) {
                $scope.meetinglist = promise.meetinglist;
                //$scope.joinmeetinglist = promise.joinmeetinglist;
                $scope.totalmeetingsofday = promise.totalmeetingsofday;
                $scope.obj.totalsch = 0;
        ...
        $scope.onschedulecahnge = function (schname) {
            ...
            apiService.create("PreLiveMeetingSchedule/onschedulecahnge", data).then(function (promise) {
                $scope.meetinglist = promise.meetinglist;                
                angular.forEach($scope.meetinglist, function (obj) {
                    if (obj.lmslmeeT_PlannedStartTime.substring(0, 2) > 12) {
                        obj.paotS_ScheduleTimeonly = (Number(obj.lmslmeeT_PlannedStartTime.substring(0, 2)) - 12).toString();
