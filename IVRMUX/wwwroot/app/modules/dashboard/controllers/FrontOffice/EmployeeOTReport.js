(function () {
    ...
    angular
        .module('app')
        .controller('EmployeeOTReportController', EmployeeOTReportController)

    EmployeeOTReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'apiService', 'Flash', '$http', '$q', '$stateParams', '$filter', 'Excel', '$timeout']
    function EmployeeOTReportController($rootScope, $scope, $state, $location, apiService, Flash, $http, $q, $stateParams, $filter, Excel, $timeout) {
        ...
        $scope.GetReport = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                if (groupidss != undefined) {
                    ...
                    apiService.create("EmployeeMonthlyReport/getOTrpt", data).
                        then(function (promise) {

                            if (promise.filldata.length > 0) {
                                $scope.grid_view = true;
                                $scope.yearlyemprep = promise.filldata;
                                $scope.temp_list = promise.filldata;
                                ...
                                //    var totalAbsent = 0;
                                //    totalAbsent = $scope.yearlyemprep[i].absentdays;
                                //    if (totalAbsent > 0) {
                                //        $scope.temp_list.push({
                                //            "ecode": $scope.yearlyemprep[i].ecode, "ename": $scope.yearlyemprep[i].ename,
                                //            "HRMDES_DesignationName": $scope.yearlyemprep[i].HRMDES_DesignationName, "NoOfAbsent": $scope.yearlyemprep[i].absentdays, "hrme_id": $scope.yearlyemprep[i].hrme_id
                                //        });
                                //    }
                                //}
                                //$scope.chktwd1 = false;
