(function () {
    ...
    angular
        .module('app')
        .controller('EmployeeMonthlyReportController', EmployeeMonthlyReportController)
    EmployeeMonthlyReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'apiService', 'Flash', '$http', '$q', '$stateParams', '$filter', 'Excel', '$timeout']
    function EmployeeMonthlyReportController($rootScope, $scope, $state, $location, apiService, Flash, $http, $q, $stateParams, $filter, Excel, $timeout) {
        //$scope.currentPage = 1;
        ...
        $scope.GetReport = function () {
            if ($scope.myForm.$valid) {
                ...
                if (groupidss !== undefined) {
                    ...
                    apiService.create("EmployeeMonthlyReport/getrpt", data).then(function (promise) {

                        if (promise.filldata !== null && promise.filldata.length > 0) {
                            ...
                            //$scope.columnnum = $scope.columnnames.length + 6;
                            $scope.columnnum = promise.filldata.length + 2;
                            $scope.yearlyemprep = promise.filldata;
                            ...
                            $scope.totalworkingdays = diffDays+1;
                            $scope.absentList = [];
                            for (var i = 0; i < $scope.yearlyemprep.length; i++) {
                                var totalAbsent = 0;
                                var totalHoliday = 0;
                                for (var k = 0; k < $scope.columnnames.length; k++) {
                                    if ($scope.yearlyemprep[i][$scope.columnnames[k]] != 0 && $scope.yearlyemprep[i][$scope.columnnames[k]] != 1)
                                    {
                            ...
                            // row headers of new format for St.James Client
                            $scope.TempNameList = [];
                            angular.forEach($scope.yearlyemprep, function (item) {
                                if ($scope.TempNameList.length === 0) {
                                    $scope.TempNameList.push({ ename: item.ename, HRME_Id: item.HRME_Id, tpdays: item.tpdays, hwkdays: item.hwkdays, HRME_DOJ: item.HRME_DOJ });
                                }
                                else if ($scope.TempNameList.length > 0) {
                                    var count = 0;
                                    angular.forEach($scope.TempNameList, function (item1) {
                                        if (item1.HRME_Id === item.HRME_Id) {
                                            count += 1;
                                        }
                                    });
                                    if (count === 0) {
