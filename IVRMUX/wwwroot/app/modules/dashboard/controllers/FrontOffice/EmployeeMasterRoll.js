(function () {
    ...
    angular
        .module('app')
        .controller('EmployeeMasterRollReportController', EmployeeMasterRollReportController)

    EmployeeMasterRollReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'apiService', 'Flash', '$http', '$q', '$stateParams', '$filter', 'Excel', '$timeout']
    function EmployeeMasterRollReportController($rootScope, $scope, $state, $location, apiService, Flash, $http, $q, $stateParams, $filter, Excel, $timeout) {
        //$scope.currentPage = 1;
                ...
                if (groupidss != undefined) {
                    ...
                    apiService.create("EmployeeMonthlyReport/getrpt", data).
                        then(function (promise) {
                            ...
                            if (promise.filldata.length > 0 && promise.filldataS.length > 0) {
                                $scope.yearlyempreps = promise.filldataS;
                                //var str = $scope.yearlyemprepD;
                                //var res = str.replace("Earned Basic", "baSIC");
                                //$scope.yearlyempreps = res;
                                ...
                                for (var i = 0; i < $scope.yearlyemprep.length; i++) {
                                    var totalAbsent = 0;
                                    totalAbsent = $scope.totalworkingdays - $scope.yearlyemprep[i].tpdays;
                                    if (totalAbsent > 0) {
