...
(function () {
    ...
    angular
.module('app')
        .controller('CLGConsolidatedReportController', CLGConsolidatedReportController)

    CLGConsolidatedReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'apiService', 'Flash', '$http', '$q', '$stateParams', '$filter', 'Excel', '$timeout']
    function CLGConsolidatedReportController($rootScope, $scope, $state, $location, apiService, Flash, $http, $q, $stateParams, $filter, Excel, $timeout) {
        ...
        $scope.LoadData = function () {            
            apiService.getDATA("CLGConsolidatedReport/getalldetails").
       then(function (promise) {
        ...
        $scope.GetReport = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("CLGConsolidatedReport/getrpt", data).
                    then(function (promise) {
                        debugger;
                        $scope.yearrname = '';
                        ...
                        if ($scope.timetablelist.length > 0 && $scope.timetablelist != null) {
                            ...
                            if ($scope.TYPE == 'ST') {
                                            ...
                                            if (ss.hrmE_Id == tt.HRME_Id && pp.ttmP_Id == tt.TTMP_Id) {
                                                ...
                                                if (count == 0) {
                                                    ...
                                                    if (tt.MINID == tt.MAXID) {
                                                        var prd = 1;
                                                        newCol = tt.AMCO_CourseName + '::' + tt.AMB_BranchName + '::' + tt.AMSE_SEMName + '::' + tt.ACMS_SectionName + '::' + tt.ISMS_SubjectName + '::' + tt.MinDay + '::' + prd
                                                    }
                                                    else {
                                                        var prd1 = tt.MAXID - tt.MINID + 1;
                                                        newCol = tt.AMCO_CourseName + '::' + tt.AMB_BranchName + '::' + tt.AMSE_SEMName + '::' + tt.ACMS_SectionName + '::' + tt.ISMS_SubjectName + '::' + tt.MinDay + '-' + tt.MaxDay + '::' + prd1
                                                    }
                                                ...
                                                if (count > 0) {
                                                    ...
                                                    if (tt.MINID == tt.MAXID) {
                                                        newCol = newCol + ' ' + '&&' + ' ' + tt.AMCO_CourseName + '::' + tt.AMB_BranchName + '::' + tt.AMSE_SEMName + '::' + tt.ACMS_SectionName + '::' + tt.MinDay + '::' + prd;
                                                    }
                                                    else {
                                                        var prd1 = tt.MAXID - tt.MINID + 1;
                                                        newCol = newCol + ' ' + '&&' + ' ' + tt.AMCO_CourseName + '::' + tt.AMB_BranchName + '::' + tt.AMSE_SEMName + '::' + tt.ACMS_SectionName + '::' + tt.ISMS_SubjectName + '::' + tt.MinDay + '-' + tt.MaxDay + '::' + prd1;
                                                    }
                                                }
                                ...
                                //        if (ss.amcO_Id == tt.AMCO_Id && ss.amB_Id == tt.AMB_Id && ss.amsE_Id == tt.AMSE_Id && ss.acmS_Id == tt.ACMS_Id) {
                                //            periodlst.push(tt);
                                //        }
                                ...
                                angular.forEach($scope.class_sectons, function (ss) {
                                    ...
                                    angular.forEach($scope.prdlst, function (pp) {
                                        ...
                                        debugger;
                                        angular.forEach($scope.stdtt, function (tt) {
                                            if (ss.amcO_Id == tt.AMCO_Id && ss.amB_Id == tt.AMB_Id && ss.amsE_Id == tt.AMSE_Id && ss.acmS_Id == tt.ACMS_Id && pp.ttmP_Id == tt.TTMP_Id) {
                                ...
                                angular.forEach($scope.daysList, function (dd) {
                                    ...
                                    angular.forEach($scope.period_list, function (pp) {
                                        ...
                                        angular.forEach($scope.brklist, function (tt) {
                                            ...
                                            if (parseInt(pp.ttmP_PeriodName) == tt.ttmbC_AfterPeriod && tt.ttmD_Id == dd.ttmD_Id) {/*&& */
                                                debugger;
                                                cnt += 1; 
                                                ppid = 'id' + pp.ttmP_Id + dd.ttmD_Id + 'id';
                                                    ppname ='BREAK';
                                                ppname1 = 'BREAK-  ' + '(' + tt.ttmbC_BreakStartTime + ' ' + 'TO' + ' ' + tt.ttmbC_BreakEndTime+')';
                                            }

                                        })

                                        var pptime = '';
                                        angular.forEach($scope.periodtimelist, function (vv) {

                                        

                                            if (parseInt(pp.ttmP_Id) == vv.ttmP_Id && vv.ttmD_Id == dd.ttmD_Id) {
                                               
                                                pptime = ' (' + vv.ttmdpT_StartTime + ' ' + 'TO' + ' ' + vv.ttmdpT_EndTime + ')';
                                            }
                                        ...
                                        if (cnt == 0) {
                                            prlist.push({ pid: pp.ttmP_Id, pname: pp.ttmP_PeriodName, title: 'P-' + pp.ttmP_PeriodName + pptime, field: 'id' + pp.ttmP_Id + dd.ttmD_Id, name: 'id' + pp.ttmP_Id + dd.ttmD_Id, width:150 })
                                        }
                                        if (cnt > 0) {
                                            debugger;
                                            prlist.push({ pid: pp.ttmP_Id, pname: pp.ttmP_PeriodName, title: 'P-' + pp.ttmP_PeriodName + pptime, field: 'id' + pp.ttmP_Id + dd.ttmD_Id, name: 'id' + pp.ttmP_Id + dd.ttmD_Id, width: 150})
                                            prlist.push({ pid: ppid, pname: ppname, title: ppname1, field: ppid, name: ppid, width: 200 })
                                        }
                                ...
                                console.log($scope.mainarrayclm);
                               ...
                               // console.log($scope.colarrayall);
                                ...
                                $scope.ttdata = promise.finaltable

                                $scope.maindata = [];
                                angular.forEach(stfidss, function (rr) {
                                    $scope.empdata = [];
                                    var thtt = 0;
                                    var pt = 0;
                                    angular.forEach($scope.ttdata, function (xx) {
                                        if (rr.hrmE_Id == xx.HRME_Id) {
                                            thtt += xx.Theory;
                                            pt += xx.Practical;
                                            $scope.empdata.push(xx);
                                        }
                                    })
                                    if ($scope.empdata.length>0) {
                                        $scope.maindata.push({ HRME_Id: rr.hrmE_Id, STAFF: rr.empName, TT: $scope.empdata, tht: thtt,pt:pt,total:thtt+pt});
                                    }
        ...
        $scope.printData = function () {
            ...
            if ($scope.TYPE == 'STD') {
                ...
                innerContents = document.getElementById("SWDPRNT").innerHTML;
            }
