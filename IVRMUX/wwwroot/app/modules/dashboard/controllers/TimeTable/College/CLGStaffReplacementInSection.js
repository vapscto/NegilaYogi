...
(function () {
    ...
    angular
.module('app')
        .controller('CLGStaffReplacementInSectionController', CLGStaffReplacementInSectionController)

    CLGStaffReplacementInSectionController = ['$rootScope', '$scope', '$state', '$location', 'apiService', 'Flash', '$http', '$q', '$stateParams', 'superCache']
    function CLGStaffReplacementInSectionController($rootScope, $scope, $state, $location, apiService, Flash, $http, $q, $stateParams, superCache) {
        ...
        $scope.GetReport = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("CLGStaffReplacementInSection/getrpt", data).
                    then(function (promise) {
                            ...
                            for (var p = 0; p < $scope.period_list.length; p++) {
                                ...
                                for (var c = 0 ; c < promise.break_list.length; c++) {
                                    ...
                                    if ($scope.AMCO_Id == promise.break_list[c].amcO_Id && $scope.AMB_Id == promise.break_list[c].amB_Id && $scope.AMSE_Id == promise.break_list[c].amsE_Id && parseFloat($scope.period_list[p].ttmP_PeriodName) == promise.break_list[c].ttmbC_AfterPeriod) {
                                        ...
                                        $scope.period_break_list.push({ ped_id: 0, ped_name: 'Break', brk_name: promise.break_list[c].ttmbC_BreakName })
                                        break_flag = true;
                                ...
                                for (var i = 0; i < $scope.period_break_list.length; i++) {
                                    ...
                                    for (var k = 0; k < $scope.tt_list.length; k++) {
                                        ...
                                        if ($scope.tt_list[k].ttmP_Id == $scope.period_break_list[i].ped_id && $scope.tt_list[k].ttmD_Id == $scope.day_list[j].ttmD_Id) {
                                            ...
                                            if (count == 0) {
                                                newCol = { pedid: $scope.period_break_list[i].ped_id, dayid: $scope.day_list[j].ttmD_Id, value: $scope.tt_list[k].amcO_CourseName + ' :' + $scope.tt_list[k].amB_BranchName + ' :' + $scope.tt_list[k].amsE_SEMName + ' :' + $scope.tt_list[k].acmS_SectionName + ' :' + $scope.tt_list[k].ismS_SubjectName, pedname: $scope.period_break_list[i].ped_name, dayname: $scope.day_list[j].ttmD_DayName, color: 'black', value1: $scope.tt_list[k].staffName + ' TO ' + $scope.tt_list[k].ismS_SubjectName, stfid: $scope.tt_list[k].hrmE_Id }
                                                count += 1;
                                            }
                                            else if (count > 0) {
                                    ...
                                    if (newCol == "") {
                                        ...
                                        if ($scope.period_break_list[i].ped_id == 0) {
                                            for (var x = 0; x < promise.break_list_all.length; x++) {
                                                if ($scope.day_list[j].ttmD_Id == promise.break_list_all[x].ttmD_Id && $scope.period_break_list[(i - 1)].ped_id == promise.break_list_all[x].ttmbC_AfterPeriod) {

                                                    newCol = { pedid: $scope.period_break_list[i].ped_id, dayid: $scope.day_list[j].ttmD_Id, value1: promise.break_list_all[x].ttmbC_BreakName, pedname: $scope.period_break_list[i].ped_name, dayname: $scope.day_list[j].ttmD_DayName, color: 'black' }
        ...
        $scope.BindData = function () {
            apiService.getDATA("CLGStaffReplacementInSection/getalldetails").
       then(function (promise) {
        ...
        $scope.cell_click = function (dayid, periodid, day, period, hrmeId) {
            ...
            if ($scope.from != null && $scope.from != "") {
                ...
                apiService.create("CLGStaffReplacementInSection/getpossiblePeriod", data).
                then(function (promise) {
                    $scope.table_list = [];
                    $scope.table_list123 = [];
                    $scope.table_list = $scope.temp_grid;
        ...
        $scope.get_category = function () {
            ...
            if ($scope.asmaY_Id === "") {
                ...
                apiService.create("CLGStaffReplacementInSection/get_catg", data).
         then(function (promise) {
        ...
        $scope.replacement_to_save = function () {
            ...
            if ($scope.from != null && $scope.from != "" && $scope.to != null && $scope.to != "") {
                ...
                apiService.create("CLGStaffReplacementInSection/savedetail", data).
                  then(function (promise) {
                      if (promise.returnval === true) {
                          swal("Periods Replaced Sucessfully !");
                          $scope.GetReport();
                      }
                      else {
