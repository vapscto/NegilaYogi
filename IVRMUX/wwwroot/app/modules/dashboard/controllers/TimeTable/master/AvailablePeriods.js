...
(function () {
    ...
    angular
.module('app')
        .controller('AvailablePeriodsController', SMSResendController)
    ...
    function SMSResendController($rootScope, $scope, $state, $location, apiService, Flash, $http, $q, $stateParams, superCache, $filter) {
        ...
        $scope.BindData = function () {
            apiService.getDATA("MasterDay/getavdata").
       then(function (promise) {
        ...
        $scope.allocateperiod = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                angular.forEach($scope.periodlist, function (user) {
                    if (!!user.selected) {
                ...
                if ($scope.albumNameArray.length > 0) {
                    var data = {
                        ...
                        alocateids: $scope.albumNameArray
                    }
                    apiService.create("MasterDay/allocateperiod", data).
                        then(function (promise) {
                            if (promise.returnVal == true) {
                                swal('Allocated Successfully');
                            }
                            else {
                                swal('ERROR');
                                $scope.showdata(); 
        ...
        $scope.showdata = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                apiService.create("MasterDay/getPeriods", data).
                    then(function (promise) {


                        if (promise.periodlist != null && promise.periodlist.length>0) {
