...
(function () {
    ...
    function TTConsolidatedController($rootScope, $scope, $state, $location, apiService, Flash, $http, $q, $stateParams, $filter, Excel, $timeout) {
        ...
        $scope.itemsPerPage3 = 20;
        $scope.itemsPerPage4 = 20;
        $scope.itemsPerPagespc = 20;
        $scope.itemsPerPagecspc = 20;
        $scope.currentPage = 1;
        ...
        $scope.LoadData = function () {
            apiService.getDATA("TTConsolidated/getalldetails").
                then(function (promise) {
                    $scope.year_list = promise.acayear;
                    $scope.categorylst = promise.categorylist;
                    $scope.class_list = promise.classlists;

                    $scope.section_list = promise.sectionlist;
                    //$scope.class_lists = promise.clalist;

                    //$scope.classsslst = promise.classsslist;
                    //$scope.categorylst = promise.categorylist;
        ...
        $scope.all_checkclass = function () {
            ...
            var toggleStatus = $scope.userclasscheck;
        ...
        $scope.togchkbx4 = function () {
            $scope.userclasscheck = $scope.class_list.every(function (options) {
                return options.class;
            });
        }
        $scope.togchkbx5 = function () {
            $scope.usersectioncheck = $scope.section_list.every(function (options) {
                return options.sec;
            });
        }
        ...
        $scope.GetReport = function () {
            ...
            if ($scope.myForm.$valid) {
                ...
                //    angular.forEach($scope.class_list, function (mm) {
                //        if (mm.class == true) {
                ...
                //        }
                //    })
                //    angular.forEach($scope.section_list, function (nn) {
                //        if (nn.sec == true) {
                ...
                //        //"classdss": $scope.classdss,
                //        //"sectiondss": $scope.sectiondss
                //        //ttmC_Id
                //    }
                //}
                ...
                apiService.create("TTConsolidated/getrpt", data).
                    then(function (promise) {
                        ...
                        $scope.categoryName = '';
                        angular.forEach($scope.categorylst, function (cc) {
        ...
        $scope.interacted = function (field) {
            ...
            //var TTFGD_Id = _ttcategorycontext.TT_Final_Generation_DetailedDMO.Where(t => t.ASMCL_Id == data.ASMCL_Id && t.ASMS_Id == data.ASMS_Id && TTMCId == data.TTMC_Id && MIId == data.MI_Id && ASMAY_Id == data.ASMAY_Id && t.TTFG_Id == TTFG_Ids).Select(r => r.TTFGD_Id).OrderByDescending(s => s).FirstOrDefault();



                //var HRMEIds = _ttcategorycontext.TT_Final_Generation_DetailedDMO.Where(t => t.ASMCL_Id == data.ASMCL_Id && t.ASMS_Id == data.ASMS_Id && TTMCId == data.TTMC_Id && MIId == data.MI_Id && ASMAY_Id == data.ASMAY_Id && t.TTFG_Id == TTFG_Ids && t.TTFGD_Id == TTFGD_Id).Select(r => r.HRME_Id).OrderByDescending(s => s).FirstOrDefault();

                //var HRMEId = _ttcategorycontext.TT_Final_Generation_DetailedDMO.Where(t => t.ASMCL_Id == data.ASMCL_Id && t.ASMS_Id == data.ASMS_Id && TTMCId == data.TTMC_Id && MIId==data.MI_Id && ASMAY_Id == data.ASMAY_Id && TTFG_Ids== TTFGD_Id && t.HRME_Id==data.HRME_Id ).Select(r => r.HRME_Id).OrderByDescending(s => s).FirstOrDefault();


                //var ISMSId = _ttcategorycontext.TT_Final_Generation_DetailedDMO.Where(t => t.ASMCL_Id == data.ASMCL_Id && t.ASMS_Id == data.ASMS_Id && TTMCId == data.TTMC_Id && MIId == data.MI_Id && ASMAY_Id == data.ASMAY_Id).Select(r => r.ISMS_Id).OrderByDescending(s => s).FirstOrDefault();

                //var reslts = _ttcategorycontext.TT_Final_Generation_DetailedDMO.Where(R => R.ISMS_Id == data.ISMS_Id && R.ASMCL_Id == data.ASMCL_Id && R.ASMS_Id == data.ASMS_Id && TTMCId == data.TTMC_Id && MIId == data.MI_Id && ASMAY_Id == data.ASMAY_Id && R.TTMD_Id == data.TTMD_Id && R.TTMP_Id == data.TTMP_Id).ToList();
                ...
                //var rstl2 = _ttcategorycontext.TT_Final_Generation_DetailedDMO.Where(R => R.ISMS_Id == data.ISMS_Id && R.ASMCL_Id == data.ASMCL_Id && R.TTMD_Id == item.TTMD_Id && R.TTMP_Id == item.TTMP_Id && R.ASMS_Id == data.ASMS_Id && TTMCId == data.TTMC_Id && MIId == data.MI_Id && ASMAY_Id == data.ASMAY_Id).ToList();
                //if (rstl2.Count > 0)
                //{

                //var rests = _ttcategorycontext.TT_Final_Generation_DetailedDMO.Where(R => R.HRME_Id == data.HRME_Id && R.ISMS_Id == data.ISMS_Id && R.ASMCL_Id == data.ASMCL_Id && R.TTMD_Id == data.TTMD_Id && R.TTMP_Id == data.TTMP_Id && R.ASMS_Id == data.ASMS_Id && TTMCId == data.TTMC_Id && MIId == data.MI_Id && ASMAY_Id == data.ASMAY_Id).ToList();
                ...
                //var ISMSIds = _ttcategorycontext.TT_Final_Generation_DetailedDMO.Where(t => t.ASMCL_Id == data.ASMCL_Id && t.ASMS_Id == data.ASMS_Id && TTMCId == data.TTMC_Id && MIId == data.MI_Id && t.TTMD_Id == item.TTMD_Id && t.TTMP_Id == item.TTMP_Id && ASMAY_Id == data.ASMAY_Id).Select(r => r.ISMS_Id).OrderByDescending(s => s).FirstOrDefault();
