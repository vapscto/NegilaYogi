(function () {
    ...
    angular
        .module('app')
        .controller('PrincipalDashboardJSHSController', PrincipalDashboardJSHSController)
    PrincipalDashboardJSHSController.$inject = ['$rootScope', '$scope', '$state', '$location', 'dashboardService', 'Flash', 'apiService', '$stateParams', '$filter', 'superCache', '$window']
    function PrincipalDashboardJSHSController($rootScope, $scope, $state, $location, dashboardService, Flash, apiService, $stateParams, $filter, superCache, $window) {
        $scope.sms = 0;
        $scope.email = 0;
        $scope.staffbrthlist = [];
        $scope.stdabsentlist = [];
        ...
        $scope.DeletRecord = function (stringdisplay, SweetAlert) {
            swal({
                ...
                if (inputValue !== "") {
                    ...
                    apiService.create("EmployeeDashboard/savepaymentremarks", data).then(function (promise) {
                        if (promise !== null) {
        ...
        $scope.onsmsemail = function () {
            $window.location.href = 'http://' + HostName + '/#/app/chairmanmsemailreport/';
        };

        $scope.stfbirclick = function () {
            $window.location.href = 'http://' + HostName + '/#/app/StaffBirthdayReport/';
        };

        $scope.stdabs = function () {
            $window.location.href = 'http://' + HostName + '/#/app/OverallDailyAttendanceReportNew/';
        };
    }
})();