...
(function () {
    ...
    angular
        .module('app')
        .controller('PushNotifyController', PushNotifyController)
    PushNotifyController.$inject = ['$rootScope', '$scope', '$state', '$location', 'dashboardService', 'Flash', 'apiService', '$stateParams', '$filter', 'superCache']
    function PushNotifyController($rootScope, $scope, $state, $location, dashboardService, Flash, apiService, $stateParams, $filter, superCache) {
    ...
    $scope.itemsPerPage = 10;
        ...
        if ($scope.radioval == 'Student') {
            ...
            $scope.studentList = [];
            apiService.create("PushNotify/Getdetails/", data).
        ...
        if ($scope.radioval == 'Staff') {
            ...
            $scope.employeelst = [];
            $scope.stfmsg = "";
            apiService.getURI("PushNotify/Getdepartment", id).
     then(function (promise) {
    ...
    $scope.get_designationnew = function () {
        ...
        if (groupidss != undefined) {
            ...
            apiService.create("PushNotify/get_designation", data).
            then(function (promise) {
    ...
    $scope.get_employeenew = function () {
        ...
        if (groupidss != undefined) {
            ...
            apiService.create("PushNotify/get_employee", data).
            then(function (promise) {
    ...
    $scope.GetStudentDetails = function () {
        ...
        apiService.create("PushNotify/GetStudentDetails/", data).
   then(function (promise) {
    ...
    $scope.SendData = function () {
        ...
        if ($scope.myForm.$valid) {
            ...
            apiService.create("PushNotify/savedetail", Send).then(function (promise) {
        ...
        if ($scope.printstudents.length > 0) {
         ...
         function (isConfirm) {
             if (isConfirm) {
                 ...
                 apiService.create("PushNotify/savedetail", data).then(function (promise) {
                     if (promise.msg == 'key') {
                         swal("Authentication key is Not Mapped");
                     }
                     else {
                         swal("Notification Sent Count Details :" + "   SUCCESS:" + promise.scnt + "    FAILUAR:" + promise.fcnt);
                         $state.reload();
        ...
        if ($scope.printstudents.length > 0) {
         ...
         function (isConfirm) {
             if (isConfirm) {
                 ...
                 apiService.create("PushNotify/savedetail", data)
                     .then(function (promise) {
