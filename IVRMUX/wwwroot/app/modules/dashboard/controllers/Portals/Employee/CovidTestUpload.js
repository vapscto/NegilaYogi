(function () {
    'use strict';
    angular.module('app').controller('CovidTestUploadController', CovidTestUploadController)

    CovidTestUploadController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', '$filter', '$q', '$window']
    function CovidTestUploadController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, $filter, $q, $window) {
        ...
        $scope.Loaddata = function () {
            ...
            $scope.search = "";
            var pageid = 2;
            apiService.getURI("CovidTestUpload/onloaddata", pageid).then(function (promise) {
        ...
        $scope.saverecord = function () {
            if ($scope.myForm.$valid) {
                var data = {
                    ...
                    "ISTCOVTST_TestResult": $scope.istcovtsT_TestResult,
                    "ISTCOVTST_FileName": $scope.istcovvaC_FileName = 'a',
                    "ISTCOVTST_FilePath": $scope.istcovvaC_FilePath,
                    "ISTCOVTST_TestDate": new Date($scope.istcovtsT_TestDate).toDateString(),
                };

                apiService.create("CovidTestUpload/saverecord", data).then(function (promise) {
                    if (promise !== null) {
        ...
        $scope.EditData = function (indl) {
            ...
            $scope.istcovvaC_VaccinationDate = indl.istcovvaC_VaccinationDate;
            $scope.imcovvaC_Id = indl.imcovvaC_Id;
            $scope.istcovvaC_Dose = indl.istcovvaC_Dose;
            $scope.istcovvaC_FileName = indl.istcovvaC_FileName;
            $scope.istcovvaC_FilePath = indl.istcovvaC_FilePath;
            $scope.scroll();
        };
        $scope.deactiveY = function (user, SweetAlert) {
            var data = {
                "ISTCOVTST_Id": user.istcovtsT_Id,
                "ISTUCOVTST_Id": user.istucovtsT_Id
            };
            var dystring = "";
            if (user.istcovtsT_ActiveFlag === true) {
                dystring = "Deactivate";
            }
            else if (user.istcovtsT_ActiveFlag === false) {
                dystring = "Activate";
            }
            swal({
                title: "Are you sure?",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        apiService.create("CovidTestUpload/deactiveY", data).then(function (promise) {
