(function () {
    'use strict';
    angular.module('app').controller('EmployeeProfileUpdateApprovalController', EmployeeProfileUpdateApprovalController);
    EmployeeProfileUpdateApprovalController.$inject = ['$window', '$rootScope', '$scope', '$state', '$location', 'apiService', 'Flash', '$http', '$q', '$stateParams', '$filter', '$sce'];
    function EmployeeProfileUpdateApprovalController($window, $rootScope, $scope, $state, $location, apiService, Flash, $http, $q, $stateParams, $filter, $sce) {

        $scope.obj = {};
        $scope.loaddataprofileupdateapproval = function () {

            var pageid = 2;
            $scope.itemsPerPage = 10;
            $scope.currentPage = 1;

            apiService.getURI("EmployeeProfileUpdateApproval/loaddataprofileupdateapproval", pageid).then(function (promise) {

                if (promise !== null) {
                    $scope.GetRequestedData = promise.getRequestedData;
                    $scope.GetRequestedDataList = promise.getRequestedDataList;
                };
            });
        };

        $scope.OnChangeOfEmployee = function () {
            ...
            apiService.create("EmployeeProfileUpdateApproval/OnChangeOfEmployee", data).then(function (promise) {
        ...
        $scope.SaveApprovedData = function (dd) {
            ...
            if ($scope.myForm1.$valid) {
                swal({
                    ...
                    if (isConfirm) {
                        ...
                        apiService.create("EmployeeProfileUpdateApproval/SaveApprovedData", data).then(function (promise) {
