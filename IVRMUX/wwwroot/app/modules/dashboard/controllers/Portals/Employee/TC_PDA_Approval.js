(function () {
    ...
    angular
        .module('app')
        .controller('TC_PDA_ApprovalController', TC_PDA_ApprovalController)

    TC_PDA_ApprovalController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache']
    function TC_PDA_ApprovalController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache) {
        ...
        $scope.loaddata = function () {
            ...
            var pageid = 2;
            apiService.getURI("EmployeeLeaveApply/getdata_PDA", pageid)
                .then(function (promise) {
                    $scope.student_dd = promise.student_dd;
                    $scope.tc_fda_list = promise.tc_fda_list;
                    $scope.presentCountgrid = $scope.tc_fda_list.length;
                    $scope.editopn = false;
                })
        }
        ...
        $scope.savedata = function () {
            ...
            if ($scope.myForm.$valid) {
                var data = {
                    ...
                    "ATCPDAAPP_ApprovedDate": $scope.ATCPDAAPP_ApprovedDate,
                    "ATCPDAAPP_ApprovalFlg": true,
                  
                    "ATCPDAAPP_Remarks": $scope.ATCPDAAPP_Remarks,
                    "ATCPDAAPP_Id": $scope.Id
                }
                apiService.create("EmployeeLeaveApply/SaveEdit_PDA", data).
                    then(function (promise) {
                        ...
                        if (promise.returndata === "Update") {
                            ...
                            $scope.AMST_Id = "";
                            $scope.studentname = "";
                            $scope.ATCPDAAPP_ApprovedDate = "";
                            $scope.ATCPDAAPP_ApprovalFlg = false;
                            $scope.ATCPDAAPP_Remarks = "";
                            $scope.libstudetails = [];
                            $scope.exmdetails = [];
                            $scope.loaddata();
                            return;
        ...
        //    apiService.getURI("EmployeeLeaveApply/details_PDA", pageid).
        //        then(function (promise) {
        //            $scope.editopn = true;
        ...
        $scope.getstudetails = function (ww) {
            ...
            apiService.create("EmployeeLeaveApply/getstudetails_PDA", data).then(function (promise) {
        ...
        //        function (isConfirm) {
        //            if (isConfirm) {
        //                apiService.create("EmployeeLeaveApply/deactivate_PDA", flr).
        //                    then(function (promise) {
