(function () {
    ...
    angular
        .module('app')
        .controller('CLGAlumnistudentReportController', CLGAlumnistudentReportController)
    CLGAlumnistudentReportController.$inject = ['$rootScope', '$scope', '$state', '$location', 'Flash', 'appSettings', 'apiService', '$http', 'superCache', 'Excel', '$timeout', '$filter']
    function CLGAlumnistudentReportController($rootScope, $scope, $state, $location, Flash, appSettings, apiService, $http, superCache, Excel, $timeout, $filter) {
        ...
        $scope.checkstudent = function (courseId,branchId,semid) {
                var data = {
                    "AMCO_Id": courseId,
                    "AMB_Id": branchId,
                    "AMSE_Id": semid,
            ...
            apiService.create("CLGAlumnistudentsearch/getsemdata/", data).then(function (promise) {

                if (promise.searchResult.length>0) {
                    $scope.searchResult = promise.searchResult;
                    $scope.presentCountgrid = promise.searchResult.length;
                    }
                    else {
