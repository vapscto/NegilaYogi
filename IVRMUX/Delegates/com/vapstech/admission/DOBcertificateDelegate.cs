...
{
    public class DOBcertificateDelegate
    {
        private readonly object resource;
        private readonly string serviceBaseUrl;
        private const String JsonContentType = "application/json; charset=utf-8";

        public Adm_M_StudentDTO getdetails(int id)
        {
            ...
            try
            {
                ...
                var response = client.GetAsync("api/DOBcertificateFacade/getdata/"+id).Result;
        ...
        public Adm_M_StudentDTO getStudDatabyclass(Adm_M_StudentDTO data)
        {
            ...
            try
            {
                ...
                var byteContent = new ByteArrayContent(buffer);
                byteContent.Headers.ContentType = new MediaTypeHeaderValue("application/json");
                var response = client.PostAsync("api/DOBcertificateFacade/getstudbyclass", byteContent).Result;


                if (response.IsSuccessStatusCode)
                {
                    activatedeac = response.Content.ReadAsStringAsync().Result;
                    data = JsonConvert.DeserializeObject<Adm_M_StudentDTO>(activatedeac, new JsonSerializerSettings
                    {
        ...
        public Adm_M_StudentDTO GetStudDataById(Adm_M_StudentDTO stuDTO)
        {
            ...
            try
            {
                ...
                var byteContent = new ByteArrayContent(buffer);
                byteContent.Headers.ContentType = new MediaTypeHeaderValue("application/json");
                var response = client.PostAsync("api/DOBcertificateFacade/getStudData", byteContent).Result;
                //
