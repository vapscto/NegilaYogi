...
{
    public class FeesMakerAndCheckerImpl:interfaces.FeesMakerAndCheckerInterface
    {

        public FeeGroupContext _FeeGroupContext;
        public DomainModelMsSqlServerContext _context;
        public FeesMakerAndCheckerImpl(FeeGroupContext frgContext, DomainModelMsSqlServerContext context)
        {
        ...
        public FeesMakerAndCheckerDTO Getreportdetails(FeesMakerAndCheckerDTO data)
        {
            ...
            try
            {
                ...
                var feemasnum = _FeeGroupContext.feemastersettings.Where(t => t.MI_Id == data.MI_ID).ToList();//&& t.userid == data.UserId
                var flag = "";
                ...
                if (makerchecker == "True")
                {
                    ...
                    {
                        cmd.CommandText = "FEE_PAYMENT_APPROVALSTUDENTLIST2022";
            ...
            SqlDbType.VarChar)
                ...
                SqlDbType.BigInt)
                    ...
                    using (var cmd = _FeeGroupContext.Database.GetDbConnection().CreateCommand())
                    {
                        cmd.CommandText = "School_PAYMENTREMARKS";
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandTimeout = 30000;
          ...
          SqlDbType.VarChar)
                    ...
                    if (flag == "T")
                    {
                        ...
                        using (var cmd = _FeeGroupContext.Database.GetDbConnection().CreateCommand())
                        {
                            cmd.CommandText = "Get_Student_Status_Details_Groupwise_updated";
                            cmd.CommandType = CommandType.StoredProcedure;
        ...
        {
            List<FeesMakerAndCheckerDTO> Approver = new List<FeesMakerAndCheckerDTO>();

            var hrmEempid = _FeeGroupContext.Staff_User_Login.Where(t => t.MI_Id == data.MI_ID && t.Id == data.userid).Distinct().ToList();
            long empid = 0;
            ...
            try
            {
                ...
                if (data.studentdata != null)
                {
                    ...
                    {
                        outputval = _FeeGroupContext.Database.ExecuteSqlCommand("FEE_PAYMENT_APPROVAL2022 @p0,@p1,@p2,@p3,@p4", data.studentdata[i].FYP_Id, data.HRME_Id, data.FYPAPP_ApprovedFlg, data.studentdata[i].FYPAPP_Remarks, data.userid);
                    }
                }
                ...
                //if (data.studentdata != null && data.returnval == "save")
                //{
                //    for (int p = 0; p < data.studentdata.Length; p++)
                //    {
                //        var finalapprovalcnt = _FeeGroupContext.FeePaymentDetailsDMO.Where(t => t.MI_Id == data.MI_ID && t.FYP_Id == data.studentdata[p].FYP_Id&& t.FYP_ApprovedFlg == true
                //        ).ToList();
                //        if (finalapprovalcnt.Count > 0)
                //        {
                //            for (int i = 0; i < data.studentdata.Length; i++)
                //            {
                //                var emailidofstudent = _FeeGroupContext.Adm_M_Student.Where(t => t.MI_Id == data.MI_ID && t.AMST_Id == data.studentdata[i].AMCST_Id).ToList();
                ...
                //            for (int i = 0; i < data.studentdata.Length; i++)
                //            {
                //                List<FeesMakerAndCheckerDTO> Tasklist = new List<FeesMakerAndCheckerDTO>();


                //                var finalapproval = _FeeGroupContext.FeePaymentDetailsDMO.Where(t => t.MI_Id == data.MI_ID && t.FYP_Id == data.studentdata[i].FYP_Id 
                //              //&& t.FYP_ApprovedFlg == true
                //                ).ToList();
                ...
                //                if (emailidofstudent.FirstOrDefault().AMST_emailId != "" && emailidofstudent.FirstOrDefault().AMST_emailId != null)
                //                {
                //                    var checktemplate = _context.smsEmailSetting.Where(a => a.MI_Id == data.MI_ID && a.ISES_Template_Name == "FEEAPPROVEDREC"
                ...
                //                                        while (dataReader.Read())
                //                                        {
                //                                            Tasklist.Add(new FeesMakerAndCheckerDTO
                //                                            {
                ...
                //                            data.username = _FeeGroupContext.applicationUser.Where(d => d.Id == data.userid).Select(t => t.NormalizedUserName).FirstOrDefault();

                //                            using (var cmdrec = _FeeGroupContext.Database.GetDbConnection().CreateCommand())
                //                            {
                ...
                //                                            Approver.Add(new FeesMakerAndCheckerDTO
                //                                            {
                ...
                //     //                       var fillstudentviewdetails = (from a in _FeeGroupContext.Fee_T_College_PaymentDMO
                //     //                                                     from b in _FeeGroupContext.Fee_Y_PaymentDMO
                //     //                                                     from c in _FeeGroupContext.Clg_Fee_AmountEntry_DMO
                //     //                                                     from l in _FeeGroupContext.CLG_Fee_College_Master_Amount_Semesterwise
                //     //                                                     from d in _FeeGroupContext.FeeHeadClgDMO
                //     //                                                     from e in _FeeGroupContext.Clg_Fee_Installments_Yearly_DMO
                //     //                                                     from f in _FeeGroupContext.Adm_College_Yearly_StudentDMO
                //     //                                                     from g in _FeeGroupContext.MasterCourseDMO
                //     //                                                     from h in _FeeGroupContext.ClgMasterBranchDMO
                //     //                                                     from k in _FeeGroupContext.CLG_Adm_Master_SemesterDMO
                //     //                                                     from n in _FeeGroupContext.Adm_Master_College_StudentDMO
                //     //                                                     from j in _FeeGroupContext.Fee_Y_Payment_College_StudentDMO
                //     //                                                     from m in _FeeGroupContext.Fee_College_Student_StatusDMO
                ...
                //     //                       var currpaymentdetails = (from a in _FeeGroupContext.FeePaymentDetailsDMO
                //     //                                                 from b in _FeeGroupContext.FeePa
                //     //                                                 from c in _FeeGroupContext.Fee_Y_Payment_PaymentModeDMO
                ...
                //     //                                                 select new FeePaymentDetailsDTO
                //     //                                                 {
        ...
        public string sendmailexamtt(long MI_Id, string mail_id, string url, string emailccdetails, string emailbccdetails, string temp)
        {
            try
            {
                ...
                var template = _context.smsEmailSetting.Where(e => e.MI_Id == MI_Id && e.ISES_Template_Name == temp
                ...
                if (alldetails.Count > 0)
                {
                    ...
                    using (var cmd = _FeeGroupContext.Database.GetDbConnection().CreateCommand())
                    {
                        var template1010 = _context.smsEmailSetting.Where(e => e.MI_Id == MI_Id && e.ISES_Template_Name == "FEEAPPROVEDREC" && e.ISES_MailActiveFlag == true).Select(d => d.IVRMIM_Id).ToList();
