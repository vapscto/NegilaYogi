...
{
    public class StudentBuspassFormImpl : Interfaces.StudentBuspassFormInterface
    {
        ...
        readonly ILogger<StudentBuspassFormImpl> Buspasss;
        //  public StudentApplicationContext _StudentApplicationContext;

        public StudentBuspassFormImpl(PortalContext buspassContext, DomainModelMsSqlServerContext db, FeeGroupContext feecontext, ProspectusContext ProspectusContext, ILogger<StudentBuspassFormImpl> _Buspasss)
        {
        ...
        public StudentBuspassFormDTO academicload(StudentBuspassFormDTO data)
        {
            try
            {
                var aaa = _buspasscontext.Adm_M_Student.Single(f => f.AMST_Id == data.AMST_Id).ASMAY_Id;

                List<AcademicYear> acayr = new List<AcademicYear>();
                var acayrorder = _buspasscontext.AcademicYearDMO.Single(r => r.MI_Id == data.MI_Id && r.Is_Active == true && r.ASMAY_ActiveFlag == 1 && r.ASMAY_Id == data.ASMAY_Id).ASMAY_Order;

                var acayrorder1 = _buspasscontext.AcademicYearDMO.Single(r => r.MI_Id == data.MI_Id && r.Is_Active == true && r.ASMAY_ActiveFlag == 1 && r.ASMAY_Id == aaa).ASMAY_Order;

                var appliedlist = _buspasscontext.Adm_Student_Transport_ApplicationDMO.Where(d => d.MI_Id == data.MI_Id && d.AMST_Id == data.AMST_Id && d.ASTA_ActiveFlag == true).ToList();

                if (appliedlist.Count>0)
                {
                    var appliedcurrlist = _buspasscontext.Adm_Student_Transport_ApplicationDMO.Where(d => d.MI_Id == data.MI_Id && d.AMST_Id == data.AMST_Id && d.ASTA_ActiveFlag == true && d.ASTA_FutureAY==data.ASMAY_Id).ToList();
                  
                    if (appliedcurrlist.Count > 0)
                    {
                        acayr = _buspasscontext.AcademicYearDMO.Where(r => r.MI_Id == data.MI_Id && r.Is_Active == true && r.ASMAY_ActiveFlag == 1 && r.ASMAY_Order >=acayrorder && r.ASMAY_Order>= acayrorder1).ToList();
        ...
        public StudentBuspassFormDTO getloaddata(StudentBuspassFormDTO data)
        {
            try
            {
                ...
                data.prospectusPaymentlist = (from j in _feecontext.Adm_Student_Transport_ApplicationDMO
                                              where (j.AMST_Id == data.AMST_Id && j.ASTA_FutureAY== data.transportyear && j.ASTA_Amount>0)
                                              select new StudentBuspassFormDTO
                                              {
                                                  AMST_Id = j.AMST_Id,
                                                  ASTA_Id = j.ASTA_Id,
                ...
                if (rolename == "Student")
                {
                    ...
                    data.routeDetails = (from a in _buspasscontext.Adm_Student_Transport_ApplicationDMO
                                         ...
                                         where (a.AMST_Id == d.AMST_Id && d.MI_Id == data.MI_Id && d.MI_Id == a.MI_Id && d.AMST_ActiveFlag == 1 && d.AMST_SOL == "S" && a.AMST_Id == data.AMST_Id && a.ASTA_FutureAY==data.transportyear)
                                         select new StudentBuspassFormDTO
                                         {
                                             AMST_Id = a.AMST_Id,
                                             ASTA_Id = a.ASTA_Id,
                ...
                if (allyear.Count > 0)
                {
                    ...
                    if (studentcurrentyear.Length > 0)
                    {
                        if (studentcurrentyear.FirstOrDefault().ASMAY_Id != data.transportyear)
                        {
                            ...
                            List<StudentBuspassFormDTO> temp_group = new List<StudentBuspassFormDTO>();
                            ...
                            var openingbal = (from a in _buspasscontext.FeeStudentTransactionDMO
                                              where (a.MI_Id == data.MI_Id && a.ASMAY_Id == data.studentaccyear && grp_ids.Contains(a.FMG_Id) && a.AMST_Id == data.AMST_Id && a.FSS_ToBePaid > 0)
                                              select new StudentBuspassFormDTO
                                              {
                            ...
                            if (openingbal == 0)
                            {
                                data.routeDetails = (from a in _buspasscontext.Adm_Student_Transport_ApplicationDMO
                                                     from d in _buspasscontext.Adm_M_Student
                                                     where (a.AMST_Id == d.AMST_Id && d.MI_Id == data.MI_Id && d.MI_Id == a.MI_Id && a.AMST_Id == data.AMST_Id)
                                                     select new StudentBuspassFormDTO
                                                     {
                                                         AMST_Id = a.AMST_Id
                    ...
                    var acayrorder = (from a in _buspasscontext.AcademicYearDMO
                                      where (a.MI_Id == data.MI_Id && a.ASMAY_ActiveFlag == 1 && a.ASMAY_Id == data.studentaccyear)
                    ...
                    {
                        acaorder = acayrorder[s].ASMAY_Order;
                    }

                    List<AcademicYear> acayr = new List<AcademicYear>();
                    acayr = _buspasscontext.AcademicYearDMO.Where(r => r.MI_Id == data.MI_Id && r.ASMAY_ActiveFlag == 1 && r.ASMAY_Order >= acaorder).ToList();
                    data.fillyear = acayr.ToArray();

                    List<AcademicYear> curracayr = new List<AcademicYear>();
                    curracayr = _buspasscontext.AcademicYearDMO.Where(r => r.MI_Id == data.MI_Id && r.ASMAY_ActiveFlag == 1 && r.ASMAY_Id == data.transportyear).ToList();
        ...
        public StudentBuspassFormDTO savedata(StudentBuspassFormDTO data)
        {
            try
            {
                ...
                var duplicatecount = _buspasscontext.Adm_Student_Transport_ApplicationDMO.Where(t => t.MI_Id == data.MI_Id && t.AMST_Id == data.AMST_Id
                ...
                if (duplicatecount == 0)
                {
                    ...
                    //praveen added
                    var check_regnew = _feecontext.AdmissionStudentDMO.Where(t => t.MI_Id == data.MI_Id && t.AMST_Id == data.AMST_Id && t.AMST_ActiveFlag == 1 && t.AMST_SOL == "S" && t.ASMAY_Id == data.transportyear).ToList();
                    ...
                    {
                        var year_order = (from a in _feecontext.AcademicYear
                                          where (a.MI_Id == data.MI_Id && a.ASMAY_Id == data.transportyear && a.ASMAY_ActiveFlag == 1)
                                          select new StudentBuspassFormDTO
                                          {
                                              ASMAY_Order = a.ASMAY_Order - 1
                                          }).ToList();
                        var year_order_id = (from a in _feecontext.AcademicYear
                                             where (a.MI_Id == data.MI_Id && a.ASMAY_Order == year_order[0].ASMAY_Order && a.ASMAY_ActiveFlag == 1)
                                             select new StudentBuspassFormDTO
                                             {
                                                 ASMAY_Id = a.ASMAY_Id
                                             }).ToList();
                        ...
                        data.regularnew = (from a in _feecontext.FeeGroupDMO
                                           ...
                                           from c in _feecontext.FeeStudentTransactionDMO
                                           from d in _feecontext.FeeHeadDMO
                                           where (b.FMG_Id == a.FMG_Id && b.FMH_Id == d.FMH_Id && c.FMG_Id == b.FMG_Id && c.FMH_Id == b.FMH_Id && c.FMG_Id == a.FMG_Id && c.FMH_Id == d.FMH_Id && c.ASMAY_Id == b.ASMAY_Id && d.FMH_Flag == "T" && c.MI_Id == data.MI_Id && c.ASMAY_Id == year_order_id[0].ASMAY_Id && c.FSS_CurrentYrCharges > 0 && c.AMST_Id == data.AMST_Id)
                                           select new StudentBuspassFormDTO
                                           {
                                               AMST_Id = c.AMST_Id
                                           }
                    ...
                    studentdetails = _buspasscontext.Adm_M_Student.Where(t => t.AMST_Id == data.AMST_Id).ToList();
                    ...
                    if (studentdetails.Count() > 0)
                    {
                        ...
                        data.payementcheck = (from a in _feecontext.FeeAmountEntryDMO
                                              ...
                                              from h in _feecontext.feeYCC
                                              from i in _feecontext.Fee_Y_Payment_School_StudentDMO
                                              where (a.FMG_Id == d.FMG_Id && a.FMH_Id == f.FMH_Id && a.FMCC_Id == h.FMCC_Id && g.FYCC_Id == h.FYCC_Id && b.FMA_Id == a.FMA_Id && h.ASMAY_Id == a.ASMAY_Id && i.FYP_Id == b.FYP_Id && f.FMH_Flag == "T" && d.FMG_CompulsoryFlag == "T" && i.AMST_Id == data.AMST_Id)
                ...
                else if (duplicatecount > 0)
                {
                    ...
                    var admissstatus = _buspasscontext.Adm_Student_Transport_ApplicationDMO.Where(t => t.MI_Id == data.MI_Id && t.AMST_Id == data.AMST_Id && t.ASTA_ApplStatus == "Approved" && t.ASTA_FutureAY == data.ASMAY_Id).Count();
                    if (admissstatus == 0)
                    {
        ...
        public Array paymentPart(StudentBuspassFormDTO enq)
        {
            ...
            try
            {
              //  paymentdetails = _ProspectusContext.Fee_PaymentGateway_DetailsDMO.Where(t => t.MI_Id == enq.MI_Id).ToList();
                ...
                paymentdetails = _feecontext.Fee_PaymentGateway_Details.Where(t => t.MI_Id == enq.MI_Id && t.FPGD_PGName == enq.paytype).Distinct().ToList();
                ...
                var FeeAmountresult = (from a in _feecontext.feeYCC
                                       ...
                                       where (d.FMH_Id == g.FMH_Id && d.FMCC_Id == a.FMCC_Id && a.ASMAY_Id == d.ASMAY_Id && a.FYCC_Id == c.FYCC_Id && d.ASMAY_Id == enq.ASMAY_Id && d.MI_Id == enq.MI_Id && d.FMG_Id == e.FMG_Id && g.FMH_Flag == "NT" && c.ASMCL_Id == enq.ASMCL_Id)
                                       select new FeeAmountEntryDMO
                                       {
                                           FMA_Id = d.FMA_Id,
                ...
                if (FeeAmountresult != null)
                {
                    ...
                    PaymentDetailsDto.transaction_response_url = "http://localhost:57606/api/StudentBuspassForm/paymentresponse/";
        ...
        public PaymentDetails Razorpaypaymentresponse(PaymentDetails response)
        {
            try
            {
                ...
                var accountvalidation = (from a in _feecontext.Fee_PaymentGateway_Details
                ...
                var fetchstudentdeatils = (from a in _feecontext.AdmissionStudentDMO
                                           ...
                                           where (a.AMST_Id==b.AMST_Id && a.AMST_Id == Convert.ToInt64(fetchfmhotid[0].Amst_Id))
                                           select new FeeStudentTransactionDTO
                                           {
                ...
                for (int r = 0; r < fetchfmhotid.Count(); r++)
                {
                    ...
                    else
                    {
                        ...
                        fet.SOURCE = "";
                        fet.RECIPIENT = "";
                        fet.AMOUNT = (Convert.ToInt32(fetchamount.FirstOrDefault().FMA_Amount.ToString()) * 100).ToString();
                ...
                if (response.status == "success")
                {
                    ...
                    if (groups.Count == 0)
                    {
                        ...
                        {
                            List<MasterConfiguration> mstConfig = new List<MasterConfiguration>();
                            mstConfig = _db.mstConfig.Where(d => d.MI_Id.Equals(fetchfmhotid[0].MI_Id) && d.ASMAY_Id.Equals(fetchfmhotid[0].ASMAY_Id)).ToList();

                            var studDet = _db.Adm_Student_Transport_ApplicationDMO.Where(t => t.MI_Id == fetchfmhotid[0].MI_Id && t.ASTA_FutureAY == fetchfmhotid[0].ASMAY_Id && t.AMST_Id == fetchfmhotid[0].Amst_Id).ToList();

                            SMS sms = new SMS(_db);
                            string s = sms.sendSms(fetchfmhotid[0].MI_Id, Convert.ToInt64(studDet.FirstOrDefault().ASTA_FatherMobileNo), "TRANSPORT_REG", fetchfmhotid[0].Amst_Id).Result;

                            var studDett = _db.Adm_M_Student.Where(t => t.MI_Id == fetchfmhotid[0].MI_Id && t.AMST_Id == fetchfmhotid[0].Amst_Id).ToList();

                            Email Email = new Email(_db);
                            string m = Email.sendmail(fetchfmhotid[0].MI_Id, studDett.FirstOrDefault().AMST_emailId, "TRANSPORT_REG", fetchfmhotid[0].Amst_Id);
                        }
                    }
